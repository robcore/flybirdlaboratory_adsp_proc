...
NOTE: The @brief description above does not appear in the PDF.
      ...
      Elite_APPI_mainpage.dox file. Contact Tech Pubs for assistance.
===========================================================================*/

/* =========================================================================
  Copyright (c) 2010-2011, 2013-2014 Qualcomm Technologies, Inc.(QTI)
...
/* =========================================================================
   ...
   04/04/13   ss      Added functionality driven by Voice requirements.
   02/27/13   sw      (Tech Pubs) Edited Doxgen markup and comments for 2.0.
   04/19/11   sw      (Tech Pubs) Final edits for Rev D (of the PDF).
   03/25/11   ss      Put in supporting conditions for APPI
                      (1) Replaced non-interleaved with deinterleaved.
                      (2) Updated all possible data formats that we support currently.
                      (3) Updated information about non-tuning parameters
   02/04/11   ss      Updated comments section with various implementation
                      time specifics such as:
                      (1) Usage of actual_data_len and max_data_len fields
                      (2) Specific debug information to be printed.
                      (3) How APPI Wrappers should deal with data formats.
   12/03/10   sw      (Tech Pubs) Final edits for Rev B (of the PDF).
   11/17/10   ss      (1) ASW Coding convention changes.
                      (2) Data pointer is now int8_t* instead of the earlier int16_t*.
                      (3) Modified reinit function prototype.
                      (4) The set_param input pointer is now const type.
                      (5) Added getsize and init function prototypes.
   10/08/10   sw     (Tech Pubs) Edited/added Doxygen comments and markup.
   ========================================================================= */

#include "qurt_elite_types.h"

/** @addtogroup appi_macros
@{ */

/** Used by the library to reset all internal buffers and filter states.
  Typically, this is set when a seek operation is performed.

  This parameter is supported for appi_vtbl_t::set_param() only, and there is
  no payload. 
*/
#define APPI_PARAM_ID_ALGORITHMIC_RESET 0x00010707

/** Number of zero samples that must be passed into the input of the library
  to ensure that its internal buffers and filter states all become zero.

  For IIR filters, the group delay in terms of number of samples can be
  returned.

  This parameter is supported for appi_vtbl_t::get_param() only. The payload
  consists of one uint32_t value.
*/
#define APPI_PARAM_ID_ALGORITHMIC_DELAY 0x00010708

/** Used to optimize performance.

  If the module is not doing any work (for example, it is disabled), it returns
  FALSE (0) when the APPI service queries for this parameter. In this case, the
  appi_vtbl_t::process() function for this module is not called.

  The service queries this parameter after every appi_vtbl_t::set_param() call
  and every  appi_vtbl_t::reinit() call. The module returns TRUE (1) if it
  wants the service to resume calling its process() function.
...
/** Propagates a media format change at runtime due to a change when a module
  ID is configured via appi_vtbl_t::set_param().
 */
#define APPI_PARAM_ID_MEDIA_FORMAT_CHANGE  0x000108D5

/** Propagates a delayed media format change at runtime due to a delayed
  change when a module ID is configured via appi_vtbl_t::set_param().
 */
#define APPI_PARAM_ID_DELAYED_MEDIA_FORMAT_CHANGE	   0x00010909

/** Used for Kilo Packets Per Second (KPPS) queries. This is required for
  updating the clock voting of a module.

  The value is an unsigned 32-bit integer (it uses the same structure as is
  used for #APPI_PARAM_ID_ALGORITHMIC_DELAY). 

  This parameter is supported for appi_vtbl_t::get_param() only. @newpage
*/
#define APPI_PARAM_ID_KPPS               0x00010776

/** Sets the handle of the peer service.

  For example, for voice, the Rx path on the device serves as the peer to the
  Tx path on the device, and vice versa.

  This parameter is supported for appi_vtbl_t::get_param() only.
*/
#define APPI_PARAM_ID_PEER               0x00010777

/** Notifies appi_vtbl_t::set_param() that the parameter structure being sent
  must be read in the voice_param_data_t calibration payload format.
...
/** Sets the data format property.
  The content is associated with the appi_prop_t structure.
*/
#define APPI_PROP_ID_DATA_FORMAT        0x00010779

/** Sets the processing frame size property.

  The frame size is an unsigned 32-bit integer that represents the size (in
  bytes) per channel of the primary data input. @newpage
*/
#define APPI_PROP_ID_FRAME_SIZE         0x0001077A

/** Unique identifier for kilobits per second (kbps).

  This value must be provided by considering cache size as zero, which gives
  the worst kbps. The value is an unsigned 32-bit integer (it uses the same
  structure as is used for #APPI_PARAM_ID_ALGORITHMIC_DELAY).

  This parameter is supported for appi_vtbl_t::get_param() only.
*/
#define APPI_PARAM_ID_KBPS               0x000108C8

/** Indicates the format of the compressed data in IEC61937
  (AC3, EAC3, DTS, etc.) (refer to @xhyperref{S1,[S1]}).

  This parameter is supported for appi_vtbl_t::set_param() only.
  The payload is a 32-bit integer.
*/
#define APPI_PARAM_ID_COMPRESSED_FORMAT 0x0001076D

/** Communicates a headroom requirement to the Volume Control module through
  the APPI service.

  Currently, this is used only by the popless equalizer.
*/
#define APPI_PARAM_ID_ALGORITHMIC_HEADROOM 0x000108d8
