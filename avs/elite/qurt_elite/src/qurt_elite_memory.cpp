...
$Header: //components/rel/avs.adsp/2.6.1/elite/qurt_elite/src/qurt_elite_memory.cpp#3 $
...
static void qurt_elite_memory_stats_update(void* ptr, uint32_t isMalloc);
...
void *qurt_elite_memory_malloc(uint32_t unBytes, QURT_ELITE_HEAP_ID heapId)
{
   ...
   qurt_elite_memory_stats_update(ptr,IS_MALLOC);

#ifdef DEBUG_QURT_ELITE_MEMORY
   MSG_3(MSG_SSID_QDSP6, DBG_HIGH_PRIO, "QURT_ELITE_MALLOC_FREE_TRACER: Thread 0x%x mallocs %lu bytes, gets ptr 0x%p",
...
void qurt_elite_memory_free(void *ptr)
{
  ...
  qurt_elite_memory_stats_update(ptr,IS_FREE);
  free(ptr);
}
...
{
   qurt_elite_mutex_init(&qurt_elite_globalstate.avs_stats.lock);
   qurt_elite_globalstate.avs_stats.num_mallocs = 0;
   qurt_elite_globalstate.avs_stats.num_frees = 0;
   qurt_elite_globalstate.avs_stats.curr_heap = 0;
   qurt_elite_globalstate.avs_stats.peak_heap = 0;
   /* As the count of NON AVS malloc and free is not available, set it to -1 */
...
{
   qurt_elite_mutex_destroy(&qurt_elite_globalstate.avs_stats.lock);
   qurt_elite_globalstate.avs_stats.num_mallocs = 0;
   qurt_elite_globalstate.avs_stats.num_frees = 0;
   qurt_elite_globalstate.avs_stats.curr_heap = 0;
   qurt_elite_globalstate.avs_stats.peak_heap = 0;
...
static void qurt_elite_memory_stats_update(void* ptr, uint32_t isMalloc)
{
	...
	qurt_elite_mutex_lock(&qurt_elite_globalstate.avs_stats.lock);
	...
	if(isMalloc)
	{
		qurt_elite_globalstate.avs_stats.curr_heap += unBytes;
		qurt_elite_globalstate.avs_stats.num_mallocs ++;
	}
	else
	{
		qurt_elite_globalstate.avs_stats.curr_heap -= unBytes;
		qurt_elite_globalstate.avs_stats.num_frees ++;
		}
	if (qurt_elite_globalstate.avs_stats.curr_heap > qurt_elite_globalstate.avs_stats.peak_heap)
		{
        qurt_elite_globalstate.avs_stats.peak_heap = qurt_elite_globalstate.avs_stats.curr_heap;
		}
	
	/*Update the NON-AVS heap usage statistics */
	qurt_elite_globalstate.non_avs_stats.curr_heap = amssheap_get_current_heap_usages() - qurt_elite_globalstate.avs_stats.curr_heap;
    ...
    qurt_elite_mutex_unlock(&qurt_elite_globalstate.avs_stats.lock);
}
