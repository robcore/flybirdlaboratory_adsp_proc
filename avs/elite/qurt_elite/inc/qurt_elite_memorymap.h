...
$Header: //components/rel/avs.adsp/2.6.1/elite/qurt_elite/inc/qurt_elite_memorymap.h#3 $
...
/** L1 is uncached, and L2 is an uncached shared type. @newpage */
...
/**
  ...
  @param[in]  client_token           Client token.
  @param[in]  shm_mem_reg_ptr        Pointer to a single heap memory region to
                                     map.
  @param[out] shm_mem_map_handle_ptr Pointer to the memory map handle of the
...
/**
  ...
  There is no operation if the Memory Mapping mode is
  #QURT_ELITE_MEMORYMAP_HEAP_ADDR_MAPPING.
...
/**
  Increments the reference count of a client's memory map handle.
  ...
  @par
  A nonzero reference count prevents the specific memory map regions from being
  unmapped by the aDSP client.
...
/**
  Decrements the reference count of a client's memory map handle.
  ...
  longer required by this client.
  @par
