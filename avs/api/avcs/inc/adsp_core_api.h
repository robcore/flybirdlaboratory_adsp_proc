...
$Header: //components/rel/avs.adsp/2.6.1/api/avcs/inc/adsp_core_api.h#3 $

when       who     what, where, why
--------   ---     -------------------------------------------------------
10/18/14   sw      (Tech Pubs) Merged Doxy markup/comments from 2.4.
...
/**
  ...
  @detdesc
    ...
    64 (#AVS_MAX_PROFILED_SW_THREADS). @newpage
...
/** @ingroup core_event_profiling
  ...
  @par Software thread information (avcs_sw_thread_info)
  ...
  @par PMU mode information
    ...
    @par
    The mapping of pmu_mode to measured PMU events is shown below.
    @par
    @inputtable{0x0001290B_PMU_mode_mapping_info.tex} @newpage
...
/** @ingroup core_event_state
  ...
  @description
    ...
    latent response for previous requests). The clients proceed to exercise
    AVS static services.
    @par
...
struct avcs_event_periodic_profiling
{
    ...
    uint32_t              profiling_payload_address_lsw;
      /**< Pointer to the lower 32 bits of the physical address of the
           payload, %avcs_event_periodic_profiling_t
           (see <b>Profiling payload</b>). */
...
/** @ingroup core_cmd_dolby_license
  ...
  @msg_payload{avcs_service_cmd_set_dolby_license_t}
    This payload comprises the %avcs_service_cmd_set_dolby_license_t, followed
    immediately by the payload, whose format is shown in <b>Payload format</b>.
    @tablens{weak__avcs__service__cmd__set__dolby__license__t}

  @par Payload format
    @inputtable{0x0001290E_Payload_format.tex} @newpage
...
struct avcs_service_cmd_set_dolby_license_t
{
    ...
    uint32_t                 property_flag;
    /**< Indicates whether the shared memory provided is virtual (0) or
         physical (1). */
}
#include "adsp_end_pack.h"
;
...
/** @ingroup core_cmd_get_low_power_segment
  ...
  @description
    ...
    gapless playback is done on a single SLIMbus@regns device or a single I2S
...
/** @ingroup core_resp_low_power_segment
  In response to the #AVCS_CMD_GET_LOW_POWER_SEGMENTS_INFO command, returns
  low-power memory segment information.
...
/** @addtogroup core_resp_low_power_segment
@{ */
...
/** @} */ /* end_addtogroup core_resp_low_power_segment */
...
{
    uint16_t              type;
      /**< Indicates the memory type of this segment.

           @values #READ_ONLY_SEGMENT or #READ_WRITE_SEGMENT only */

    uint16_t              category;
      /**< Indicates whether the segment is audio or operating system.

           @values #AUDIO_SEGMENT or #OS_SEGMENT only. */

    uint32_t              size;
      /**< Size (in bytes) of this segment.

           @values A non-zero value.*/
...
/** @cond OEM_only */
/** @addtogroup core_cmd_set_license
@{ */
/** Sets the license information. Currenly, this is used for both DTS
    Headphone:X@tm (DTS-HPX) and Dolby.

  @apr_hdr_fields
   Opcode -- AVCS_CMD_SET_LICENSE

  @msg_payload{avcs_cmd_set_license}
...
/** @} */ /* end_addtogroup core_cmd_set_license */
...
{
   uint32_t id;
   /**< Unique ID used to refer to the license. 
...
/** @ingroup core_cmd_get_license
  ...
  @msg_payload{avcs_cmd_get_license_validation_result}
...
/** @ingroup core_resp_get_license
  ...
  @msg_payload{avcs_cmdrsp_get_license_validation_result}
