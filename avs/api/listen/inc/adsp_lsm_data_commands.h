...
$Header: //components/rel/avs.adsp/2.6.1/api/listen/inc/adsp_lsm_data_commands.h#3 $
...
/** @ingroup lsmdata_cmd_read
  ...
  @apr_msgpayload{lsm_data_cmd_read_t}
    @tablens{weak__lsm__data__cmd__read__t}
  ...
  @dependencies
    ...
    A memory map handle must first have been obtained from the aDSP using
...
/** Each frame's buffer address must be a physical address that has
    been mapped via #LSM_SESSION_CMD_SHARED_MEM_MAP_REGIONS.
*/
struct lsm_data_cmd_read_t
{
    ...
    /**< Upper 32 bits of the address of the buffer where the aDSP buffers the
         ...
         that has been mapped via #LSM_SESSION_CMD_SHARED_MEM_MAP_REGIONS. 
...
struct lsm_data_event_read_done_t
{
   ...
   /**< Upper 32 bits of the valid, mapped address of the buffered data.
        ...
        provided by the client processor in #LSM_DATA_CMD_READ.
   ...
   /**< Bit field of flags.
        ...
        - 0 -- Frame metadata is absent
         ...
         All other bits are reserved; the aDSP sets them to 0 for the look
         ahead buffer. */
}
#include "adsp_end_pack.h"
;
