...
#define NUMBER_OF_STATIC_SERVICES 11
...
/** License type for getting the Dolby License. */
#define ADSP_LICENSE_TYPE_DOLBY 1

/**
  Gets a pointer to a license from the global license storage.

  @param[in] type   The type of license to be returned.
  @param[out] pSize   The size of the license data in bytes.
  @param[out] ppLicense   A pointer to the license will be stored
                    here. The license must not be modified.

  @return
  ADSP_EOK if the license was found.
  ADSP_ENOTEXIST if the license was not found.

  @dependencies
  The core service must have been created when this function is called.
*/
ADSPResult AdspCoreSvc_GetLicense(const uint32_t type, uint32_t *pSize, void **ppLicense);
