...
$Header: //components/rel/ssc.adsp/2.6.1/pm/src/sns_pm_unit_test.c#1 $
...
typedef struct ut_status_s {
  ...
  sns_pm_client_info_s clients[NUM_MAX_PM_CLIENTS];
} ut_status_s;
...
void sns_pm_configure_ut_set1(void)
{
  ...
  test_package_t1.num_test_cases++;
  test_package_t1.testcases[test_package_t1.num_test_cases].description = "[T1] VOTE_MIPS 50";
  test_package_t1.testcases[test_package_t1.num_test_cases].fn_type = VOTE_MIPS;
  test_package_t1.testcases[test_package_t1.num_test_cases].fn_param.mips = 50;
  test_package_t1.num_test_cases++;
