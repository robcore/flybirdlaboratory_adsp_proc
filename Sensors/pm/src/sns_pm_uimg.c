                                      ...
                                      uint32 latency_us )
{
  ...
  if(sns_pm.status.current_mode == SNS_IMG_MODE_MICRO && sns_pm_uimg_enabled)
  {
    ...
    if(sns_pm.status.max_tolerable_delay > 5)
    {
      //PM_MSG_0(LOW, "Set Latency : Enable XO shutdown in Micro Image");
      if(uSleep_setCXOMode(USLEEP_CXO_SHUTDOWN_ENABLE) != USLEEP_SUCCESS)
      {
    ...
    else
    {
      ...
      if(uSleep_setCXOMode(USLEEP_CXO_SHUTDOWN_DISABLE) != USLEEP_SUCCESS)
      {
        /* TODO : print out failure reason */
      }
    }
...
#ifdef UIMG_LOG
          /* Signal debug task to flush out log packets  */
          if (sns_q_cnt(&sns_debug_logpkt_buff.hold_queue) > 0)
          {
...
===========================================================================*/
static void sns_pm_notify_smsm_client(bool suspend)
{
  int i;
  sns_profiling_log_qdss(SNS_PM_SPARE_BROADCAST, 1, suspend);
  for(i = 0; i < sns_pm.status.num_clients; i++)
  {
    if(sns_pm.status.client_handle[i]->callback != NULL)
    {
