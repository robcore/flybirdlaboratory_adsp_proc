...
/* $Header: //components/rel/ssc.adsp/2.6.1/scm/framework/src/sns_scm.c#4 $ */
/* $DateTime: 2015/02/04 11:36:19 $ */
/* $Author: pwbldsvc $ */
...
/*============================================================================
  ...
  ----------  ---  -----------------------------------------------------------
  2015-02-02  dk   Updated QMD enable/disable logic to use sns_scm_motion_state_s
  2014-11-14  pn   Handles sensor_status indication with unchanged num_clients
  2014-09-03  tc   Removed the canceling of periodic timers
  2014-08-28  sc   Call sns_init_done() before SCM waits for service connection
  2014-07-22  ps   Fix compiler warnings
  2014-06-23  sc   Fixed a typo in sns_scm_task assigning service signal
...
STATIC sns_err_code_e sns_scm_process_smgr_ind(const void *smgrIndPtr)
{
   ...
   for (i = 0; i < sns_scm_data_req_dbase[dataReqId]->algoInstCount; i++)
   {
      ...
      if (sns_scm_algo_dbase[algoInstPtr->algoIndex]->serviceId == SNS_SCM_GYRO_CAL_SVC)
      {
         ...
         //execute the algorithm
         sns_profiling_log_qdss( SNS_SCM_ALGO_UPDATE_ENTER, 1,
            sns_scm_algo_dbase[algoInstPtr->algoIndex]->serviceId );
         sns_scm_algo_dbase[algoInstPtr->algoIndex]->algoApi.
         sns_scm_algo_update(algoInstPtr->stateData.memPtr,
                             algoInstPtr->inputData.memPtr,
                             algoInstPtr->outputData.memPtr);
         sns_profiling_log_qdss( SNS_SCM_ALGO_UPDATE_EXIT, 1,
            sns_scm_algo_dbase[algoInstPtr->algoIndex]->serviceId );
      ...
      else if (sns_scm_algo_dbase[algoInstPtr->algoIndex]->serviceId == SNS_SCM_MAG_CAL_SVC)
      {
         ...
         if (outp->accuracy_HI >= QMAG_CAL_MIN_HI_GOOD_ACCURACY)
         {
            ...
            {
               cal_mag_norm_sq +=
                  FX_FIXTOFLT_Q16_SP(indPtr->Item[0].ItemData[col] - outp->offset_HI[col]) *
                  FX_FIXTOFLT_Q16_SP(indPtr->Item[0].ItemData[col] - outp->offset_HI[col]);
            }
