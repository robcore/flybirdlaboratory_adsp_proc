...
/* $Header: //components/rel/ssc.adsp/2.6.1/smgr/src/sns_rh_mr.c#4 $ */
/* $DateTime: 2014/12/30 14:36:05 $ */
/* $Author: pwbldsvc $ */
  ...
  uint8_t         svc_num)
{
  ...
  msg_ptr->header.timestamp         = sns_em_get_timestamp();
  ...
  sns_os_mutex_pend(sns_rh_mr.req_queue_mutex_ptr, 0, &os_err);
  sns_q_put(&sns_rh_mr.req_queue, &msg_ptr->q_link);
  (void) sns_os_mutex_post(sns_rh_mr.req_queue_mutex_ptr);
  ...
  void* service_cookie)
{
  ...
  for ( i=0; i<ARR_SIZE(sns_rh_connection_list); i++ )
  {
    if ( i == (uint32_t) connection_handle )
    {
      if ( sns_rh_connection_list[i].is_valid )
      {
...
void sns_rh_mr_init(void)
{
  ...
  sns_rh_mr.req_queue_mutex_ptr = sns_os_mutex_create(SNS_SMGR_QUE_MUTEX, &err);
...
void sns_rh_mr_handle_disconnect_sig(void)
{
  ...
  for ( i=0; i<ARR_SIZE(sns_rh_connection_list); i++ )
  {
    ...
    {
      SNS_SMGR_PRINTF2(
        HIGH, "disconnect_sig - conn=%u cli_hndl=0x%x", 
        i, sns_rh_connection_list[i].client_handle);
