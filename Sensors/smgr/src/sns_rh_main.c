...
/* $Header: //components/rel/ssc.adsp/2.6.1/smgr/src/sns_rh_main.c#4 $ */
/* $DateTime: 2015/02/27 12:34:01 $ */
/* $Author: pwbldsvc $ */
    ...
    for ( i=0; i<ARR_SIZE(sensor_ptr->const_ptr->data_types); i++ )
    {
      ...
      if ( ddf_sensor_ptr != NULL )
      {
        ...
        resp_ptr->num_buffered_reports[resp_ptr->SensorInfo.data_type_info_len] =
          sns_smgr_depot_get_depth(sensor_ptr->ddf_sensor_ptr[i]->bimg_depot_ptr);
        if ( dsi_ptr->suid != 0 )
        {
  ...
  sns_rh_mr_req_q_item_s** msg_ptr_ptr)
{
  ...
  else if ( ddf_sensor_ptr->sensor_ptr->sensor_state > SENSOR_STATE_IDLE )
  {
...
SMGR_STATIC void sns_rh_process_msg(void)
{
  ...
  while ( (item_ptr = sns_rh_mr_get_msg()) != NULL )
  {
    ...
    SNS_SMGR_PRINTF4(
      MED, "process_msg - conn=0x%x svc=%d msg=0x%x now=%u",
      item_ptr->header.connection_handle, item_ptr->header.svc_num,
      item_ptr->header.msg_id, time_now);
...
===========================================================================*/
SMGR_STATIC void sns_rh_init(void)
{
  uint8_t  err;
...
/*===========================================================================
  FUNCTION:   sns_rh_smgr_init_wait_loop
===========================================================================*/
/*!
  @brief Waits until SMGR signals init completion.
 ...
 */
/*=========================================================================*/
SMGR_STATIC void sns_rh_smgr_init_wait_loop(OS_FLAG_GRP* sig_grp)
{
  OS_FLAGS  sig_flags, unprocessed_sigs = 0;
  OS_FLAGS  sig_mask = SNS_RH_SMGR_INIT_DONE_SIG;

  sns_os_sigs_add(sig_grp, sig_mask);
  while ( sig_mask != 0 )
  {
    uint8_t  err;
    sig_flags = sns_os_sigs_pend(sig_grp, sig_mask,
                                 OS_FLAG_WAIT_SET_ANY + OS_FLAG_CONSUME, 0, &err);
    SNS_SMGR_PRINTF1(HIGH, "smgr_init_wait_loop - rcvd sigs(0x%08x)", sig_flags);
    unprocessed_sigs |= sig_flags & ~sig_mask;
    if ( sig_flags & SNS_RH_SMGR_INIT_DONE_SIG )
    {
      sns_os_sigs_del(sig_grp, SNS_RH_SMGR_INIT_DONE_SIG);
      SNS_SMGR_PRINTF0(HIGH, "smgr_init_wait_loop - done!");
    }
    sig_mask &= ~sig_flags;
  }
  if ( unprocessed_sigs != 0 )
  {
    SNS_SMGR_PRINTF1(
      HIGH, "smgr_init_wait_loop - delayed sigs=0x%08x", unprocessed_sigs);
    sns_rh_signal_me(unprocessed_sigs);
  }
}
...
void sns_rh_task(void* p_arg)
{
  OS_FLAG_GRP* sig_grp;
  sns_rh_init();
  sig_grp = sns_rh_init_uimg();
  sns_rh_mr_init();
  sns_rh_smgr_init_wait_loop(sig_grp);
