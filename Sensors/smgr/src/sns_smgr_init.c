...
/* $Header: //components/rel/ssc.adsp/2.6.1/smgr/src/sns_smgr_init.c#1 $ */
...
/* Buffer depths expressed in second worth of samples at max supported rate */
#ifdef SNS_HIMEM_UIMG
#define SMGR_UBUFFER_DURATION                   2
#define SMGR_BUFFER_DURATION                   10
#define SMGR_ACCEL_BUFFER_DEPTH             10000
#else
#define SMGR_UBUFFER_DURATION                 0.5
#define SMGR_BUFFER_DURATION                    5
#define SMGR_ACCEL_BUFFER_DEPTH              1000
#endif
...
SMGR_STATIC void sns_smgr_compute_buffer_size(
  sns_smgr_ddf_sensor_s* ddf_sensor_ptr)
{
  sns_smgr_depot_info_s* ui_depot_info_ptr =
    &ddf_sensor_ptr->ddf_sensor_info->uimg_depot_info;
  sns_smgr_depot_info_s* bi_depot_info_ptr =
  ...
  {
    ui_depot_info_ptr->depth = max_rate * SMGR_UBUFFER_DURATION;
  }

  if ( SMGR_SENSOR_TYPE(ddf_sensor_ptr->sensor_ptr, 
                        ddf_sensor_ptr->data_type) == SNS_DDF_SENSOR_ACCEL )
  {
    bi_depot_info_ptr->depth = SMGR_ACCEL_BUFFER_DEPTH;
  }
  else
  {
    bi_depot_info_ptr->depth = max_rate * SMGR_BUFFER_DURATION;
  }

  /* calculate memory size taking care of memory alignment */
  if ( ui_depot_info_ptr->depth > 0 )
  {
    ui_depot_info_ptr->mem_size = 
      (sizeof(sns_smgr_sample_depot_s) + 
       sizeof(sns_smgr_sample_s) * (ui_depot_info_ptr->depth-1) + 
       sizeof(intptr_t)-1) / sizeof(intptr_t) * sizeof(intptr_t);
  }
  bi_depot_info_ptr->mem_size = 
    (sizeof(sns_smgr_sample_depot_s) + 
     sizeof(sns_smgr_sample_s) * (bi_depot_info_ptr->depth-1) + 
     sizeof(intptr_t)-1) / sizeof(intptr_t) * sizeof(intptr_t);
  ...
  SNS_SMGR_PRINTF4(
    LOW, "compute_buf_size - ui=%u/%u bi=%u/%u", 
    ui_depot_info_ptr->depth, ui_depot_info_ptr->mem_size, 
    bi_depot_info_ptr->depth, bi_depot_info_ptr->mem_size);
}
...
SMGR_STATIC bool sns_smgr_allocate_depot_mem(void)
{
  ...
  // uimg_depot_mem_ptr could be NULL if no driver supports uImage.
  // However bimg_depot_mem_ptr should never be NULL because 
  // there must always be a copy of the depot in bigImage.
  ...
  for ( ix = 0; ix < ARR_SIZE(sns_smgr.sensor); ix++ )
  {
    ...
    for ( i=0; i<ARR_SIZE(sensor_ptr->const_ptr->data_types); i++ )
    {
      if ( sensor_ptr->const_ptr->data_types[i] != SNS_DDF_SENSOR__NONE )
      {
        ...
        cs_ptr->sensor_status.last_status_ts = time_now;
...
void sns_smgr_dd_init( void )
{
       ...
       sns_smgr_allocate_depot_mem() )
  {
    ...
    sns_hw_power_rail_config(SNS_PWR_RAIL_OFF);
