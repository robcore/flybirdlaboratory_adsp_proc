...
/* $Header: //components/rel/ssc.adsp/2.6.1/smgr/src/sns_rh_internal_svc.c#7 $ */
/* $DateTime: 2015/03/27 16:06:46 $ */
/* $Author: pwbldsvc $ */
...
SMGR_STATIC void sns_rh_generate_sensor_event_ind(const sns_rh_sensor_event_s* se_ptr)
{
  ...
  {
    SNS_SMGR_PRINTF4(
      HIGH, "sensor_event_ind - conn_hndl=0x%x ev=0x%08x%08x status=%u", 
      q_item_ptr->header.connection_handle,
      se_ptr->sensor_event >> 32, se_ptr->sensor_event, se_ptr->status);
  ...
  uint8_t                                           item_idx)
{
  ...
  if ( q16_rpt_rate > smgr_rate_hz_to_q16(eg_req_ptr->SamplingRate) )
  {
  ...
  sns_smgr_event_gated_buffering_resp_msg_v02* resp_ptr )
{
  ...
  if ( ackNak == SNS_SMGR_RESPONSE_ACK_SUCCESS_V01 )
  {
    ...
    if ( se_ptr->enabled )
    {
      if ( e_rpt_ptr != NULL )
      {
        e_rpt_ptr->sensor_event_q = &se_ptr->enabled_state_rpt_q;
        sns_rh_sol_schedule_report(hdr_ptr, e_rpt_ptr);
      }
      if ( d_rpt_ptr != NULL )
      {
        d_rpt_ptr->sensor_event_q = &se_ptr->disabled_state_rpt_q;
...
SMGR_STATIC void sns_rh_disable_sensor_event(sns_rh_sensor_event_s* se_ptr)
{
  if ( se_ptr->sensor_event == SNS_SMGR_SENSOR_EVENT_ID_MOTION_DETECT_1_V02 )
  {
         ...
         (sns_q_cnt(&se_ptr->enabled_state_rpt_q) == 0) )
    {
      ...
      sns_smgr_signal_me(SNS_SMGR_DISABLE_MD_SIG);
    }
  }
}
...
void sns_rh_process_md_int_enabled_sig(void)
{
  ...
  if ( se_ptr != NULL )
  {
    ...
    se_ptr->last_event_ts = sns_em_get_timestamp();
    se_ptr->enabled = false;
    sns_rh_generate_sensor_event_ind(se_ptr);

    SNS_SMGR_PRINTF1(HIGH, "md_int_enabled_sig - #en_rpt=%u", 
                     sns_q_cnt(&se_ptr->enabled_state_rpt_q));
...
void sns_rh_process_md_int_disabled_sig(void)
{
  ...
  if ( se_ptr != NULL )
  {
    ...
    SNS_SMGR_PRINTF1(HIGH, "md_int_disabled_sig - #dis_rpt=%u", 
                     sns_q_cnt(&se_ptr->disabled_state_rpt_q));
    sns_rh_update_sensor_event(se_ptr->ddf_sensor_ptr);
  }
}
...
void sns_rh_process_md_int_sig(void)
{
  ...
  if ( se_ptr != NULL )
  {
    ...
    else
    {
      ...
      SNS_SMGR_PRINTF1(HIGH, "md_int_sig - #occ_rpt=%u", 
                       sns_q_cnt(&se_ptr->occurred_state_rpt_q));
  ...
  sns_rh_mr_req_q_item_s* msg_ptr)
{
  ...
  ind_ptr->wakeup_rate   = ddf_sensor_ptr->current_wakeup_rate_hz;
  ind_ptr->timestamp     = cs_ptr->sensor_status.last_status_ts;

  SNS_SMGR_PRINTF3(
    HIGH, "sensor_status_ind - sensor=%u #client=%d ts=%u", 
...
void sns_rh_update_sensor_event(sns_smgr_ddf_sensor_s* ddf_sensor_ptr)
{
  ...
  if ( se_ptr != NULL && se_ptr->ddf_sensor_ptr == ddf_sensor_ptr && !se_ptr->enabled )
  {
    ...
    SMGR_FOR_EACH_ASSOC_ITEM( se_ptr->ddf_sensor_ptr, item_ptr, item_ptr_ptr )
    {
      ...
      else if ( item_ptr->parent_report_ptr->sensor_event_q == NULL )
      {
    ...
    if ( ((occ_rpts > 0) || (dis_rpts > 0)) && !occurred_streams )
    {
      if ( !independent_streams )
      {
        ...
        sns_smgr_signal_me(SNS_SMGR_ENABLE_MD_SIG);
      }
      else
      {
