    ...
    case SNS_SMGR_BUFFERING_IND_V01 :
      {
        ...
        if (err == QMI_NO_ERR)
        {
          #if 0
          uint32_t index;
          qurt_printf("\n SMGR Report IND: ReportID: %d, Indices_len: %d, Samples_len: %d", ind_msg.ReportId,ind_msg.Indices_len, ind_msg.Samples_len);

          for (index = 0; index < ind_msg.Indices_len; index++)
          {
            printf ("\n SMGR Report IND: Item: %d DataType: %d SamplingRate: %f\n", ind_msg.Indices[index].SensorId,
                     ind_msg.Indices[index].DataType, FX_FIXTOFLT_Q16(ind_msg.Indices[index].SamplingRate));
