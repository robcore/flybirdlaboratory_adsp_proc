...
$Header: //components/rel/ssc.adsp/2.6.1/debug/src/sns_debug_dsps.c#3 $
...
typedef enum
{
  ...
  SNS_DIAG_DEBUG_OPTION_GENERIC_0,  /*Enable-disable Logging*/
  SNS_DIAG_DEBUG_OPTION_GENERIC_1,
  SNS_DIAG_DEBUG_OPTION_GENERIC_2,
...
void sns_diag_dsps_set_debug_options(sns_diag_set_debug_options_req_msg_v01* req)
{
  ...
  for (i = 0; i < req->options_len && i < SNS_DIAG_MAX_OPTIONS_V01; i++)
  {
    switch (req->options[i].option) /* These are NOT enumerated in the IDL API */
    {
      case SNS_DIAG_DEBUG_OPTION_UIMG_VOTE:
        SNS_PRINTF_STRING_ERROR_1(SNS_DBG_MOD_DSPS_DIAG, "DEBUG : Setting UIMG vote option to %d ",
                                  req->options[i].value);
        sns_pm_set_uimg_vote_override(req->options[i].value == 1);
        break;

      case SNS_DIAG_DEBUG_OPTION_DROP_EXT_IND:
        SNS_PRINTF_STRING_ERROR_1(SNS_DBG_MOD_DSPS_DIAG, "DEBUG : Setting Drop Ext Ind option to %d ",
                                  req->options[i].value);
        sns_usmr_set_disable_ext_ind(req->options[i].value == 1);
        break;
        
      case SNS_DIAG_DEBUG_OPTION_GENERIC_0:
        SNS_PRINTF_STRING_ERROR_1(SNS_DBG_MOD_DSPS_DIAG, "DEBUG : Setting Logging option to %d ",
                                  req->options[i].value);
...
void sns_debug_task (void *arg_ptr)
{
  ...
  qcsi_err = qmi_csi_register(sns_smr_get_svc_obj(SNS_DEBUG_SVC_ID_V01),
                              debug_connect_cb,
                              debug_disconnect_cb,
                              debug_handle_req_cb,
                              NULL,
  ...
  qcsi_err = qmi_csi_register(sns_smr_get_svc_obj(SNS_DIAG_DSPS_SVC_ID_V01),
                              debug_connect_cb,
                              debug_disconnect_cb,
                              diag_handle_req_cb,
                              NULL,
...
sns_err_code_e sns_debug_init(void)
{
  ...
  sns_pm_err_code_e pm_err = sns_pm_client_init(&sns_debug_pm_client_handle, NULL, "DEBUG");
