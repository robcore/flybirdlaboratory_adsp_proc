    ...
    void *ind_buf, unsigned int ind_buf_len, void *ind_cb_data )
{
  ...
  SNS_OS_MEMZERO(&smr_ind_header, sizeof(sns_smr_header_s));
    ...
    qmi_client_error_type transp_err )
{
  ...
  SNS_OS_MEMZERO(&smr_resp_header, sizeof(sns_smr_header_s));
    ...
    void *service_cookie )
{
  ...
  SNS_OS_MEMZERO(&smr_req_header, sizeof(sns_smr_header_s));
                       ...
                       uint8_t algo_inst_id )
{
  ...
  if( NULL == algo_conn_hndl->service_handles[ svc_index ] )
  {
    ...
    if(qmi_err != QMI_NO_ERR) {
       SNS_PRINTF_STRING_ERROR_1(SNS_SAM_DBG_MOD,
                                "Error in qmi_client_init %d", qmi_err );
       return NULL;
    }
  }
  else
