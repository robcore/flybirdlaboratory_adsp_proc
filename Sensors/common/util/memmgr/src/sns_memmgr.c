...
/*============================================================================
  ...
  $Header: //components/rel/ssc.adsp/2.6.1/common/util/memmgr/src/sns_memmgr.c#1 $
...
QURT_ELITE_HEAP_ID sns_heap_id = 0;

#if defined(QDSP6)
/* State of the heaps. */
mem_heap_type     sns_heap_state;
mem_heap_totals_type sns_heap_total;
#endif
...
sns_err_code_e sns_heap_init(void)
{
   ...
   sns_sensors_heap_lowmem_cb.num_clients = 0;
   sns_default_heap_lowmem_cb.num_clients = 0;
   for( i = 0; i < SNS_MAX_NUM_MEMMGR_CLIENTS; i++ )
   {
     sns_sensors_heap_lowmem_cb.cb_func[i] = NULL;
...
sns_memmgr_lowmem_cb_register( sns_memmgr_lowmem_cb_t cb, sns_heap_type heapType )
{
  ...
  if( SNS_DEFAULT_HEAP_TYPE == heapType )
  {
  ...
  else if( SNS_SENSORS_HEAP_TYPE == heapType )
  {
    if( sns_sensors_heap_lowmem_cb.num_clients < SNS_MAX_NUM_MEMMGR_CLIENTS )
    {
      sns_sensors_heap_lowmem_cb.cb_func[sns_sensors_heap_lowmem_cb.num_clients] = cb;
      sns_sensors_heap_lowmem_cb.num_clients++;
      err = SNS_SUCCESS;
    }
  }
...
{
  mem_heap_get_totals(&sns_heap_state, &sns_heap_total);
  return (sns_heap_total.used_bytes + sns_heap_total.wasted_bytes + sns_heap_total.header_bytes);
}
#endif
