...
{
  //using accelerometer values as default
  ...
  if( SNS_SMGR_SUID_ACCEL_1_V01 == *sensorUID )
  {
         ...
         i++ )
    {
      if( attr_array[i].supported )
      {
        ...
        {
          thresholdPersistData->power = FX_CONV_Q16(attr_array[i].attrValue, 0) / 1000;
        }
        else if( SAM_ALGO_ATTR_MAX_SAMPLE == attr_array[i].attribute )
        {
          thresholdPersistData->accelMaxSampleRate = attr_array[i].attrValue;
        }
      }
    }
    thresholdPersistData->sensorSUID = sensorUID;
    depMetFunc( algo ); //we will have the assumption for now that accel is always supported
  }
}
  ...
  sns_sam_algo_attr attributes[] )
{
       ...
       i++ )
  {
    ...
    {
    case SAM_ALGO_ATTR_POWER: //accel
      attributes[i].attrValue = thresholdPersistData->power;
    ...
    case SAM_ALGO_ATTR_MAX_SAMPLE:
      attributes[i].attrValue = thresholdPersistData->accelMaxSampleRate;
      attributes[i].supported = true;
    break;
  ...
  sns_sam_ind_msg const *indMsg, sns_sam_input_q *inputData )
{
    ...
    SAM_EMAX != err; err = sns_sam_get_single_buffered_sample( bufInd, &bufSample, &buf_samples_cnt) )
  {
    if( SAM_ENONE == err )
    {
      ...
      cbFunc->sns_sam_algo_add_input( cbFunc->algoInst, (intptr_t)&input, bufSample.current_sample.timestamp );
    }
  }
  return SAM_ENONE;
}
