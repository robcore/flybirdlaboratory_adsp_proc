...
threshold_update_results( sns_sam_algo_callback const *cbFunc,
  threshold_state_s *state, const threshold_input_s *input, sns_sam_timestamp timestamp )
{
  ...
  sns_sam_algo_persist *persistData, uint32_t *processedCnt )
{
  ...
  for( *processedCnt = 0,
       algoInputBuf = (sns_sam_algo_input *)sns_q_check( &inputData->inputData );
       NULL != algoInputBuf;
       algoInputBuf = (sns_sam_algo_input *)sns_q_next( &inputData->inputData, &algoInputBuf->qLink ) )
  {
    input = (threshold_input_s*)algoInputBuf->data;
    cbFunc->sns_sam_algo_qdss_algo_update( cbFunc->algoInst,
      SNS_SAM_ALGO_PROCESS_INPUT_ENTER, 0 );

    /* F irst time running the algo, always report */
    if( 0 == state->last_timestamp )
    {
      threshold_update_results( cbFunc, state, input, algoInputBuf->timestamp );
      return SAM_ENONE;
    }
    ...
    for( i = 0; i < num_axis; i++ )
    {
      ...
      if( threshold_met )
      {
        ...
        {
          state->ts_start_period[i] = algoInputBuf->timestamp;
        }

        if( algoInputBuf->timestamp >= state->ts_start_period[i] )
        {
          if( algoInputBuf->timestamp - state->ts_start_period[i] >=
        ...
        else
        {
          if( ((UINT32_MAX - state->ts_start_period[i]) +
                algoInputBuf->timestamp) >= state->config.report_period )
          {
  ...
  sns_sam_algo_callback const *cbFunc, sns_sam_algo_output *outputData )
{
  ...
  {
    SNS_OS_MEMSCPY( output->sample, sizeof(output->sample), state->last_report, sizeof(output->sample) );
