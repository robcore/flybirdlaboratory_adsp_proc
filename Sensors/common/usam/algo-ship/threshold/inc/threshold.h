...
typedef struct {
  //right now set to accel value
  uint64_t power; /* Consumption for the accel sensor as reported by SMGR */
  q16_t maxSampleRate; /* Maximum sampling rate of the gyroscope */
  sns_sam_sensor_uid const *sensorSUID; /* SUID saved in dep_sensors_met() */
  // Max sample rate supported by dependent sensors
  uint32_t accelMaxSampleRate;
} threshold_persist_s;
/*============================================================================
