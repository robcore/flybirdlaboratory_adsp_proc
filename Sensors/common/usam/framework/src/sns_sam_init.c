...
sns_sam_smgri_event_query( void )
{
  ...
  errQMI = qmi_idl_get_message_c_struct_len(
    ...
    QMI_IDL_REQUEST, SNS_SMGR_SENSOR_EVENTS_QUERY_REQ_V02,
    &reqMsg.msg.bufSize );
  if( QMI_IDL_LIB_NO_ERR == errQMI )
  {
  ...
  {
    SNS_PRINTF_STRING_ERROR_1( samModule,
      "Unable to to lookupSMGRI event query req size %i", errQMI );
    return SAM_EFAILED;
  }
...
sns_sam_smgr_req_all( void )
{
  ...
  errQMI = qmi_idl_get_message_c_struct_len(
    ...
    QMI_IDL_REQUEST, SNS_SMGR_ALL_SENSOR_INFO_REQ_V01,
    &reqMsg.msg.bufSize );
  if( QMI_IDL_LIB_NO_ERR == errQMI )
  {
