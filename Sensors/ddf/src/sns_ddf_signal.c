...
/*==============================================================================
  ...
  $Header: //components/rel/ssc.adsp/2.6.1/ddf/src/sns_ddf_signal.c#2 $
    ...
    sns_ddf_signal_irq_trigger_e  trigger)
{
    ...
    {
      if ( sns_ddf_sig_tb.sig_info[sig_idx].is_uimg_refac == FALSE )
      {
      ...
      else
      {
        ...
        {
          // Future TBD:
          // if err == -1, i.e. uGPIOInt Reg failed,
          // should try old GPIOInt_RegisterIsr
        }
      }
...
sns_ddf_status_e sns_ddf_signal_deregister (uint32_t gpio_num)
{
    ...
    if (sns_ddf_sig_tb.sig_info[sig_idx].is_registered)
    {
      ...
      {
        err = uGPIOInt_DeregisterInterrupt(
           gpio_num);
      }
      if (err != -1)
      {
