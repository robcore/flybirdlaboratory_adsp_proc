...
)
{
  ...
  *min_slave_frequency_hz = cfg->max_slave_freq_hz;
  *max_slave_frequency_hz = cfg->min_slave_freq_hz;
#endif
  return SNS_DDF_SUCCESS;
}
...
)
{
  ...
  uint8_t read_buffer [bytes+1]; //TODO: buffers on the heap if bytes>256
  uint8_t reg_addr8 = (uint8_t)reg_addr;

  SNS_OS_MEMSET(write_buffer, 0, sizeof(uint8_t) * (bytes+1));
  SNS_OS_MEMSET(read_buffer,  0, sizeof(uint8_t) * (bytes+1));

  reg_addr8       |= (1 << 7);   //set RW bit = 1 for read
  write_buffer[0]  = reg_addr8;  //NOTE: currently 8bit reg addr only
