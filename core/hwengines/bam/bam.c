...
===============================================================================
 ...
 $Header: //components/rel/core.adsp/2.6.1/hwengines/bam/bam.c#1 $
    ...
    while(pipe->acked_offset != endoffset)
    {
        if( pipe->pipe_cfg.options & BAM_O_ACK_LAST_ONLY )
        {
            ...
            */
            ...
            iovec = ((bam_iovec_type *) (pipe->pipe_cfg.desc_base_va + endoffset));
...
{
    if ((tgt_cfg->options & BAM_TGT_CFG_NO_INIT) != 0 )
        return FALSE;
...
bam_handle bam_pipe_init(bam_handle bamhandle, uint32 pipe_num, bam_pipe_config_type *pipe_cfg, bam_callback_type *pipe_cb)
{
    ...
    /* Map the descriptor VA if it hasn't been mapped */
...
BAM_API_NON_PAGED bam_status_type bam_pipe_transfer(bam_handle pipehandle, uint32 buf_pa, uint16 buf_size, uint16 xfer_opts, void *user_data)
{
    ...
    if(pipe==NULL)
    {
        bam_osal_debug(MSG_ERR,"bam_pipe_transfer:Invalid BAM pipe handle  handle:0X%x\n", pipe);
        return BAM_FAILED;
    }
    ...
    if( pipe->pipe_cfg.mode ==  BAM_MODE_BAM2BAM)
    {
        bam_osal_debug(MSG_ERR, "bam_pipe_transfer:Bam is in BAM2BAM mode, cannot call bam_pipe_transfer\n");
        return BAM_FAILED;
    }
    ...
    {
       bam_osal_debug(MSG_ERR, "bam_pipe_transfer:CMD descriptor cannot have EOT\n"); 
       goto exit;
    }
              ...
              !(xfer_opts & (BAM_IOVEC_FLAG_CMD|BAM_IOVEC_FLAG_EOT)))
    {
       ...
       bam_osal_debug(MSG_ERR, "bam_pipe_transfer:NWD must be paired with EOT or CMD descriptor\n"); 
       goto exit;
    }
    if(xfer_opts & BAM_IOVEC_FLAG_CMD && xfer_opts & BAM_IOVEC_FLAG_IMM_CMD )
    {
       bam_osal_debug(MSG_ERR, "bam_pipe_transfer:CMD descriptor cannot have immediate flag set\n"); 
       goto exit;
    }
...
bam_status_type bam_pipe_bulktransfer(bam_handle pipehandle, uint32 buf_pa, uint32 buf_size, uint16 xfer_opts, void *user_data)
{
    ...
    if( pipe->pipe_cfg.mode ==  BAM_MODE_BAM2BAM)
    {
        bam_osal_debug(MSG_ERR, "bam_pipe_bulktransfer:Bam is in BAM2BAM mode, cannot call bam_pipe_bulktransfer\n");
        return BAM_FAILED;
    }
    if(xfer_opts & BAM_IOVEC_FLAG_CMD)
    {
       bam_osal_debug(MSG_ERR, "bam_pipe_bulktransfer:CMD descriptor is not valid for bulk transfer\n"); 
       return BAM_FAILED;
    }
    ...
    {
        bam_osal_debug(MSG_ERR, "bam_pipe_bulktransfer:Not enough descriptor, free_count:%d, num_desc:%d\n", free_count, num_descs);
        goto exit;
    }
    while(buf_size!=0)
    {
...
bam_status_type bam_pipe_trigger_zlt_event(bam_handle pipehandle)
{
    ...
    if ( (iovec->flags & BAM_IOVEC_FLAG_EOT) == 0)
    {
