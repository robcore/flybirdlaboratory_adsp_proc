...
$Header: //components/rel/core.adsp/2.6.1/mproc/smem/badger/smem_target.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/18/14   bm      Cleanup to make this file support only badger
05/30/14   bm      Use new IMEM address for 8994
...
/*-------------------------------------------------------------
   Address in System IMEM where SMEM physical base address and 
   size will be saved by SBL.
 ------------------------------------------------------------- */
/* 0xFE800000(SYSTEM_IMEM_BASE) + 0x3000 – 0x10(sizeof(smem_info_imem_type)) */
#ifdef SMEM_TARG_8994
#define SMEM_TARG_INFO_ADDR           0xFE805FF0
#else
#define SMEM_TARG_INFO_ADDR           0xFE802FF0
#endif //SMEM_TARG_8994
