...
$Header: //components/rel/core.adsp/2.6.1/mproc/ipc_router/src/ipc_router_protocol.c#1 $
...
)
{
      ...
      header->dest_addr.port_id > 0xffff))
  {
    ...
    w_header.rtr.flags |= IPC_ROUTER_V3_CTRL_FLAG_OPT_HDR;
...
)
{
  ...
  len = hdr->msg_size;
  len += PAD_LEN(len) + sizeof(ipc_router_v3_header_type);
  *packet_size = len;
