...
$Header: //components/rel/core.adsp/2.6.1/mproc/ipc_router/src/ipc_router_core.c#1 $
...
)
{
  ...
  pkt = ipc_router_packet_new_headroom(IPC_ROUTER_MAX_HEADER_SIZE);
  if(!pkt)
  {
    return NULL;
  }
...
/*===========================================================================
FUNCTION      ipc_router_core_build_header
...
SIDE EFFECTS  None
===========================================================================*/
static int ipc_router_core_build_header
(
 ipc_router_xport_type *xport,
 ipc_router_header_type *header,
 ipc_router_packet_type **msg
 )
{
  ...
  return xport->protocol_ops->inject(header, msg);
}
...
{
  int rc = ipc_router_core_build_header(xport, header, msg);
...
static void ipc_router_xport_down(ipc_router_xport_type *xport)
{
  ...
  while(NULL != (client_list = LIST_HEAD(results.clients)))
  {
    ...
    ipc_router_unregister_client(&client_list->addr);
...
)
{
  ...
  if(rc == IPC_ROUTER_STATUS_SUCCESS)
  {
    ...
    {
      rc = ipc_router_bcast_cntl_msg(&cntl_msg, IPC_ROUTER_XPORT_NET_ALL,
                                      client->priority, client->scope);
    }
  }
 ...
 )
{
  ...
  if(rc != IPC_ROUTER_STATUS_SUCCESS)
  {
    ...
    IPC_ROUTER_OS_MESSAGE(IPC_ROUTER_MSG_ERR,
        ...
        cntl_msg->command, tx_header.dest_addr.processor_id, 0);
  }
  return rc;
}
...
)
{
  ...
  cr.arguments.svc_arg.compare = NULL;
  cr.arguments.svc_arg.compare_data = NULL;
...
)
{
  ...
  if(!msg)
  {
    msg = ipc_router_core_build_body((unsigned char *)buf, len);
    if(!msg)
    {
  ...
  ret = ipc_router_send_to_client(&header, &msg, client, dest_client);
...
)
{
  ...
  uint32 num_entries = 1, num_servers = 0;
  int rc;

  if(!service || !addr)
    return IPC_ROUTER_STATUS_INVALID_PARAM;

  rc = ipc_router_core_find_all_servers(client, service, &server,
                                        &num_entries, &num_servers,
