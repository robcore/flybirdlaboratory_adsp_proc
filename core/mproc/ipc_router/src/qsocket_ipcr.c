...
/*===========================================================================
  FUNCTION  sock_compare_cb
===========================================================================*/
/*!
@brief

  Callback which will be called by IPC Router core during compare ops

@note
  None
*/
/*=========================================================================*/
static boolean sock_compare_cb(uint32 inst1, uint32 inst2, void *compare_data)
{
  uint32 mask = (uint32)(uintptr_t)compare_data;

  if((inst2 & mask) == (inst1 & mask))
  {
    return TRUE;
  }
  return FALSE;
...
*/
/*=========================================================================*/
static int ipcr_mcast(int fd, ipcr_addr_t *name, void *buf, size_t len)
{
  ipcr_sock_t *sock = get_sock(fd);
  int rc = -1;
              ...
              struct qsockaddr *_addr, qsocklen_t addrlen)
{
  ...
  {
    return ipcr_mcast(fd, addr, buf, len);
  }
...
int qpoll(struct qpollfd *pfd, qnfds_t num, int timeout_ms)
{
  ...
  while(ret == 0)
  {
    ...
    for(i = 0; i < num; i++)
    {
      ipcr_sock_t *sock = get_sock(pfd[i].fd);
      pfd[i].revents = 0;
      if(sock == NULL)
      {
                    ...
                    unsigned int flags)
{
  ...
  rc = ipc_router_core_find_all_servers(NULL, &service, servers, &num_entries,
                            &num_servers, sock_compare_cb, (void *)(uintptr_t)instance_mask);
