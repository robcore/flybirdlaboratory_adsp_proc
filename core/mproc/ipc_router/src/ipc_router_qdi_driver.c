...
$Header: //components/rel/core.adsp/2.6.1/mproc/ipc_router/src/ipc_router_qdi_driver.c#1 $
...
do {  \
  ...
  { \
    timer_def_osal(&((waiter).timer), NULL, TIMER_NATIVE_OS_SIGNAL_TYPE,  \
                    &((waiter).signal), TIMEOUT_SIG); \
...
SIDE EFFECTS  None
===========================================================================*/
static void notify_waiters(int fd, int event, ipc_router_address_type *addr)
{
...
SIDE EFFECTS  None
===========================================================================*/
static int enqueue_data_packet
(
  ipc_router_user_ep_t *user,
  ipc_router_address_type *src_addr,
  ipc_router_packet_type *pkt,
    ...
    case IPC_ROUTER_EVENT_CLOSED:
      {
        ...
        notify_waiters(handle->qdi_handle, sig_mask, NULL);
      }
      break;
    case IPC_ROUTER_EVENT_RESUME_TX:
      notify_waiters(handle->qdi_handle, IPC_QDI_POLLOUT, &info->client);
      break;
    ...
    boolean confirm_rx)
{
  ...
  if(enqueue_data_packet(clnt, src_addr, msg, len, confirm_rx) != 0)
  {
    /* TODO Log error */
    ipc_router_packet_free(&msg);
  }
 ...
 * - out_buf and in_max_len is allowed to be NULL/0 only if 
     IPC_ROUTER_QDI_RDFLAG_MSGPEEK is set. Otherwise they are required.
...
)
{
  ...
  if(!handle || !out_len || (in_max_len > 0 && !out_buf) ||
    (0 == (flags & IPC_ROUTER_QDI_RDFLAG_MSGPEEK) && (!out_buf || !in_max_len)))
  {
    return IPC_ROUTER_QDI_PARAM_ERR;
  }

  /* XXX User must cleanup (call CLOSE handle invoke on the disconnected user handle).
   * We probably need an invocation to allow them to query. But since the
   * only user is qsocket -- which has only 1 user per fd we dont have a 
   * notify use case -- change if assumption changes in future */
  if(disconnected_users(handle) > 0)
  {
  ...
  if(!(flags & IPC_ROUTER_QDI_RDFLAG_DONTWAIT))
  {
    ...
    while(1)
    {
      ...
      {
        /* Invalid operation to wait if there are no active receivers
         * on this fd. */
  ...
  /* If there is nothing or we just want the len, 
   * just do that return success without removing
   * the packet from head */
  if(!buf || (flags & IPC_ROUTER_QDI_RDFLAG_MSGPEEK))
  {
    *out_len = buf ? buf->len : 0;
    rc = 0;
    goto bail;
  }
...
)
{
  ...
  {
    if(handle->conn.status != CONN_STATUS_CONNECTED)
    {
...
)
{
  ...
  if(options->type_pref_valid)
  {
    ...
    handle->conn.val.name.type = 0xffffffff;
  }
...
)
{
  ...
  while(ret == 0)
  {
    ...
    for(i = 0; i < len; i++)
    {
      ...
      h = i == 0 ? handle : 
        (ipc_router_qdi_handle_t *)qurt_qdi_objref_get_from_handle(QDI_HANDLE_LOCAL_CLIENT, pfd[i].fd);
      ...
      {
        /* TODO: This check must be done on _all_ clients to make 
         * this a uniform. The IPC_QDI_POLLOUT will be set only
         * if we can write to all users */
        if(LIST_HEAD(h->clients)->conn.status == CONN_STATUS_CONNECTED)
        {
          /* TODO */
        }
        else
...
)
{
  ...
  while(LIST_CNT(handle->clients) > 0)
  {
    ...
    qurt_signal_wait_any(&waiter.signal, IPC_QDI_POLLIN);
    qurt_signal_clear(&waiter.signal, IPC_QDI_POLLIN);
    ipc_router_os_sem_lock(&handle->clients_lock);
  }
  ipc_router_os_sem_unlock(&handle->clients_lock);
...
)
{
  ...
  switch(method)
  {
    ...
    case IPC_ROUTER_QDI_SEND:
      client = ipc_router_qdi_get_obj(handle, a1.num);
      ret = ipc_router_qdi_send_invocation(client_handle, client, a2.ptr, a3.ptr, (uint32)a4.num, a5.num);
      ipc_router_qdi_put_obj(client);
      break;
