...
$Header: //components/rel/core.adsp/2.6.1/mproc/ipc_router/src/ipc_router_database.c#1 $ $DateTime: 2014/10/16 12:45:40 $ $Author: pwbldsvc $
...
{
  IPC_ROUTER_OS_MESSAGE(IPC_ROUTER_MSG_ERR, "ipc_router_database.c:%d malloc() error", line, 0, 0);
}
...
)
{
  ...
  {
    while(NULL != (server_to_delete = LIST_HEAD(g_server_tbl[i])))
    {
 ...
 )
{
  ...
  switch(cr->crit)
  {
    ...
    case IPC_ROUTER_CRITERIA_SERVICE:
      ...
      i = LIST_HEAD(g_server_tbl[find_server_slot(cr->arguments.svc_arg.service->type)]);
      while(i)
      {
