...
$Header: //components/rel/core.adsp/2.6.1/mproc/ipc_router/src/ipc_router_xal_smdl_common.c#2 $
$DateTime: 2014/10/21 14:27:51 $
$Author: pwbldsvc $
...
void ipc_router_xal_smdl_handle_rx(ipc_router_xal_smdl_port_type *port)
{
  ...
  do {
    ...
    while((bytes = smdl_rx_peek(port->handle)) > 0)
    {
      ...
      {
        packet = NULL;
        if(ipc_router_packet_expand(&packet, bytes) < bytes)
        {
          /* TODO: Log Error */
          break;
        }
