...
static uint16_t qmi_csi_cb_err_translate(qmi_csi_cb_error err)
{
  ...
  return qmi_csi_cb_err_array[err];
}
 ...
 )
{
  ...
  uint32_t txn_handle = 0, c_struct_len;
  uint8_t cntl_flag;
      ...
      )) != QMI_CSI_CB_NO_ERR)
  {
    if(c_struct)
      FREE(c_struct);
    rc = qmi_csi_cb_err_translate((qmi_csi_cb_error)rc);
    goto rx_cb_free_txn_bail;
  }
 ...
 )
{
  ...
  qmi_client_handle client_handle = NULL;
  void *service_cookie = NULL, *connection_handle = NULL;
