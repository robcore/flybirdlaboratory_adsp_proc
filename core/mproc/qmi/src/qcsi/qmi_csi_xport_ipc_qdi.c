/******************************************************************************
  @file    qmi_csi_xport_ipc_qdi.c
  @brief   The QMI Common Service Interface (CSI) transport adapter for 
           the IPC Router QDI Interface

  DESCRIPTION
  QMI Common Service Interface transport adapter module for QDI Interface
  of IPC Router.
...
static void flush_tx_q(struct qmi_csi_xport_handle *xp, ipc_router_address_type *addr)
{
  ...
  while(NULL != (buf = LIST_HEAD(dest->bufs)))
  {
    if(IPC_ROUTER_STATUS_SUCCESS !=
        ipc_router_qdi_send(xp->handle, addr, buf->msg, buf->len))
    {
 ...
 )
{
  ...
  if(xp)
  {
    ...
    if(ipc_router_qdi_open(&xp->handle, 0, qmi_csi_rx_cb, xp, qmi_csi_event_cb,
          xp, &ipc_options) != IPC_ROUTER_STATUS_SUCCESS)
    {
      LOCK_DEINIT(&xp->tx_q_lock);
      LOCK_DEINIT(&xp->rx_q_lock);
      FREE(xp);
