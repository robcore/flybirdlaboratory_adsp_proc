...
================================================================================
$Header: //components/rel/core.adsp/2.6.1/power/sleep/src/os/qurt/sleep_os.c#1 $
...
/**
 *  @brief gSleepSoftDeadline
 *
 * The soft deadline (hint of next wakeup) that is currently
 * being used by the idle code, in absolute ticks.  It only
 * makes sense to query this value during the idle context
 * (i.e., from an LPR).
 */
static uint64 gSleepSoftDeadline;
...
/**
 * @brief Flag to indicate if processor is running in sleep context.
 */
static boolean s_processor_in_stm = FALSE;
                                    ...
                                    uint32 *latency_budget )
{
  ...
  *hard_duration = sleep_os_get_hard_wakeup_time();
   ...
   * up from soon anyway. */
  CORE_VERIFY( NPA_QUERY_SUCCESS == npa_query( gSleepWakeupNode,
                                               SLEEP_QUERY_WAKEUP_TIME,
                                               &qres ) );
  *soft_duration = SCLK_TO_TICKS64(qres.data.value);
  ...
  qres64 = SCLK_TO_TICKS64(qres.data.value);
  *latency_budget = safe_unsigned_truncate(qres64);
...
uint32 sleep_perform_lpm(void)
{
  ...
  uint64              hard_duration, soft_duration;
  ...
  if ( (sleep_perform_lpm != sleep_idle_entry_ptr) ||  
       (qurt_power_shutdown_prepare() != QURT_EOK) )
  {
    /* Unmask global interrupt which was masked in
     * call to sleep_os_wait_for_idle() */
    qurt_power_shutdown_fail_exit();
  ...
  s_processor_in_stm = TRUE;

  sleep_target_disable_dog();

  /* Disable differed timers to get more idle duration
   * ADSP requires that we defer timers before we get the sleep duration due
   * to the different type of timers used.
   */
