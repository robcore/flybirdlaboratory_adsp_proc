...
================================================================================
$Header: //components/rel/core.adsp/2.6.1/power/sleep/src/lpr/sleep_target.c#1 $
 ...
 */
#define SLEEP_ENTRY_TICKS 32816
...
static void sleep_rpm_wakeup_handler(uint32 param);
                                        ...
                                        unsigned int  data_size )
{
  ...
  npa_client_handle sleep_cpu_vdd_lpr_client_handle = NULL;  
  ...
  /* Creating NPA clients for registering internal LPRs */
  CORE_VERIFY_PTR( sleep_cpu_vdd_lpr_client_handle = 
                   npa_create_sync_client( SLEEP_LPR_NODE_NAME,
                                           "core/power/sleep/cpu_vdd",
                                           NPA_CLIENT_REQUIRED ) );
  ...
  /* Registering different LPRs and enabling their modes  */
  sleep_define_lpr_str( "cpu_vdd", sleep_cpu_vdd_lpr_client_handle );
  npa_issue_required_request(sleep_cpu_vdd_lpr_client_handle, SLEEP_ALL_LPRMS);
  
  sleep_define_lpr_str( "rpm", sleep_rpm_lpr_client_handle );
  npa_issue_required_request( sleep_rpm_lpr_client_handle, SLEEP_ALL_LPRMS );
}
...
sleep_target_config * sleep_target_init(void)
{
  ...
  CORE_VERIFY( DalInterruptController_RegisterISR( 
                hDALIntCtl,
                (uint32)14,
                (DALIRQ)sleep_rpm_wakeup_handler,
                (const DALIRQCtx) 0,
...
static uint64 sleep_target_enable_IDLT( uint64 wakeup_tick )
{
  ...
  HWIO_OUT( LPASS_QDSP6SS_IDLT_MATCH_VAL, (uint32)idlt_sclk );
  HWIO_OUT( LPASS_QDSP6SS_IDLT_ENABLE, 0x1 );
...
static void sleep_target_disable_IDLT( void )
{
  HWIO_OUT( LPASS_QDSP6SS_IDLT_ENABLE, 0x0 );
}
