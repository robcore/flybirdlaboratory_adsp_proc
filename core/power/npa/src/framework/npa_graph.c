...
$Header: //components/rel/core.adsp/2.6.1/power/npa/src/framework/npa_graph.c#1 $
============================================================================*/
...
void npa_destroy_resource( npa_resource *resource )
{
  ...
  {
    event = resource->events.creation_event;
  }
  else
  {
    event = 
      resource->events.header ? resource->events.header->reg_event : NULL;
  }
                             ...
                             npa_event_callback  *callback )
{
  ...
  ULOG_RT_PRINTF_2( npa.log, 
                    "npa_create_node (handle: 0x%08x) (node: \"%s\")",
        ...
        resource_def = &definition->resources[++i] )
  {
    ...
    ULOG_RT_PRINTF_4( npa.log, 
                      "\tcreated resource (resource: \"%s\") (handle: 0x%08x) "
  ...
  ULOG_RT_PRINTF_2( npa.log, 
                    "\tend npa_create_node (handle: 0x%08x) (node: \"%s\")",
                    definition, definition->name );
}
                              ...
                              npa_resource_state   default_state[] )
{
  ...
  if ( NULL != creation_event )
  {
    ...
    if ( resource->events.creation_event == creation_event )
    {
      resource->events.creation_event = creation_event->next;
    }
