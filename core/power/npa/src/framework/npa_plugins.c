...
$Header: //components/rel/core.adsp/2.6.1/power/npa/src/framework/npa_plugins.c#1 $
============================================================================*/
                                            ...
                                            npa_client_handle  client )
{
       ...
       npa_resource_has_attribute( resource, NPA_RESOURCE_VECTOR_STATE ) )
  {
    ...
    core_memcpy( resource->state_vector, 
                 vector_size,
    ...
    if ( client->type == NPA_CLIENT_SUPPRESSIBLE_VECTOR )
    {
      ...
      {
        core_memcpy( resource->suppressible_state_vector, 
                     vector_size,
                     NPA_PENDING_REQUEST(client).pointer.vector,
                     vector_size );
      }
    ...
    else 
    {
      ...
      {
        core_memcpy( resource->required_state_vector, 
                     vector_size,
                     NPA_PENDING_REQUEST(client).pointer.vector,
                     vector_size );
      }
