...
$Header: //components/rel/core.adsp/2.6.1/power/npa/src/framework/npa_internal.h#1 $
============================================================================*/
...
/* Request attributes set and used internal to the framework */
#define NPA_REQUEST_DROP_VOTE NPA_REQUEST_RESERVED_ATTR1
...
typedef struct npa_continuation
{
  ...
  union
  {
    ...
    {
      DALSYSEventHandle    enqueue_event; /* DAL Event Handle for enqueuing
                                           * async requests */
...
{
  npa_event             *reg_event;  /* Regular events list */
  npa_event             *sys_event;  /* System events list */
  
  /* Count of change/watermark/threshold/custom events */
  volatile unsigned int  reg_count;
...
typedef struct npa_resource_internal_data
{
  ...
  /* Time spent in the resource driver */
