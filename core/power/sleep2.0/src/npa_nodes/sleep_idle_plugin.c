...
================================================================================
$Header: //components/rel/core.adsp/2.6.1/power/sleep2.0/src/npa_nodes/sleep_idle_plugin.c#3 $
$DateTime: 2014/11/26 08:33:45 $
==============================================================================*/
#include "sleep_lpr.h"
...
{
  /* The reason we need this is to avoid a race condition when driver
   * triggers ATS and ATS finishes query before NPA framework updates the 
   * resource state. It will cause ATS to use the stale value causing incorrect
   * mode selection. Until NPA support synchronous POST_CHANGE events/callbacks
   * we need this mechanism. We will also need to add query function for the 
   * resource which will return this field as state of the resource. */
