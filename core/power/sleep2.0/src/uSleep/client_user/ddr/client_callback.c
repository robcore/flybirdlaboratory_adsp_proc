...
================================================================================
$Header: //components/rel/core.adsp/2.6.1/power/sleep2.0/src/uSleep/client_user/ddr/client_callback.c#1 $
...
static void uSleep_createClientThread(unsigned int processId)
{
  ...
  snprintf(threadName, sizeof(threadName), "USLEEPCBCLNT_%u", processId);
...
void uSleep_callbackHandler(void *parameter)
{
  ...
  while(1)
  {
    ...
    {
      CORE_VERIFY_PTR(workingList->callbackFcn);

      /* Notify clients we are switching modes - return status is ignored for
       * now */
      workingList->callbackFcn(state);
