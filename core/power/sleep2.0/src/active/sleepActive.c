...
=================================================================================
$Header: //components/rel/core.adsp/2.6.1/power/sleep2.0/src/active/sleepActive.c#1 $
...
/* Maintains last low power mode selected by Active Time Solver. */
static sleep_synth_lprm* sleepActiveLastSynthMode;

/*==============================================================================
                           STATIC FUNCTION DEFINITIONS
  =============================================================================*/
/**
 * @brief Unconfigure previously applied settings. Currently, this unconfig is
 *        straight forward but eventually we may need to consider both old
 *        and new idle entry type to unconfigure properly.
 */
void static sleepActive_unconfigPreviousSettings(void)
{
  /* Unconfigure previously configured cached synth mode */
  if ( (sleepActiveLastSynthMode != NULL) && 
       (synthLPRM_isCacheable( sleepActiveLastSynthMode )) )
  {
    synthLPRM_exit( sleepActiveLastSynthMode , solverInput.fLUT );
  }

  sleepActiveLastSynthMode = NULL;
}
...
static void sleepActive_TriggerSolver( void )
{
  ...
  if ( sleepActiveLastSynthMode == solver_output.selected_synthLPRM )
  {
    return;
  }

  /* Unconfig previously configured modes as we have a different mode now. */
  sleepActive_unconfigPreviousSettings();
  ...
  sleepActiveLastSynthMode = solver_output.selected_synthLPRM;
}
...
static boolean sleepActive_ProcessControlSignals( uint32 signals )
{
  ...
  if ( signals & SLEEP_SIGNAL_CONTROL )
  {
         ...
         idleMode == SLEEP_OS_IDLE_MODE_USLEEP )
    {
      ...
      /* Unconfigure previous settings as we have new idle entry type */
      sleepActive_unconfigPreviousSettings();
