...
$Header: //components/rel/core.adsp/2.6.1/power/adsppm/inc/adsppm.h#2 $
...
typedef enum
{
    ...
    Adsppm_State_CoreUSLEEPInit = 0x40000,
 ...
 *****************************************************************************/
/**
 * @enum AdsppmClientTypeType - ADSPPM client types
 */
typedef enum
{
...
{
    coreUtils_Q_LinkType  adsppmHandleQLink; //!< link to next element in list/queue
...
/**
 * @enum AdsppmMemPowerReqParamType - Memory power state request parameter structure
 */
typedef struct
{
...
/**
 * This function is called in API, to check core id is existing
 * in this platform based on adsppm config file
 */
Adsppm_Status ADSPPM_CheckCoreId(AdsppmCoreIdType core);

/**
 * This function is called in API, to check mem ID is existing
 * in this platform based on adsppm config file
 */
Adsppm_Status ADSPPM_CheckMemId(AdsppmMemIdType mem);

/**
 * This function is called in API, to check clock ID is existing
 * in this platform based on adsppm config file
 */
Adsppm_Status ADSPPM_CheckClockId(AdsppmClkIdType clock);

/**
 * This function is called in API, to check busport ID is
 * existing in this platform based on adsppm config file
 */
Adsppm_Status ADSPPM_CheckBusPort(AdsppmBusPortIdType busport);
