...
static void BwHandleInitCb(void *context, unsigned int event_type, void *data, unsigned int data_size)
{
    ...
    busdrvInfo.gTransactionHandle = npa_define_transaction("Adsppm_Bw", NPA_TRANSACTION_LAZY_LOCKING, 1, resources);
    ...
    if(NULL != busdrvInfo.gTransactionHandle)
    {
        ...
        if(NULL != mipsMasterSlavePairs)
        {
            for(i = 0; i < busdrvInfo.gNumMasterSlave ; i++)
            {
                ...
                busdrvInfo.gBwRequestHandle[i].busdrvRoute.masterPort = busdrvInfo.gExtRouteArray[i].masterPort.adsppmMaster;
                busdrvInfo.gBwRequestHandle[i].busdrvRoute.slavePort = busdrvInfo.gExtRouteArray[i].slavePort.adsppmSlave;
                ...
                {
                    busdrvInfo.gBwRequestHandle[i].handle =
                            npa_create_sync_client_ex(ICBARB_NODE_NAME,
                ...
                {
                    ADSPPM_LOG_PRINTF_2(ADSPPM_LOG_LEVEL_ERROR, "icbarb client create failed:M=%u, S=%u",
                            busdrvInfo.gBwRequestHandle[i].busdrvRoute.masterPort,
                            busdrvInfo.gBwRequestHandle[i].busdrvRoute.slavePort);
                    busdrvInfo.gBusInitSuccess = FALSE;
                    result = FALSE;
            ...
            if(TRUE == result)
            {
                snprintf(busdrvInfo.gMipsBwRequestHandle.clientName, MAX_ADSPPM_CLIENT_NAME, "MIPS request");
                busdrvInfo.gMipsBwRequestHandle.handle =
                        npa_create_sync_client_ex(ICBARB_NODE_NAME,
        ...
        for(i = 0; i < BUS_NUMBER_MIPS_ROUTES; i++) //The request always contains 2 entries
        {
            for(j = 0; j < busdrvInfo.gNumMipsRequest; j++)
            {
                        ...
                        (pRequest[i].busRoute.slavePort == busdrvInfo.pMipsRouteArray[j].slavePort))
                {
                    ...
                    ADSPPM_LOG_PRINTF_6(ADSPPM_LOG_LEVEL_INFO, "%s(%u) to %s(%u), Ib: %u, Ab: %u",
                            adsppm_getBusPortName(pRequest[i].busRoute.masterPort), pRequest[i].busRoute.masterPort,
                            adsppm_getBusPortName(pRequest[i].busRoute.slavePort), pRequest[i].busRoute.slavePort,
                            (uint32)bwRequest[j].arbData.type3.uIb, (uint32)bwRequest[j].arbData.type3.uAb);
                    ADSPPM_LOG_FUNC_PROFILE4("master: %u, slave: %u, Ib: %u, Ab: %u",
                            pRequest[i].busRoute.masterPort, pRequest[i].busRoute.slavePort,
                            (uint32)bwRequest[j].arbData.type3.uIb, (uint32)bwRequest[j].arbData.type3.uAb);
                    ADSPPM_QDSS_EVENT_4(ADSPPM_BW_REQ, pRequest[i].busRoute.masterPort, pRequest[i].busRoute.slavePort,
                            (uint32)bwRequest[j].arbData.type3.uIb, (uint32)bwRequest[j].arbData.type3.uAb);
                    busdrvInfo.pMipsBwValues[j].Ab = pRequest[i].bwValue.busBwAbIb.Ab;
                    busdrvInfo.pMipsBwValues[j].Ib = pRequest[i].bwValue.busBwAbIb.Ib;
                }
            }
        }
            ...
            for(j = 0; j < num; j++)
            {
                for(i = 0; i < busdrvInfo.gNumMasterSlave; i++)
                {
                            ...
                            (pRequest[j].busRoute.slavePort == busdrvInfo.gBwRequestHandle[i].busdrvRoute.slavePort))
                    {
                        ...
                        {
                            ADSPPM_LOG_PRINTF_4(ADSPPM_LOG_LEVEL_INFO, "%s(%u) to %s(%u)",
                                    adsppm_getBusPortName(pRequest[j].busRoute.masterPort), pRequest[j].busRoute.masterPort,
                                    adsppm_getBusPortName(pRequest[j].busRoute.slavePort), pRequest[j].busRoute.slavePort);
                            ADSPPM_LOG_FUNC_PROFILE2("Master: %u, Slave: %u",
                                    pRequest[j].busRoute.masterPort, pRequest[j].busRoute.slavePort);
                            ADSPPM_QDSS_EVENT_4(ADSPPM_BW_REQ, pRequest[j].busRoute.masterPort, pRequest[j].busRoute.slavePort,
                                    (uint32)pRequest[j].bwValue.busBwAbIb.Ib, (uint32)pRequest[j].bwValue.busBwAbIb.Ab);
                            if((0 == pRequest[j].bwValue.busBwAbIb.Ab) && (0 == pRequest[j].bwValue.busBwAbIb.Ib)) //0 == variable
                            {
                                ...
                                ADSPPM_LOG_FUNC_PROFILE0("release bus request, ab, ib is 0");
                            }
                            else
                            {
                                ...
                                ADSPPM_LOG_PRINTF_2(ADSPPM_LOG_LEVEL_INFO, "Ib: %u, Ab: %u",
                                        (uint32)bwRequest[0].arbData.type3.uIb, (uint32)bwRequest[0].arbData.type3.uAb);
                                ADSPPM_LOG_FUNC_PROFILE2("Ib: %u, Ab: %u",
                                        (uint32)bwRequest[0].arbData.type3.uIb, (uint32)bwRequest[0].arbData.type3.uAb);
                            }
        ...
        for(j = 0; j < num; j++)
        {
            for(i = 0; i < busdrvInfo.gNumMasterSlave; i++)
            {
                if((pRequest[j].busRoute.masterPort == busdrvInfo.gBwRequestHandle[i].busdrvRoute.masterPort) &&
                        (pRequest[j].busRoute.slavePort == busdrvInfo.gBwRequestHandle[i].busdrvRoute.slavePort))
                {
                    ADSPPM_LOG_PRINTF_4(ADSPPM_LOG_LEVEL_INFO, "%s(%u) to %s(%u) release bw",
                            adsppm_getBusPortName(pRequest[j].busRoute.masterPort), pRequest[j].busRoute.masterPort,
                            adsppm_getBusPortName(pRequest[j].busRoute.slavePort), pRequest[j].busRoute.slavePort);
...
/*void freeResource(void) //should not be called
{
    ADSPPM_LOG_FUNC_ENTER;
    free(busdrvInfo.gExtRouteArray);
    free(busdrvInfo.gBwRequestHandle);
