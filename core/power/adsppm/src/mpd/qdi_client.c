...
Adsppm_Status ADSPPM_CheckCoreId(AdsppmCoreIdType core)
{
    ...
    if(Adsppm_Client_Ctx.initialized)
    {
        ret = qurt_qdi_handle_invoke(Adsppm_Client_Ctx.handle, ADSPPM_CHECKCOREID, &core, &sts);
    }
    if(ret < 0)
    {
...
Adsppm_Status ADSPPM_CheckMemId(AdsppmMemIdType mem)
{
    ...
    if(Adsppm_Client_Ctx.initialized)
    {
        ret = qurt_qdi_handle_invoke(Adsppm_Client_Ctx.handle, ADSPPM_CHECKMEMID, &mem, &sts);
    }
    if(ret < 0)
    {
        sts = Adsppm_Status_Failed;
...
Adsppm_Status ADSPPM_CheckClockId(AdsppmClkIdType clock)
{
    ...
    if(Adsppm_Client_Ctx.initialized)
    {
        ret = qurt_qdi_handle_invoke(Adsppm_Client_Ctx.handle, ADSPPM_CHECKCLKID, &clock, &sts);
    }
    if(ret < 0)
    {
        sts = Adsppm_Status_Failed;
...
Adsppm_Status ADSPPM_CheckBusPort(AdsppmBusPortIdType busport)
{
   Adsppm_Status sts = Adsppm_Status_Success;
    ...
    if(Adsppm_Client_Ctx.initialized)
    {
        ret = qurt_qdi_handle_invoke(Adsppm_Client_Ctx.handle, ADSPPM_CHECKBUSPORT, &busport, &sts);
    }
    if(ret < 0)
    {
        sts = Adsppm_Status_Failed;
