...
===============================================================================
$Header: //components/rel/core.adsp/2.6.1/power/mpm/src/hal/vmpm_HAL.c#1 $
 ...
 * driver. 
 */
vmpm_HAL_PlatformType registerLayout UMPM_DATA_SECTION;
...
{
  uint32 num_regs, offset, vmpm_wakeup_addr, vmpm_config_base_addr;
  
  vmpm_HAL_GetNumberMasks(&num_regs);
  offset = num_regs * sizeof(uint32);               /* Offset between subsequent 
                                                     * config register sets */

  vmpm_wakeup_addr = (uint32)msg_ram_base_ptr;      /* WakeupReg address */
  vmpm_config_base_addr = vmpm_wakeup_addr + 0x8;   /* Config regs base addr -
                                                     * WakeupReg is of 8 bytes 
                                                     * fixed */
