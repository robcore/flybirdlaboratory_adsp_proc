...
$Header: //components/rel/core.adsp/2.6.1/buses/slimbus/src/dal/SlimBusBamLib.c#1 $
...
static void SlimBusBamPipeCb(bam_result_type bam_result)
{
  ...
  if (0 != eEvent &&
      NULL != hEvent)
  {
    SlimBus_EventTriggerEx(hEvent, (uint32)pUser);
  }
}
...
void SlimBus_BamDoDeferredWork(SlimBusDevCtxt *pDevCtxt)
{
  ...
  if (NULL != pCtxt)
  {
    ...
    while (pCtxt->uXferTail != pCtxt->uXferHead)
    {
      ...
      uint32 uDescIdx, uDescSize, uLenMask;
...
SlimBus_ConfigMasterPipe( DalDeviceHandle * h, SlimBusResourceHandle  hPort, SlimBusPortFlowType  eFlow, const SlimBusPipeConfigType * pConfig) 
{
  ...
  SB_LOG_VERB_3("Configure master pipe (client: 0x%p) "
                "(hport: 0x%lx) (flow: %d)", pClientCtxt, hPort, eFlow);
  ...
  do
  {
    ...
    if (SB_SUCCESS != result)
    {
      SB_LOG_ERR_2("Error initializing BAM handle during config pipe "
                   "(client: 0x%p) (h: 0x%lx)", pClientCtxt, hPort);
      break;
    }
      ...
      do
      {
        ...
        if (NULL == pCtxt->hBAM)
        {
          ...
          SB_LOG_ERR_3("Failure to initialize BAM pipe "
                       "(client: 0x%p) (hport: 0x%lx) (hpipe: 0x%p)",
...
SlimBus_SubmitBamTransfer( DalDeviceHandle * h, SlimBusResourceHandle  hPort, SlimBusBamTransferType  eTransferDir, SlimBusBamIOVecType * pIOVec, void * pUser) 
{
  ...
  if (NULL != pszErrStr)
  {
    ...
    SB_LOG_ERR_4("%s (client: 0x%lx) (portres: 0x%lx) (chanres: 0x%lx)", pszErrStr,
                 (uint32)pClientCtxt, pPort->handle, ((NULL!=pChannel)?pChannel->handle:NULL));
    SlimBus_UnlockDevice(pDevCtxt);
  }
