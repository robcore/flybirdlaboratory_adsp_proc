...
$Header: //components/rel/core.adsp/2.6.1/buses/slimbus/src/common/SlimBus.c#1 $
...
#define COMMON_WORKAROUNDS (SB_WRKRND_O_PC_FIFO_SAMPLE_OFFSET|SB_WRKRND_O_RX_PORT_RESET|SB_WRKRND_O_IE_CLEAR|SB_WRKRND_O_SETUP_TRUST)

/* Global variable to allow interactive override of log level */
int32 g_iSlimBusLogLevelOverride = SB_LOG_LEVEL_UNDEFINED;

static const SlimBusWorkaroundType auCoreRevWorkrnd[SB_MAX_CORE_VERSIONS] =
{
  (SlimBusWorkaroundType)(SB_WRKRND_O_NO_PORT_MULTI_CHAN|COMMON_WORKAROUNDS),
  (SlimBusWorkaroundType)(COMMON_WORKAROUNDS)
};
...
SlimBus_WaitForAvailDeviceTrans(SlimBusClientCtxt *pClientCtxt, SlimBusDeviceType *pDevice)
{
  ...
  if (pDevice->uAvailTrans == 0)
  {
    ...
    if (NULL == pEventNode)
    {
      SB_LOG_ERR_1("No free events while waiting for available transaction (client 0x%p)", pClientCtxt);
      return SB_ERROR;
    }
    ...
    {
      pEventNode->TxMarker.uCount = 0;
      PushTailEvent(&pDevice->TransEventQueue, pEventNode);

      /* Activate the timer if it is not already active */
      ...
      result = SlimBus_EventWait(pEventNode->hEvent);

      SlimBus_LockDevice(pDevCtxt);

      /* On success, Rx queue processing logic will remove the event from trans queue */
      if (pEventNode->TxMarker.uCount > SB_TIMEOUT_FACTOR)
      {
        result = SLIMBUS_ERR_MSG_RESP_TIMEOUT;
      }
      else if (SB_SUCCESS != result)
      {
        RemoveEventNode(pEventNode);
      }

      if (SB_SUCCESS != result)
      {
        SB_LOG_ERR_2("Timeout while waiting for available transactions (client 0x%p) (LA: 0x%x)",
                     pClientCtxt, pDevice->LA);
        break;
      }
    }
...
SlimBus_WaitForBusActive(SlimBusClientCtxt *pClientCtxt, SlimBusDeviceType *pDevice)
{
      ...
      SB_PAUSE_NOT_PAUSED != pDevCtxt->pSched->ePause )
  {
    ...
    if (NULL == pEventNode)
    {
      SB_LOG_ERR_1("No free events while waiting for bus active (client 0x%p)", pClientCtxt);
      return SB_ERROR;
    }
    ...
    if (SB_SUCCESS != result)
    {
      SB_LOG_ERR_2("Timeout while waiting for bus active (client 0x%p) (LA: 0x%x)",
                   pClientCtxt, pDevice->LA);
      RemoveEventNode(pEventNode);
    }
...
uint32 SlimBus_SubmitRxDescriptors( SlimBusMsgFifoType *pMsgFifo )
{
  ...
  {
    uint32 uBase = pMsgFifo->uTailIdx&~uLenMask;
  ...
  while (uDescIdx + uDefaultLen < pMsgFifo->uTailIdx + pMsgFifo->uLen )
  {
    uint32 uNewDescIdx = uDescIdx + uDefaultLen;
    ...
    result = SlimBus_BamTransferIOVec(pMsgFifo->pBamCtxt,
                                      (uint32)&pMsgFifo->pPhysBuf[uDescIdx&uLenMask],
                                      (uNewDescIdx-uDescIdx)*sizeof(uint32), NULL, 
                                      SLIMBUS_BAM_IOVEC_FLAG_INT, TRUE);
    if (SB_SUCCESS != result)
    {
      ...
      SB_LOG_ERR_4("Error transferring Rx iovec (uNewDescIdx: 0x%lx) (uDescIdx: 0x%lx) (uTailIdx: 0x%lx) (uHeadIdx: 0x%lx)",
                   uNewDescIdx, uDescIdx, pMsgFifo->uTailIdx, pMsgFifo->uHeadIdx);
      break;
    }
...
)
{
  ...
  uint32 uLenMask, uTailIdx, uOldHeadIdx;
  uint32 *pBuf;
  ...
  if (NULL == pMsgFifo || NULL == pMsgFifo->pBuf || SB_MAX_MSG_LENGTH_BYTES < uNumBytes)
  {
    if (NULL != pMsgFifo)
    {
      SB_LOG_ERR_3("Invalid params to getmsgbuffer (pMsgFifo: 0x%p) (pBuf: 0x%p) "
                   "(uNumBytes: 0x%lx)", pMsgFifo, pMsgFifo->pBuf, uNumBytes);
    }
    return NULL;
  }
  ...
  {
    uint32 uSubtract;
    SlimBus_InterruptAcquireLock(SB_GET_PDEVCTXT(pMsgFifo->hDevCtxt));
    ...
    SlimBus_InterruptReleaseLock(SB_GET_PDEVCTXT(pMsgFifo->hDevCtxt));
  }
  
  if (pMsgFifo->uHeadIdx + uWordSize >= uTailIdx + pMsgFifo->uLen)
  {
    /* error: out of buffer space */
    SB_LOG_ERR_3("No buffer space in queue (uHeadIdx: 0x%lx) (uTailIdx: 0x%lx) (uLen: 0x%lx)",
                 pMsgFifo->uHeadIdx, pMsgFifo->uTailIdx, pMsgFifo->uLen);
    return NULL;
  }
...
)
{
  ...
  {
    uint32 uLenMask = pMsgFifo->uLen-1;
    uint32 uHeadIdx = pMsgFifo->uHeadIdx;
    ...
    {
      uint32 uDescIdx = pEvent->TxMarker.pBuf - pMsgFifo->pBuf;
      uint32 length = SB_MSG_GET(*pEvent->TxMarker.pBuf,GENERIC_0,LEN);
    ...
    while ( pMsgFifo->uTailIdx < uHeadIdx )
    {
      ...
      result = SlimBus_BamTransferIOVec(pMsgFifo->pBamCtxt,
                                        (uint32)&pMsgFifo->pPhysBuf[pMsgFifo->uTailIdx&uLenMask],
                                        (uNewTailIdx-pMsgFifo->uTailIdx)*sizeof(uint32),
                                        NULL, uFlags, FALSE);
      if (SB_SUCCESS != result)
      {
...
)
{
  ...
  if (pMsgFifo->eFlags & SB_MSG_FIFO_ENABLE_NOTIFY)
  {
    ...
    else
    {
      ...
      SB_LOG_ERR_1("No free events during EnqueueMsgEvent (client 0x%p)", pClientCtxt);
    }
  }
  return pEvent;
}
...
{
  uint32 uIdx = pBuf - pMsgFifo->pBuf;
 ...
 */
static SlimBusDeviceType* SlimBus_GetDevicePtr
(
  SlimBusDevCtxt *pDevCtxt,
            ...
            SB_MSG_MC_REPLY_INFORMATION == eMC) )
      {
        if (!SB_TEST_REPLIES_IGNORED())
        {
              ...
              SB_BF_GET(pEventNode->uRxMarker,RXMARKER,TID) == TID)
          {
            ...
            if (uCount > 0 && NULL != pEventNode->pBuf)
            {
              SlimBus_Memcpy(pEventNode->pBuf, pDataBuf, uCount);
            }
            if (NULL != pEventNode->pBufSize)
            {
              *pEventNode->pBufSize = uCount;
            }
  ...
  if (0 != uIntStatus)
  {
    ...
    for (i = 0; (i < pDevCtxt->numPorts) && (uIntMask < uIntStatus); i++)
    {
      ...
      if (uIntMask & uIntStatus)
      {
        ...
        {
          /* Keep the interrupt disabled until the underflow/overflow
             can be processed */
...
)
{
  ...
  SB_LOG_VERB_3("Got port disconnection (client: 0x%p) (resource: 0x%lx) (port: %ld)",
                pPort->pClientCtxt, pPort->handle, pPort->portNum);
  ...
  if (NULL != pPort->pChannel)
  {
    RemovePortNode(pPort);
    pPort->pChannel = NULL;
  }
  ...
  /* Unless we have the Rx port reset workaround and the BAM still needs
     to be reset, indicate that the port is disconnected */
  if ( !bNeedBamDisconnect ||
       !(pDevCtxt->eWrkrnd & SB_WRKRND_O_RX_PORT_RESET) ||
  ...
  if ( uIntStatus != 0 )
  {
         ...
         ( uIntStatus & HAL_SB_MGR_INT_TX_MSG_NACKED_TWICE ) )
    {
      ...
      while( NULL != ( pEventNode = PeekHeadEvent(&pDevCtxt->TxEventQueue) ) )
      {
        ...
        if ( !SlimBus_IsMsgSendComplete(&pDevCtxt->TxMsgFifo, pEventNode->TxMarker.pBuf) )
        {
          break;
        }
    ...
    {
      /* Signal any client that may be waiting for reconfiguration to complete */
      if (NULL != pDevCtxt->pReconfigWaitClient)
      {
        SlimBus_EventTrigger(pDevCtxt->pReconfigWaitClient->hClientEvent);
        pDevCtxt->pReconfigWaitClient = NULL;
      }
           ...
           SB_SATELLITE_DRIVER == pDevCtxt->eDrvType )
      {
        ...
        for (uIdx = 0; uIdx < pDevCtxt->numPorts; uIdx++)
        {
          ...
          if ( pPort->bBamConnected &&
               NULL != pPort->pChannel &&
               NULL != pPort->pChannel->pClientCtxt &&
               pPort->pChannel->pClientCtxt->bReConfigPending &&
               SLIMBUS_CHANNEL_STATUS_ACTIVE == pPort->pChannel->eNewStatus &&
               SLIMBUS_CHANNEL_STATUS_ACTIVE != pPort->pChannel->eCurStatus )
          {
            if ( HAL_sb_PgdPortIsChannelActive(pDevCtxt->hSB, 0, pPort->portNum) )
            {
              SlimBusDeviceType *pMgrDevice;
              SlimBusEventNodeType *pEvent = NULL;
              uint32 TID = pPort->pChannel->pClientCtxt->uSatReconfigTID;
              ...
              if (NULL != pMgrDevice)
              {
                ...
                if (NULL != pEvent)
                {
                  ...
                  if (NULL != pEvent->pBuf)
                  {
                    *(uint32*)pEvent->pBuf = SB_ACK_SUCCESS_FLAG;
                  }
                  
                  SB_LOG_INFO_3("Simulating master ACK for reconfiguration sequence "
                                "(channel already active) (client: 0x%p) (channel: %ld) "
                                "(port: %ld)", pPort->pChannel->pClientCtxt,
                                pPort->pChannel->channelNum, pPort->portNum );
  ...
  if ( 0 != uPgdIntStatus )
  {
    ...
    for (i = 0; i < pDevCtxt->numPorts; i++)
    {
      ...
      if ( uPgdIntStatus & uIntMask )
      {
        ...
        {
          if ( ( NULL != pPort->pChannel &&
                 SLIMBUS_CHANNEL_STATUS_REMOVED == pPort->pChannel->eNewStatus ) ||
               SB_HANDLE_ALLOCATED != pPort->eState )
          {
            SB_LOG_INFO_3("Detected port overflow/underflow during removal "
                          "(client: 0x%p) (resource: 0x%lx) (port: %ld)",
                          pPort->pClientCtxt, pPort->handle, pPort->portNum);
          }
          else
          {
  ...
  if ( uPgdIntStatus != 0 )
  {
    ...
    for (i = 0; i < pDevCtxt->numPorts; i++)
    {
      ...
      if ( uPgdIntStatus & uIntMask )
      {
        ...
        {
          SlimBus_EventTriggerEx(pPort->hEvent, (uint32)pPort->pEventUserData);
        }
      }
    }
  }
 ...
 * @param pDevCtxt  Pointer to the device context
 */
void SlimBusTimerCallback(SlimBusDevCtxt *pDevCtxt)
{
  SlimBusEventNodeType *pRxEventNode, *pNextRxEventNode;
  /* List of events that need to be signalled */
  SlimBusLinkNodeType SignalList;
  SBBOOL bMsgRx = FALSE, bClientWait = FALSE, bClientWaitAgain = FALSE;
  uint32 uIdx;

  /* Initialize the event signalling list structure */
  RootNodeInit(&SignalList);

  SlimBus_LockDevice(pDevCtxt);

  pDevCtxt->bInTimer = TRUE;
  pDevCtxt->bTimerActive = FALSE;

  pRxEventNode = PeekHeadEvent(&pDevCtxt->RxEventQueue);

  if ( NULL != pDevCtxt->pSched &&
       pDevCtxt->pSched->uSchedTimerCnt > 0)
  {
    if (++pDevCtxt->pSched->uSchedTimerCnt >= SB_RECONFIG_TIMEOUT_COUNT)
    {
      /* We got a timeout during the reconfiguration sequence, so abort it */
  ...
  while ( NULL != pRxEventNode )
  {
    ...
    {
      bMsgRx = TRUE;
      SB_LOG_WARN_0("Timer event while waiting for message response");
    }
    
    pNextRxEventNode = NextEventNode(&pDevCtxt->RxEventQueue, pRxEventNode);
    
    if (++pRxEventNode->TxMarker.uCount > SB_TIMEOUT_FACTOR)
    {
      LA = SB_BF_GET(pRxEventNode->uRxMarker,RXMARKER,LA);
      TID = SB_BF_GET(pRxEventNode->uRxMarker,RXMARKER,TID);
      pDevice = pDevCtxt->pDevices[LA&SB_LA_IDX_MASK];
      
      if (NULL != pRxEventNode->peMsgErr)
      {
        *(pRxEventNode->peMsgErr) = SLIMBUS_ERR_MSG_RESP_TIMEOUT;
      }
      SB_LOG_ERR_0("message ack timeout");
      
      SlimBus_CleanupMsgEvent(pDevice, TID, pRxEventNode, &SignalList);
    }
    
    pRxEventNode = pNextRxEventNode;
  }

  if ( 0 != pDevCtxt->uPortDisconnectMask )
  {
    for (uIdx = 0; uIdx < pDevCtxt->numPorts; uIdx++)
    {
      SlimBusMasterPortType *pPort = &pDevCtxt->Ports[uIdx];
      ...
      if ( pPort->uSignalDescEmptyCnt > 0 )
      {
        ...
        {
          bClientWait = TRUE;
          SB_LOG_WARN_0("Timer event while waiting for port desc to empty");
        }
        
        if ( pPort->uSignalDescEmptyCnt++ > 1 )
        {
          SB_LOG_ERR_1("timeout waiting for port desc to empty (portres: 0x%lx)", 
                       pPort->handle);
        ...
        {
          bClientWaitAgain = TRUE;
        }
      }
    }
  }
  
  if ( pDevCtxt->uNumTxMsgWaiters > 0 )
  {
    for (uIdx = 0; uIdx < pDevCtxt->numDeviceSlots; uIdx++)
    {
      ...
      if ( NULL != pDevice )
      {
        SlimBusEventNodeType *pEvent = PeekHeadEvent(&pDevice->TransEventQueue);
        while (NULL != pEvent)
        {
          SlimBusEventNodeType *pNextEvent = NextEventNode(&pDevice->TransEventQueue, pEvent);

          if (++pEvent->TxMarker.uCount > SB_TIMEOUT_FACTOR)
          {
            SB_LOG_ERR_1("Timeout while waiting for available transactions (LA: 0x%x)", 
                         pDevice->LA);
          ...
          {
            SB_LOG_WARN_1("Timer event while waiting for available transactions (LA: 0x%x)", pDevice->LA);
            bClientWaitAgain = TRUE;
          }
  ...
  if ( pDevCtxt->TxMsgFifo.bBusy &&
       ++pDevCtxt->uTxMsgBusyCnt > SB_TIMEOUT_FACTOR )
  {
    if ( pDevCtxt->bIsHwOwner &&
         !pDevCtxt->bPendingSSR )
    {
      SB_LOG_ERR_0("Timeout occured while waiting for message to be sent, triggering SSR recovery");
  ...
  if ( !bMsgRx && 
       ...
       NULL != PeekHeadEvent(&pDevCtxt->TxEventQueue) )
  {
    SB_LOG_WARN_0("Timer event while waiting for message to be sent");
    SlimBus_CheckFramerStateInternal(pDevCtxt);
  }
  
  if ( NULL != PeekHeadEvent(&pDevCtxt->TxEventQueue) ||
       NULL != PeekHeadEvent(&pDevCtxt->RxEventQueue) ||
       bClientWaitAgain ||
       ( NULL != pDevCtxt->pSched &&
  ...
  else
  {
    ...
    /* Clean up and make sure that we do not leave bBusy set */
    if ( pDevCtxt->bUseTxMsgQueue )
    {
      pDevCtxt->TxMsgFifo.bBusy = FALSE;
    }
...
void SlimBusBamResetTimerCallback(SlimBusDevCtxt *pDevCtxt)
{
  ...
  if (NULL == pDevCtxt->pBamResetClientCtxt)
  {
    SBResult result;
    result = SlimBus_Malloc(sizeof(*pDevCtxt->pBamResetClientCtxt),
                            (void**)&pDevCtxt->pBamResetClientCtxt,
                            SB_MEMTAG_20);
    if (SB_SUCCESS == result)
    {
      memset(pDevCtxt->pBamResetClientCtxt,0,sizeof(SlimBusClientCtxt));
      pDevCtxt->pBamResetClientCtxt->hDevCtxt = SlimBus_GetDevHandle(pDevCtxt);
      result = SlimBus_ClientInit(pDevCtxt->pBamResetClientCtxt);
    }
    if (SB_SUCCESS != result)
    {
      SlimBus_ErrorFatal(pDevCtxt, "Failure to allocate or init BAM reset client context");
    }
  }
  ...
  if (0 != pDevCtxt->uPortBamResetMask)
  {
    ...
    for ( uIdx = 0; uIdx < pDevCtxt->numPorts; uIdx++ )
    {
      ...
      if ( ( uIntMask & pDevCtxt->uPortBamResetMask ) &&
           pPort->bBamConnected )
      {
        SB_LOG_VERB_3("disconnecting BAM pipe (client: 0x%p) (resource: 0x%lx) (port: %ld)",
                      pPort->pClientCtxt, pPort->handle, pPort->portNum);

        SlimBus_DisconnectBAM(pPort);
        
        if ( (pDevCtxt->eWrkrnd & SB_WRKRND_O_RX_PORT_RESET) &&
             (SLIMBUS_PORT_SINK_FLOW == pPort->eFlow) )
        {
          SlimBusDeviceType *pDevice = pDevCtxt->pDevices[(pDevCtxt->pgdLA)&SB_LA_IDX_MASK];
          SB_LOG_VERB_3("sending secondary port disconnect "
                        "(client: 0x%p) (resource: 0x%lx) (port: %ld)",
                        pPort->pClientCtxt, pPort->handle, pPort->portNum);
          ...
          pDevCtxt->disconnectPortFcn(pDevCtxt->pBamResetClientCtxt, pDevCtxt,
                                      pDevice, pPort->portNum);
        }
        else
        {
          SlimBus_PortAcquireLock(pDevCtxt, pPort);
...
SlimBus_ClientInit(SlimBusClientCtxt *pCtxt)
{
  ...
  {
    SlimBusClientCtxt **paNewClientCtxt = NULL;
    
    if (SB_SUCCESS != SlimBus_Malloc(pDevCtxt->numClientSlots*2*sizeof(paNewClientCtxt[0]),
                                     (void**)&paNewClientCtxt, SB_MEMTAG_4))
    {
      return SB_ERROR;
    }
    memset(paNewClientCtxt, 0, pDevCtxt->numClientSlots*2*sizeof(paNewClientCtxt[0]));
    SlimBus_Memcpy(paNewClientCtxt, pDevCtxt->paClientCtxt,
                   pDevCtxt->numClientSlots*sizeof(paNewClientCtxt[0]));
    SlimBus_Free(pDevCtxt->paClientCtxt);
    pDevCtxt->paClientCtxt = paNewClientCtxt;
    pDevCtxt->numClientSlots *= 2;
...
SBResult SlimBus_DeviceInitInternal(SlimBusDevCtxt *pDevCtxt)
{
  ...
  do
  {
    ...
    if ( NULL == pDevCtxt->pBSP ||
         2 != pDevCtxt->pBSP->uStructVer )
    {
      result = SB_ERROR;
      break;
    }
    for (i = 0; i < SB_MAX_CORE_HW_REVS; i++)
    {
      if ( pDevCtxt->pBSP->uCoreVer[i] >= SB_MAX_CORE_VERSIONS )
      {
        result = SB_ERROR;
        break;
      }
    }
    /* Use the HW first revision workarounds as a starting point */
    pDevCtxt->eWrkrnd = auCoreRevWorkrnd[pDevCtxt->pBSP->uCoreVer[0]];
    ...
    pDevCtxt->prnSeed = (uint8)(((uint32)pDevCtxt>>2) ^ ((uint32)&result>>2));

    pDevCtxt->numDeviceSlots = SB_MAX_DEVICES;
    pDevCtxt->numGroupSlots = SB_MAX_GROUPS;

    if (pDevCtxt->numPorts > SB_MAX_MASTER_PORTS)
    {
      pDevCtxt->numPorts = SB_MAX_MASTER_PORTS;
    }
    ...
    {
      pDevCtxt->Counters[i].counterNum = i;
...
{
  HAL_sb_DeviceIntSetMask(pDevCtxt->hSB, pDevCtxt->eMsgDev, 0);
...
SlimBus_DeviceInitHardware(SlimBusDevCtxt *pDevCtxt)
{
  ...
  if ( pDevCtxt->bIsHwOwner )
  {
    ...
    if ( SlimBus_CheckGpioClockToggle(pDevCtxt) )
    {
      if ( pDevCtxt->bActiveFramer )
      {
        ...
        if ( !pDevCtxt->bHwInit &&
             SB_MASTER_DRIVER == pDevCtxt->eDrvType &&
             NULL != pDevCtxt->pSched )
        {
    ...
    {
      if (pDevCtxt->bProgBamTrust)
      {
        /* Workaround hack for BAM secure config */
        __outpdw(pDevCtxt->hSB - (pDevCtxt->pBSP->uHwioBase - pDevCtxt->pBSP->hBamDev) + 0x4030,
                 pDevCtxt->auEeAssign[1]);
        __outpdw(pDevCtxt->hSB - (pDevCtxt->pBSP->uHwioBase - pDevCtxt->pBSP->hBamDev) + 0x5030,
                 pDevCtxt->auEeAssign[1]);

        if (HAL_sb_PgdGetNumPorts(pDevCtxt->hSB, 0) > 12)
        {
          __outpdw(pDevCtxt->hSB - (pDevCtxt->pBSP->uHwioBase - pDevCtxt->pBSP->hBamDev) + 0x1d030,
                   pDevCtxt->auEeAssign[1]);
          __outpdw(pDevCtxt->hSB - (pDevCtxt->pBSP->uHwioBase - pDevCtxt->pBSP->hBamDev) + 0x1e030,
                   pDevCtxt->auEeAssign[1]);
          __outpdw(pDevCtxt->hSB - (pDevCtxt->pBSP->uHwioBase - pDevCtxt->pBSP->hBamDev) + 0x1f030,
                   pDevCtxt->auEeAssign[1]);
        }
      }
  ...
  if (uNumHwCounters < pDevCtxt->numCounterSlots)
  {
    pDevCtxt->numCounterSlots = uNumHwCounters;
  }
  
  for ( i = 0; i < pDevCtxt->numPorts; i++ )
  {
    /* Read all the R/W port registers to make sure the shadow copy gets
       initialized early in case we are shadowing these register values */
    (void) HAL_sb_PgdPortIsEnabled(pDevCtxt->hSB, 0, pDevCtxt->Ports[i].portNum);
    (void) HAL_sb_PgdPortGetMultiChanl(pDevCtxt->hSB, 0, pDevCtxt->Ports[i].portNum);
    (void) HAL_sb_PgdPortGetBlkSize(pDevCtxt->hSB, 0, pDevCtxt->Ports[i].portNum);
    (void) HAL_sb_PgdPortGetTransSize(pDevCtxt->hSB, 0, pDevCtxt->Ports[i].portNum);
  }
    
  for ( i = 0; i < pDevCtxt->numCounterSlots && i < sizeof(pDevCtxt->Counters)/sizeof(pDevCtxt->Counters[0]); i++ )
  {
    /* Read the R/W counter register to make sure the shadow copy gets
       initialized early in case we are shadowing this register value */
    (void) HAL_sb_PgdPcGetPortNum(pDevCtxt->hSB, 0, pDevCtxt->Counters[i].counterNum);
  }
...
SlimBus_DeviceOpen(SlimBusHandle h, uint32 mode)
{
  ...
  {
    SB_LOG_INFO_2("Slimbus device opened (client: 0x%p) (#open %ld)",
                  pClientCtxt, pDevCtxt->uOpenCnt+1);
    ...
    else if (SB_SUCCESS == result)
    {
      ...
      if (SB_SUCCESS == result)
      {
        ...
        if ( !SlimBus_HardwareIsEnumerated(pDevCtxt) )
        {
  ...
  {
    SB_LOG_ERR_2("Error during device open (client: 0x%p) (#open %ld)",
                 pClientCtxt, pDevCtxt->uOpenCnt);
  }
  ...
  if ( bWaitForEnum )
  {
    if (SB_SUCCESS != SlimBus_EventTimedWait(pClientCtxt,
       ...
       pClientCtxt->bOpenb4SSR )
  {
    ...
    for (uIdx = 0; uIdx < pDevCtxt->numPorts; uIdx++)
    {
      ...
      if ( pClientCtxt == pPort->pClientCtxt &&
           SB_HANDLE_ALLOCATED == pPort->eState )
      {
        SB_LOG_INFO_2("Deallocating port during device close (client: 0x%p) (hport: %lx)",
                      pClientCtxt, pPort->handle);
    ...
    SB_LOG_INFO_2("Slimbus device closed (client: 0x%p) (#open %ld)",
                  pClientCtxt, pDevCtxt->uOpenCnt);
...
SlimBus_GetMasterPgdLA( SlimBusClientCtxt *pClientCtxt, SlimBusLogicalAddrType *pLA )
{
  ...
  if ( SB_MSG_LA_MANAGER == pDevCtxt->pgdLA )
  {
    ...
    for (result = SB_ERROR; SB_SUCCESS != result && uRev < SB_MAX_CORE_HW_REVS; uRev++)
    {
      /* Try different chip revisions (EA PC[3:0]) */
      uaPgdEA[2] = (uint8) ((uaPgdEA[2] & 0xf0) | uRev);
      ...
      if (SB_SUCCESS == result)
      {
        /* Update the workaround flags once we know the HW core revision */
        pDevCtxt->eWrkrnd = auCoreRevWorkrnd[pDevCtxt->pBSP->uCoreVer[uRev]];
...
SlimBus_GetDeviceLogicalAddr(SlimBusHandle h, const uint8 *pEA, uint32 uEASize, SlimBusLogicalAddrType * pLA) 
{
  ...
  SB_LOG_INFO_3("Got LA query (client: 0x%p) (EA: 0x%04x%08lx)", 
                pClientCtxt, *(uint16*)&pEA[4], *(uint32*)&pEA[0]);
  ...
  if (SB_SUCCESS != result)
  {
    SB_LOG_WARN_3("logical address lookup failed (client: 0x%p) "
                  "(EA: 0x%04x%08lx)", pClientCtxt, *(uint16*)&pEA[4], *(uint32*)&pEA[0]);
  }
...
)
{
          ...
          (pPort->eDisconnectState != SB_PORT_NOT_DISCONNECTING) )
  {
    ...
    SB_LOG_INFO_3("Waiting for port disconnection during deallocation "
                  "(client: 0x%p) (resource: 0x%lx) (port: %ld)", pCtxt, 
       ...
       pPort->bBamConnected )
  {
    /* This condition should never be true, we should disconnect the BAM
       in the ISR */
    SB_LOG_VERB_3("disconnecting BAM pipe during dealloc (client: 0x%p) (resource: 0x%lx) "
                  "(port: %ld)", pPort->pClientCtxt, pPort->handle, pPort->portNum);
    SlimBus_DisconnectBAM(pPort);
  }
...
SlimBus_DeallocMasterPorts(SlimBusHandle h, SlimBusResourceHandle * phPortBuf, uint32  uPortBufSize) 
{
  ...
  for (portIdx = 0; portIdx < numPorts; portIdx++)
  {
    ...
    if ( i < pDevCtxt->numPorts && pDevCtxt->Ports[i].handle == phPortBuf[portIdx] )
    {
    ...
    if (pDevCtxt->Ports[i].eState == SB_HANDLE_AVAILABLE)
    {
      ...
      if ( eReqs & SLIMBUS_PORT_REQ_O_HALF_DUPLEX )
      {
             ...
             pPort->bBamConnected )
        {
          /* This condition should never be true, we should disconnect the BAM
             in the ISR */
          SB_LOG_VERB_3("disconnecting BAM pipe during alloc (client: 0x%p) (resource: 0x%lx) "
                        "(port: %ld)", pClientCtxt, pPort->handle, pPort->portNum);
          SlimBus_DisconnectBAM(pPort);
        }
  ...
  do
  {
    ...
    if (pPort->eDisconnectState != SB_PORT_NOT_DISCONNECTING)
    {
      SB_LOG_INFO_3("Waiting for port disconnection during config port "
                    "(client: 0x%p) (resource: 0x%lx) (port: %ld)", pClientCtxt, 
                    pPort->handle, pPort->portNum );
    ...
    for ( ; NULL != pPort; pPort = pOtherPort, pOtherPort = NULL )
    {
      ...
      if ( HAL_sb_IsPortUpperWatermarkSupported(pDevCtxt->hSB) )
      {
        if ( TRUE == pConfig->bUpperWaterMarkValid )
        {
          HAL_sb_PgdPortSetUpperWatermark(pDevCtxt->hSB, 0, portNum, pConfig->uUpperWaterMark/sizeof(uint32));
...
SlimBus_NextReserveMsgBandwidth(SlimBusHandle h, uint32  uBandwidth_bps) 
{
  ...
  do
  {
    ...
    if ( uSlots != pClientCtxt->uNewReservedMsgSlots )
    {
      ...
      SB_LOG_VERB_2("Got message bandwitdth request (client: 0x%p) (bps: %ld)",
                    pClientCtxt, uBandwidth_bps);
...
SlimBus_SendClientMessage(SlimBusClientCtxt *pClientCtxt, uint32 *pBuf, SlimBusDeviceType *pDevice, uint8 *pucReadData, uint32 uReadDataLen, uint32 *puActualReadDataLen, SlimBusEventHandle hEvent)
{
  ...
  /* Wait for completion of the transaction (if operation is not non-blocking) */
  ...
  {  
    ...
    if (NULL != pucReadData)
    {
      ...
      pEventNode->pBuf = pucReadData;
      pEventNode->pBufSize = puActualReadDataLen;
      pDevice->TIDMap[TID&SB_TID_IDX_MASK] = pEventNode;
    }
    else
    {
      pEventNode->uRxMarker = 0;
      pEventNode->pBuf = NULL;
      pEventNode->pBufSize = NULL;
    }
    ...
    if (pClientCtxt->hClientEvent == hEvent)
    {
      ...
      if (SB_SUCCESS != result)
      {
        SB_LOG_ERR_1("Client message transaction failure (client: 0x%p)", pClientCtxt);
      }
    }
  }
...
SlimBus_DoReadWriteOp( SlimBusClientCtxt *pClientCtxt, uint32 MC, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  const uint8*  pucWriteData,  uint32  uWriteDataLen, uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, SlimBusEventHandle  hEvent )
{
  ...
  {
    SB_LOG_ERR_2("DoReadWriteOp got bad slice size (ucCurSize: 0x%d) (uSliceLen: %ld)",
                 ucCurSize, uSliceLen);
    return SB_ERROR;
  }
  ...
  do
  {
    ...
    if (SLIMBUS_EVENT_HANDLE_NON_BLOCKING != hEvent)
    {
      ...
      {
        SB_LOG_ERR_1("No free events during DoReadWriteOp (client 0x%p)", pClientCtxt);
        result = SB_ERROR;
        break;
      }
    }
    ...
    /* Get message buffer */
    pBuf = pDevCtxt->TxMsgFifo.getBufFcn(pClientCtxt, pDevice, 
                                         &pDevCtxt->TxMsgFifo, uMsgLen);
    if (NULL == pBuf)
    {
      result = SB_ERROR;
      break;
...
SlimBus_ReadValueElement(SlimBusHandle h, SlimBusLogicalAddrType  LA, uint32  uByteAddr,  uint8*  pucReadData,  uint32  uReadDataLen,  uint32*  puActualReadDataLen, SlimBusEventHandle  hEvent) 
{
  ...
  {
    SlimBus_LockDevice(SB_GET_PDEVCTXT(pClientCtxt->hDevCtxt));
    SB_LOG_ERR_4("Read VE validation error (bOpen: %ld) (uByteAddr: 0x%lx) "
                 "(uReadDataLen: %ld) (pucReadData: 0x%p)", pClientCtxt->bOpen,
                 uByteAddr, uReadDataLen, pucReadData);
    SlimBus_UnlockDevice(SB_GET_PDEVCTXT(pClientCtxt->hDevCtxt));
    return SB_ERROR;
  }
...
SlimBus_WriteValueElement(SlimBusHandle h, SlimBusLogicalAddrType  LA, uint32  uByteAddr, uint32  uByteSize, const uint8*  pucWriteData,  uint32  uWriteDataLen, SlimBusEventHandle  hEvent) 
{
       ...
       ( uByteAddr + uByteSize > SB_VE_MAP_RESERVED_ADDR ) )
  {
    ...
    SB_LOG_ERR_5("Write VE validation error (bOpen: %ld) (uByteAddr: 0x%lx) (uByteSize: %ld) "
                 "(uWriteDataLen: %ld) (pucWriteData: 0x%p)", pClientCtxt->bOpen,
                 uByteAddr, uByteSize, uWriteDataLen, pucWriteData);
    SlimBus_UnlockDevice(SB_GET_PDEVCTXT(pClientCtxt->hDevCtxt));
    return SB_ERROR;
  }
...
SlimBus_SendUserDefinedMessage(SlimBusHandle h, SlimBusLogicalAddrType LA, uint32 MC, const uint8* pucPayload, uint32 uPayloadLen, SlimBusEventHandle hEvent)
{
       ...
       ( NULL == pucPayload && uPayloadLen > 0 ) )
  {
    ...
    SB_LOG_ERR_3("Send user-defined msg VE validation error (bOpen: %ld) "
                 "(uPayloadLen: %ld) (pucPayload: 0x%p)", pClientCtxt->bOpen,
                 uPayloadLen, pucPayload);
    SlimBus_UnlockDevice(SB_GET_PDEVCTXT(pClientCtxt->hDevCtxt));
    return SB_ERROR;
  }
  ...
  do
  {
    ...
    if (SLIMBUS_EVENT_HANDLE_NON_BLOCKING != hEvent)
    {
      if (NULL == PeekHeadEvent(&pClientCtxt->FreeEventStack))
      {
        SB_LOG_ERR_1("No free events during SendUserDefinedMessage (client 0x%p)", pClientCtxt);
        result = SB_ERROR;
        break;
      }
    }
    ...
    *pBuf = SB_MSG_ASM_FIRST_WORD(uMsgLen, SB_MSG_MT_DEST_REFERRED_USER,
                                  MC, SB_MSG_DT_UNICAST, LA);
    {
      ...
      {
        SlimBus_Memcpy(pucBuf, pucPayload, uPayloadLen);
      }
    }
  ...
  for (i = 0; i < (int)pDevCtxt->numChannelSlots; i++)
  {
    ...
    if (NULL == pChannel)
    {
      ...
      if (NULL != pChannel)
      {
        ...
        RootNodeInit(&pChannel->MasterPortList);
      }
    }
    if (NULL != pChannel &&
        pChannel->eState == SB_HANDLE_AVAILABLE)
...
SlimBus_NextDefineDataChannel(SlimBusHandle h, SlimBusResourceHandle  hChannel, const SlimBusChannelReqType * pChannelReq) 
{
  ...
  do
  {
    ...
    SB_LOG_VERB_8("Got define channel request (client: 0x%p) (resource: 0x%lx) (channel: %ld) "
                  "(#channels: %ld) (protocol: %d) (baserate: %d) (ratemult: %ld) (bitwidth: %ld)",
                  pClientCtxt, hChannel, pChannel->channelNum, uCnt, 
...

    {
      ...
      SlimBusMasterPortType *pMasterPort;
      SBBOOL bFirst = TRUE;

      /* Validate that all the master ports are configured the same */
      do
      {
        pMasterPort = PeekHeadPort(&pTempChannel->MasterPortList);
        if ( NULL != pMasterPort &&
             0 == HAL_sb_PgdPortGetChannelNumber(pDevCtxt->hSB, 0, pMasterPort->portNum) )
        {
          if (bFirst)
          {
            eFlow = pMasterPort->eFlow;
            eReqs = pMasterPort->eReqs;
            bFirst = FALSE;
          }
          else if (eFlow != pMasterPort->eFlow ||
                   eReqs != pMasterPort->eReqs)
          {
      ...
      else
      {
        ...
        else
        {
          SB_LOG_ERR_1("Auto iso mode specified for channel but unable"
                       "to determine flow direction (client: 0x%p)", pClientCtxt );
          result = SB_ERROR;
          break;
        }
      }
      ...
      SB_MSG_SET(TID,USR_CHANNEL_DEF_1,TID);
    {
      ...
      do
      {
        SlimBusMasterPortType *pMasterPort = NULL;
        
        *(pucBuf++) = (uint8)pChannel->channelNum;
        
        while (NULL != (pMasterPort = NextPortNode(&pChannel->MasterPortList, pMasterPort)))
        {
          if (0 == HAL_sb_PgdPortGetChannelNumber(pDevCtxt->hSB, 0, pMasterPort->portNum))
          {
            /* Program the push/pull fraction counter if protocol is push or pull */
            if ( ( SLIMBUS_PROTO_PULLED == eProto &&
                   SLIMBUS_PORT_SINK_FLOW == pMasterPort->eFlow ) ||
                 ( SLIMBUS_PROTO_PUSHED == eProto &&
                   SLIMBUS_PORT_SOURCE_FLOW == pMasterPort->eFlow ) )
            {
    ...
    if (SB_SUCCESS != result)
    {
      SB_LOG_ERR_1("Master ack failure in call to define channel (client: 0x%p)",
                   pClientCtxt );
    }
  } while (FALSE);
  ...
  if (SB_SUCCESS != result)
  {
    SB_LOG_ERR_1("Error during define data channel request (client: 0x%p)", pClientCtxt);
  }
...
SlimBus_ConnectPortToChannel(SlimBusHandle h, SlimBusResourceHandle  hChannel, SlimBusPortFlowType  eFlow, SlimBusResourceHandle  hPort) 
{
  ...
  do
  {
    ...
    if ( SB_GET_HANDLE_TYPE(hPort) == SB_HANDLE_TYPE_MASTER_PORT )
    {
      ...
      SB_LOG_VERB_6("Got connect master port request (client: 0x%p) "
                    "(chanres: 0x%lx) (portres: 0x%lx) (channel: %ld) (port: %ld) "
                    "(flow: %d)", pClientCtxt, hChannel, hPort, chanNum, portNum,
                    eFlow);
    ...
    else if ( SB_GET_HANDLE_TYPE(hPort) == SB_HANDLE_TYPE_SLAVE_PORT )
    {
      ...
      SB_LOG_VERB_7("Got connect slave port request (client: 0x%p) "
                    "(chanres: 0x%lx) (portres: 0x%lx) (channel: 0x%lx) (LA: 0x%x) "
                    "(port: 0x%lx) (flow: %d)", pClientCtxt, hChannel, hPort,
                    chanNum, LA, portNum, eFlow);
    }
    else
    {
      result = SB_ERROR;
    ...
    result = pDevCtxt->connectPortFcn(pClientCtxt, pDevice, eFlow, portNum, chanNum);
    if (SB_SUCCESS != result)
    {
      break;
    }
    ...
    {
      RemovePortNode(pPort);
      pPort->pChannel = pChannel;
      PushTailPort(&pChannel->MasterPortList, pPort);
  ...
  if (SB_SUCCESS != result)
  {
    SB_LOG_ERR_1("Error during connect channel request (client: 0x%p)", pClientCtxt);
  }
...
SlimBus_DisconnectPortFromChannel(SlimBusHandle h, SlimBusResourceHandle  hPort) 
{
  ...
  do
  {
    ...
    if ( SB_GET_HANDLE_TYPE(hPort) == SB_HANDLE_TYPE_MASTER_PORT )
    {
      ...
      SB_LOG_VERB_3("Got disconnect master port request (client: 0x%p) "
                    "(resource: 0x%lx) (port: %ld)", pClientCtxt, hPort, portNum);
    }
    else if ( SB_GET_HANDLE_TYPE(hPort) == SB_HANDLE_TYPE_SLAVE_PORT )
    {
      LA = SB_GET_HANDLE_LA(hPort);
      ...
      SB_LOG_VERB_4("Got disconnect slave port request (client: 0x%p) "
                    "(resource: 0x%lx) (LA: 0x%x) (port %ld)", pClientCtxt, 
                    hPort, LA, portNum);
    }
    else
    {
  ...
  if (SB_SUCCESS != result)
  {
    SB_LOG_ERR_1("Error during disconnect channel request (client: 0x%p)", pClientCtxt);
  }
...
SlimBus_NextDataChannelControlInternal(SlimBusClientCtxt *pClientCtxt, SlimBusChannelType *pChannel, SlimBusGroupType *pGroup, SlimBusChannelCtrlType  eCtrl) 
{
  ...
  do
  {
    const SlimBusMasterPortType *pMasterPort = PeekHeadPort(&pChannel->MasterPortList);
    uint32 TID;
    ...
    SB_LOG_VERB_7("Got data channel control request (client: 0x%p) "
                  "(resource: 0x%lx) (channel: %ld) (#channels: %ld) (op: %d) "
                  "(#descs: %ld) (#bytes: %ld)", pClientCtxt, hRes,
                  pChannel->channelNum, uCnt, eCtrl,
                  ((NULL==pMasterPort)?((uint32)-1):(pMasterPort->uDescSubmit)),
                  ((NULL==pMasterPort)?((uint32)-1):(pMasterPort->uBytesSubmit)) );
    ...
    /* Validate the channel state change transitions */
    {
      ...
      do
      {
        ...
        {
          SB_LOG_ERR_1("Failed to remove channel, already removed (res: %ld)",
      ...
      SB_MSG_SET(eCtrl,USR_CHANNEL_CONTROL_0,CTRL);
    {
      ...
      do 
      {
        if (eCtrl == SLIMBUS_CHAN_CTRL_REMOVE)
        {
          ...
          pMasterPort = NULL;
          while (NULL != (pMasterPort = NextPortNode(&pTempChannel->MasterPortList, pMasterPort)))
          {
            if (pMasterPort->eDisconnectState != SB_PORT_DISCONNECTING)
            {
              pDevCtxt->uPortDisconnectMask |= (1<<pMasterPort->portNum);
            }
          }
    ...
    else
    {
      ...
      SB_LOG_ERR_2("Master nacked data channel control request, rolling back "
                   "configuration (client: 0x%p) (eCtrl: %d)", pClientCtxt, eCtrl);
  ...
  if (SB_SUCCESS != result)
  {
    SB_LOG_ERR_2("Error during data channel control request "
                 "(client: 0x%p) (eCtrl: %d)", pClientCtxt, eCtrl);
  }
      ...
      pChannel->pClientCtxt == pClientCtxt)
  {
         ...
         SLIMBUS_CHANNEL_STATUS_REMOVED == pChannel->eCurStatus )
    {
      SB_LOG_INFO_2("Finishing channel dealloc now that channel removal is complete "
                    "(client: 0x%p) (chan: %ld)", pClientCtxt, pChannel->channelNum );
...
SlimBus_DoReconfigureNowInternal(SlimBusClientCtxt *pClientCtxt) 
{
  ...
  do
  {
    ...
    for (uIdx = 0; uIdx < pDevCtxt->numPorts; uIdx++)
    {
      ...
      if ( (uMask & pDevCtxt->uPortDisconnectMask) &&
           (pPort->pClientCtxt == pClientCtxt) &&
           !(uMask & HAL_sb_PgdPortGetIntStatusEe(pDevCtxt->hSB, 0, pDevCtxt->pBSP->uMyEE)) )
      {
        ...
        SlimBus_EventWait(pClientCtxt->hClientEvent);
  ...
  if (SB_SUCCESS != result)
  {
    SB_LOG_ERR_1("Error during reconfigure now request (client: 0x%p)", pClientCtxt);
  }
...
SlimBus_DoReconfigureNow(SlimBusHandle h) 
{
  ...
  SB_LOG_VERB_1("Got reconfigure now request (client: 0x%p)", pClientCtxt);
...
SlimBus_DeallocDataChannelInternal(SlimBusClientCtxt *pClientCtxt, SlimBusDevCtxt *pDevCtxt, SlimBusChannelType *pChannel) 
{
  ...
  do
  {
    ...
    if ( pChannel->eCurStatus != SLIMBUS_CHANNEL_STATUS_REMOVED )
    {
      ...
      SlimBus_NextDataChannelControlInternal(pClientCtxt, pChannel, NULL,
  ...
  if (SB_SUCCESS != result)
  {
    SB_LOG_ERR_2("Failed to deallocate data channel (res: 0x%lx) (chan: %ld)",
                 pChannel->handle, pChannel->channelNum );
  }
...
SlimBus_AllocProgressCounter(SlimBusHandle h, SlimBusResourceHandle  hPort, SlimBusBamTransferType  eTransferDir, SlimBusResourceHandle * phCounter) 
{
  ...
  do
  {
    ...
    {
      for (i = 0; i < (int)pDevCtxt->numCounterSlots; i++)
      {
        pCounter = &pDevCtxt->Counters[i];
        if (pCounter->eState == SB_HANDLE_AVAILABLE)
        {
...
SlimBus_AllocResourceGroup(SlimBusHandle h, SlimBusResourceHandle * phGroup) 
{
  ...
  for (i = 0; i < (int)pDevCtxt->numGroupSlots; i++)
  {
...
uint32 SlimBus_DeviceDetach(SlimBusHandle h)
{
  ...
  SB_LOG_INFO_2("Client detaching from device (client: 0x%p) (device: 0x%p)",
                pClientCtxt, pDevCtxt);
  ...
  if ( !pDevCtxt->bPendingSSR )
  {
    ...
    {
      SB_LOG_VERB_1("Generating reconfigure now request (client: 0x%p)",
                    pClientCtxt);
      SlimBus_DoReconfigureNowInternal(pClientCtxt);
    }
  }
  ...
  for (uIdx = 0; uIdx < pDevCtxt->numChannelSlots; uIdx++)
  {
    ...
    if ( NULL != pChannel &&
         ...
         SB_HANDLE_AVAILABLE != pChannel->eState )
    {
