...
$Header: //components/rel/core.adsp/2.6.1/buses/slimbus/src/common/SlimBusMaster.c#1 $
  ...
  else if ( SB_MSG_MT_DEST_REFERRED_USER == eMT )
  {
         ...
         SB_QC_MASTER_PROTOCOL_V1 == pDevice->pSat->uSatProtoVer )
    {
                ...
                SB_MSG_MC_USR_REQ_CONNECT_SINK == eUsrMC )
      {
        ...
        SB_LOG_VERB_5("Got satellite connect port request "
                      "(satLA: 0x%x) (channel: %d) (destLA: 0x%x) (port: %d) "
                      "(MC: 0x%x)", LA, chanNum, destLA, portNum, eMC);
...
)
{
         ...
         pParam->uNewRefCnt != pParam->uCurRefCnt ) )
  {
    ...
    *(pTxBuf++) = ( SB_MSG_SET((usSegDist >> 
                                 (32-SB_MSG_MC_NEXT_DEFINE_CHANNEL_0__SD_LSB_SHFT)),
                                NEXT_DEFINE_CHANNEL_1,SD_MSB) |
...
static void PrepareRemoveChannelParam(SlimBusDevCtxt *pDevCtxt, SlimBusChannelParamType *pParam, SBBOOL *pbRemove)
{
  if ( pParam->uNewRefCnt > 0 )
  {
    ...
    else
    {
      ...
      pDevCtxt->pSched->uSlotsUsed -= uSlots;
    ...
    *pTxBuf = SB_MSG_ASM_FIRST_WORD(SB_MSG_MC_BEGIN_RECONFIGURATION_LEN,
    ...
    do
    {
      if ( pDevCtxt->pSched->bPurge )
      {
        ...
        /* Send each message individually to prevent exhausting the 
      ...
      else if ( NULL != pBasicClient )
      {
        ...
        while ( NULL != (pCfgBuf = SlimBus_PopBuffer(pCfgMsgFifo, NULL)) )
        {
               ...
               CfgMC == SB_MSG_MC_USR_CHANNEL_DEF_ACTIVATE )
          {
            ...
            {
              SlimBusChannelParamType *pChanParam = NULL;
              uint8 ucChanNum = *(pucBuf++);
            
              pChanParam = pDevCtxt->pSched->apChanParams[ucChanNum];
              if ( NULL == pChanParam )
              {
                SlimBus_Malloc(sizeof(SlimBusChannelParamType), (void**)(&pChanParam), SB_MEMTAG_14);
                memset(pChanParam, 0, sizeof(*pChanParam));
                pDevCtxt->pSched->apChanParams[ucChanNum] = pChanParam;
                pChanParam->channelNum = ucChanNum;
      ...
      else if ( SB_FRM_HANDOVER_IDLE != pDevCtxt->pSched->eFrmHandState )
      {
        ...
        else if ( SB_FRM_HANDOVER_EXECUTE == pDevCtxt->pSched->eFrmHandState )
        {
          ...
          {
            LAIF = HAL_sb_FrmGetAssignedLA(pDevCtxt->hSB);
          }
         ...
         !bSkipSched )
    {
      ...
      {
        while ( pDevCtxt->uFrmClkGear > uClkGear &&
                NULL != (pParam = NextParamNode(pList, pParam)) )
        {
          if ( pParam->eStatus != SB_CHANNEL_PENDING_REMOVAL && 
      ...
      do
      {
        ...
        for ( ; pList != NULL; pList = pNextList, pNextList = NULL )
        {
          while ( NULL != (pParam = NextParamNode(pList, pParam)) )
          {
            ...
            if (SB_SUCCESS != SendUpdatedChannelDef(pDevCtxt, pParam, uClkGear))
            {
              result = SB_ERROR;
            }
          }
        ...
        *pTxBuf = SB_MSG_ASM_FIRST_WORD(SB_MSG_MC_RECONFIGURE_NOW_LEN,
        ...
        {
          pDevCtxt->pSched->uSchedTimerCnt = 1;
         ...
         !bAllowFailure )
    {
      ...
      for ( ; pList != NULL; pList = pNextList, pNextList = NULL )
      {
        ...
        while ( NULL != pParam )
        {
          SlimBusChannelParamType *pNextParam = NextParamNode(pList, pParam);
          ...
          else
          {
            ...
            SB_LOG_VERB_4("Active channel parameters (channel#: %ld) "
                          "(interval: %ld) (offset: %ld) (refcnt: %ld)",
                          pParam->channelNum, pParam->uCurInterval,
                          pParam->uCurOffset, pParam->uCurRefCnt);
          }
          pParam = pNextParam;
        }
      }
      ...
      SB_LOG_VERB_4("Bandwidth utilization stats (used data+msg slots: %ld+%ld / 1536) "
                    "(CG: %ld) (SM: %ld)", pDevCtxt->pSched->uSlotsUsed,
...
)
{
  ...
  result = SlimBus_EventWait(pClientCtxt->hClientEvent);
  ...
  for (uIdx = 0; uIdx < pDevCtxt->numDeviceSlots; uIdx++)
  {
         ...
         pDevice->eState == SB_ENUM_PRESENT )
    {
            ...
            uAckRetryCnt++ )
      {
        ...
        {
          SB_LOG_ERR_0("Got nack for assign logical address message");
        }
      }
...
static void SlimBus_CheckFramerHandoverSat(SlimBusDevCtxt *pDevCtxt)
{
  ...
  if (SLIMBUS_ERR_NONE != SlimBus_MasterSendAndWaitForMsgEvent(pDevCtxt, pTxBuf))
  {
    ...
    for (uClientNum = 0; uClientNum < pNgdDevice->pSat->numClientSlots; uClientNum++)
    {
      ...
      {
        RemoveClientNode(pSatClientCtxt);                            
...
)
{
  if (pIsrCtxt->uMsgIntStatus & HAL_SB_MGR_INT_RECONFIG_DONE)
  {
    ...
    {
      SB_LOG_INFO_0("Framer restarted sponateously when entering idle pause clock");
...
void SlimBus_SSReset(SlimBusDevCtxt *pDevCtxt)
{
  ...
  SB_LOG_WARN_0("Resettting hardware and clearing state due to pending SSR");
