...
$Header: //components/rel/core.adsp/2.6.1/buses/slimbus/src/common/SlimBusSat.c#1 $
        ...
        if (SB_MSG_MC_USR_LA_REPLY == eUsrMC )
        {
               ...
               (pDataBuf[1] & 0xffff) != 0 )
          {
                 ...
                 memcmp(pDevice->aEA, puBuf, SLIMBUS_ENUM_ADDR_BUF_SIZE) == 0 )
            {
              ...
              if (NULL != pEvent->pBuf)
              {
                *(uint32*)pEvent->pBuf = LA | SB_ACK_SUCCESS_FLAG;
              }
            }
            else
            {
              if (NULL != pEvent->pBuf)
              {
                *(uint32*)pEvent->pBuf = SB_ACK_FAILURE_FLAG;
              }
            }
          }
          else
...
)
{
  ...
  if (pDevCtxt->eDrvType == SB_SATELLITE_DRIVER)
  {
    ...
    if (NULL != pEventNode->TxMarker.pBuf)
    {
      uMsgHeader = *pEventNode->TxMarker.pBuf;
    }
    
    pEventNode->uRxMarker = 
      SB_BF_SET(SB_MSG_LA_MANAGER,RXMARKER,LA) |
      SB_BF_SET(TID,RXMARKER,TID);
    pEventNode->pBuf = (uint8*)&uMarker;
    ...
    if ( SB_SUCCESS == result )
    {
      if ( uMarker == 0 )
      {
        ...
        SB_LOG_ERR_2("master ack timeout (client: 0x%p) (msg header: 0x%lx)", 
                     pClientCtxt, uMsgHeader);
      }
      else 
      {
