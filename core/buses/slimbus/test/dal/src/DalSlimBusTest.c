...
$Header: //components/rel/core.adsp/2.6.1/buses/slimbus/test/dal/src/DalSlimBusTest.c#1 $
  ...
  while (TRUE)
  {
           ...
           NULL != IOVecRx.uAddr)
    {
      ...
      if (DAL_SUCCESS == result &&
          NULL != IOVecTx.uAddr)
      {
        SB_ASSERT(memcmp((void*)(IOVecTx.uAddr+pCbData->uPhysOffset),
...
void* SlimBusPortCb( void *pBuf1, uint32 uSize1, void *pBuf2, uint32 uSize2 )
{
...
void* SlimBusSimplePortCb( void *pBuf1, uint32 uSize1, void *pBuf2, uint32 uSize2 )
{
  CallbackDataType *pCbData = (CallbackDataType*)pBuf1;
  SlimBusEventNotifyType Notify;
  DALResult result;
...
//  uint32 datain2 = 0;
  uint32 datalen = 0;
  uint8 aIntEA[] = { 0x00, 0x00, 0xa0, 0x00, 0x17, 0x02 };
  uint8 aGenEA[] = { 0x00, 0x01, 0xa0, 0x00, 0x17, 0x02 };
  SlimBusLogicalAddrType genLA = 0, intLA = 0;
  DalDeviceHandle *hSB;
  DALSYSEventHandle hEvent, hBamEvent, hPortEvent, hSimplePortEvent;
  uint32 uIdx;
...
//  int i;
  ...
  for ( uIdx = 0, result = DAL_ERROR; uIdx < 1000000 && result != DAL_SUCCESS; uIdx++)
  {
    result = DalSlimBus_GetDeviceLogicalAddr( hSB, aIntEA, sizeof(aIntEA), &intLA);
    SBTEST_SLEEP();
  }
  SB_ASSERT(result == DAL_SUCCESS);
  for ( uIdx = 0, result = DAL_ERROR; uIdx < 1000000 && result != DAL_SUCCESS; uIdx++)
  {
    result = DalSlimBus_GetDeviceLogicalAddr( hSB, aGenEA, sizeof(aGenEA), &genLA);
    SBTEST_SLEEP();
  }
  SB_ASSERT(result == DAL_SUCCESS);
...
//    else if (g_sb_test_case == 2)
    {
      ...
      SB_ASSERT(result == DAL_SUCCESS);
      result = DalSlimBus_NextDataChannelControl(hSB, hChannel[7], SLIMBUS_CHAN_CTRL_REMOVE);
      SB_ASSERT(result == DAL_SUCCESS);
