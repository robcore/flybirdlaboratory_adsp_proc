...
/*===========================================================================
  ...
  $Header: //components/rel/core.adsp/2.6.1/buses/uart/mdd/src/siors232_mdd.c#1 $ $DateTime: 2014/10/16 12:45:40 $ $Author: pwbldsvc $
...
do_transmit( sio_mdd_port_type* mdd_port )
{
  ...
  while( txd->sio_bytes_to_tx > 0 )
  {
    //  Loop until either: the packet is empty, the FIFO is full,
    //  or we've transmitted the maximum number of bytes.
    ...
    //  Couldn't tx all bytes means FIFO is full.

    is_fifo_full = ( txd->sio_bytes_to_tx > 0 );

    //  Get next DSM item, if needed.  Also free current DSM item
    //  immediately -- don't hold on to it.  (No-op if there's
    //  still data in current DSM item.)

    get_more_data_for_tx( mdd_port );

    //  If FIFO is full, we're done.
