...
$Header: //components/rel/core.adsp/2.6.1/buses/uart/dal/src/DalUart.c#1 $
...
static void          deregister_rx_falling_isr(UART_CONTEXT *uart_ctxt);
static void          deregister_rx_rising_isr(UART_CONTEXT *uart_ctxt);
static void          deregister_uart_isr     (UART_CONTEXT *uart_ctxt);
...
static DALResult     register_rx_falling_isr (UART_CONTEXT *uart_ctxt);
static DALResult     register_rx_rising_isr  (UART_CONTEXT *uart_ctxt);
static boolean       register_uart_isr       (UART_CONTEXT *uart_ctxt);
static boolean       resume_from_sleep       (UART_CONTEXT *uart_ctxt);
static void          rx_break_change_intr    (UART_CONTEXT *uart_ctxt);
static void          rx_dma_callback         (void *context);
static void          rx_dma_intr             (UART_CONTEXT *uart_ctxt);
static void *        rx_falling_isr          (void *context);
static void          rx_fifo_copy            (UART_CONTEXT *uart_ctxt, uint32 num_bytes);
static void *        rx_rising_isr           (void *context);
static void          rx_stale_intr           (UART_CONTEXT *uart_ctxt);
...
static void          rx_wm_intr              (UART_CONTEXT *uart_ctxt);
...
static boolean       transmit_chunk          (UART_CONTEXT *uart_ctxt, boolean first_chunk);
static void          tx_dma_callback         (void *context);
static void          tx_dma_intr             (UART_CONTEXT *uart_ctxt);
static void          tx_ready_intr           (UART_CONTEXT *uart_ctxt);
static void          tx_wm_intr              (UART_CONTEXT *uart_ctxt);
...
static boolean create_objects(UART_CONTEXT *uart_ctxt, UartInitConfig *init_cfg)
{
   ...
   result = DALSYS_EventCreate(DALSYS_EVENT_ATTR_CLIENT_DEFAULT, &uart_ctxt->rx_idle_event, NULL);
   if (result != DAL_SUCCESS) { goto error; }

   result = DALSYS_EventCreate(DALSYS_EVENT_ATTR_CLIENT_DEFAULT, &uart_ctxt->tx_idle_event, NULL);
   if (result != DAL_SUCCESS) { goto error; }
...
FUNCTION: deregister_rx_falling_isr
...
static void deregister_rx_falling_isr(UART_CONTEXT *uart_ctxt)
{
   if (uart_ctxt->rx_falling_isr_registered)
   {
      tal_interrupt_unregister(uart_ctxt->interrupt_handle, &uart_ctxt->rx_irq, rx_falling_isr);
      uart_ctxt->rx_falling_isr_registered = FALSE;
   }
}

/*==================================================================================================

FUNCTION: deregister_rx_rising_isr
...
static void deregister_rx_rising_isr(UART_CONTEXT *uart_ctxt)
{
   if (uart_ctxt->rx_rising_isr_registered)
   {
      tal_interrupt_unregister(uart_ctxt->interrupt_handle, &uart_ctxt->rx_irq, rx_rising_isr);
      uart_ctxt->rx_rising_isr_registered = FALSE;
   }
}

/*==================================================================================================

FUNCTION: deregister_uart_isr
...
static void deregister_uart_isr(UART_CONTEXT *uart_ctxt)
{
   ...
   {
      tal_interrupt_unregister(uart_ctxt->interrupt_handle, &uart_ctxt->uart_irq, uart_isr);
...
static void destroy_objects(UART_CONTEXT *uart_ctxt)
{
   ...
   if (uart_ctxt->rx_idle_event) { DALSYS_DestroyObject(uart_ctxt->rx_idle_event); }
   if (uart_ctxt->tx_idle_event) { DALSYS_DestroyObject(uart_ctxt->tx_idle_event); }
...
static boolean get_properties(UART_CONTEXT *uart_ctxt)
{
   ...
   get_property_dword(uart_ctxt, handle, "UartBase",    &uart_ctxt->uart_physical,      0);
   get_property_dword(uart_ctxt, handle, "GsbiBase",    &uart_ctxt->gsbi_physical,      0);
   get_property_dword(uart_ctxt, handle, "GpioRfrN",    &uart_ctxt->gpio_rfrn,          0);
   get_property_dword(uart_ctxt, handle, "GpioCtsN",    &uart_ctxt->gpio_ctsn,          0);
   get_property_dword(uart_ctxt, handle, "GpioRxData",  &uart_ctxt->gpio_rx_data,       0);
   get_property_dword(uart_ctxt, handle, "GpioTxData",  &uart_ctxt->gpio_tx_data,       0);
   get_property_dword(uart_ctxt, handle, "GsbiControl", &uart_ctxt->gsbi_control,       0);
   get_property_dword(uart_ctxt, handle, "DetectBrk",   &uart_ctxt->detect_break,       FALSE);
   get_property_dword(uart_ctxt, handle, "IsCxm",       &uart_ctxt->is_cxm,             FALSE);
   get_property_dword(uart_ctxt, handle, "Irq",         &uart_ctxt->uart_irq.u.irq_num, 0);

   if (uart_ctxt->uart_physical == 0) { goto error; }
   if (uart_ctxt->gsbi_physical == 0 && uart_ctxt->gsbi_control) { goto error; }
...
{
   DALDEVICEID id = uart_ctxt->dal_id;
   uint32 gpio_configs[4];
   uint32 *ptr = gpio_configs;
   uint32 num_gpios = 0;
   TAL_RESULT result;

   if (uart_ctxt->gpio_rfrn)    { *ptr++ = uart_ctxt->gpio_rfrn;    num_gpios++; }
   if (uart_ctxt->gpio_ctsn)    { *ptr++ = uart_ctxt->gpio_ctsn;    num_gpios++; }
   if (uart_ctxt->gpio_rx_data) { *ptr++ = uart_ctxt->gpio_rx_data; num_gpios++; }
   if (uart_ctxt->gpio_tx_data) { *ptr++ = uart_ctxt->gpio_tx_data; num_gpios++; }

   result = tal_tlmm_gpio_disable(uart_ctxt->tlmm_handle, gpio_configs, num_gpios);
...
static boolean gpio_enable(UART_CONTEXT *uart_ctxt)
{
   ...
   result = tal_tlmm_gpio_enable(uart_ctxt->tlmm_handle, gpio_configs, num_gpios);
...
static void halt_rx_path(UART_CONTEXT *uart_ctxt)
{
   ...
   DALSYS_EventCtrl(uart_ctxt->rx_idle_event, DALSYS_EVENT_CTRL_RESET);
   tal_interrupt_trigger(uart_ctxt->interrupt_handle, &uart_ctxt->uart_irq);
   result = fwk_wait_event(uart_ctxt->rx_idle_event, 500);

   if (result != DAL_SUCCESS)
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_ERROR, "halt_rx_path: RX idle event not detected");
   }
...
static void halt_tx_path(UART_CONTEXT *uart_ctxt)
{
   ...
   DALSYS_EventCtrl(uart_ctxt->tx_idle_event, DALSYS_EVENT_CTRL_RESET);
   tal_interrupt_trigger(uart_ctxt->interrupt_handle, &uart_ctxt->uart_irq);
   result = fwk_wait_event(uart_ctxt->tx_idle_event, 500);

   if (result != DAL_SUCCESS)
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_ERROR, "halt_tx_path: TX idle event not detected");
   }
...
static boolean map_hardware(UART_CONTEXT *uart_ctxt)
{
   ...
   result = DALSYS_MemRegionAlloc(DALSYS_MEM_PROPS_HWIO,
                                  DALSYS_MEM_ADDR_NOT_SPECIFIED, uart_ctxt->uart_physical,
   ...
   {
      result = DALSYS_MemRegionAlloc(DALSYS_MEM_PROPS_HWIO,
                                     DALSYS_MEM_ADDR_NOT_SPECIFIED, uart_ctxt->gsbi_physical,
...
{
   DALSYS_SyncEnter(uart_ctxt->rx_sync);
   deregister_rx_falling_isr(uart_ctxt);
   deregister_rx_rising_isr(uart_ctxt);
   DALSYS_SyncLeave(uart_ctxt->rx_sync);
...
static UART_CONTEXT *mu_init(UartInitConfig *init_cfg, DALDEVICEID device)
{
   ...
   // get_properties() may have filled in uart_irq, but those values can be
   // overridden by non-zero values in init_cfg
   if (init_cfg->uart_irq)  // treat an IRQ of 0 as unspecified
   {
      uart_ctxt->uart_irq.type      = TAL_INTERRUPT_TYPE_IRQ;
      uart_ctxt->uart_irq.u.irq_num = init_cfg->uart_irq;
   }
   else if (uart_ctxt->uart_irq.u.irq_num)
   {
      uart_ctxt->uart_irq.type      = TAL_INTERRUPT_TYPE_IRQ;
   }
   else
   {
      uart_ctxt->uart_irq.type      = TAL_INTERRUPT_TYPE_INVALID;
      DALSYS_LogEvent(device, DALSYS_LOGEVENT_ERROR, "mu_init: No UART IRQ!");
      goto error;
   }

   if (init_cfg->rx_irq)  // treat an IRQ of 0 as unspecified
   {
      uart_ctxt->rx_irq.type       = TAL_INTERRUPT_TYPE_IRQ;
      uart_ctxt->rx_irq.u.irq_num  = init_cfg->rx_irq;
   }
   else if (uart_ctxt->gpio_rx_data)
   {
      uart_ctxt->rx_irq.type       = TAL_INTERRUPT_TYPE_GPIO;
      uart_ctxt->rx_irq.u.gpio_num = TAL_GPIO_NUMBER(uart_ctxt->gpio_rx_data);
   }
   else
   {
      uart_ctxt->rx_irq.type       = TAL_INTERRUPT_TYPE_INVALID;
   }

   uart_hal = &uart_ctxt->uart_hal;
   DALSYS_MemInfo(uart_ctxt->uart_mem_handle, &meminfo);
   HALuart_InitObject(uart_hal, uart_ctxt->uart_type, meminfo.VirtualAddr, meminfo.PhysicalAddr);
...
static boolean mu_open(UART_CONTEXT *uart_ctxt, UartOpenConfig *open_cfg)
{
   ...
   if ( !register_uart_isr(uart_ctxt)  ) { goto error; }
...
static DALResult power_025(UART_CONTEXT *uart_ctxt)
{
   ...
   switch (uart_ctxt->power_state)  // check current state
   {
      case POWER_OFF:  // POWER_OFF -> POWER_025
         ...
         // We must change the power state while inside this critical section because
         // register_rx_falling_isr() might defer registration and in that case we
         // need to make sure the registration happens in rx_rising_isr().

         uart_ctxt->power_state = POWER_025;
         result = register_rx_falling_isr(uart_ctxt);

         DALSYS_SyncLeave(uart_ctxt->rx_sync);

         if (result != DAL_SUCCESS) { break; }
...
static DALResult power_off(UART_CONTEXT *uart_ctxt)
{
   ...
   switch (uart_ctxt->power_state)  // check current state
   {
      ...
      case POWER_025:  // POWER_025 -> POWER_OFF
         ...
         // We must change the power state while inside this critical section to ensure
         // that we don't re-enable the falling edge ISR during rx_rising_isr().
...
static void receive_chunk(UART_CONTEXT *uart_ctxt)
{
   ...
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "receive_chunk: RX path is halted");
   }
   else
   {
...
static boolean reg_init(UART_CONTEXT *uart_ctxt, UartOpenConfig *open_cfg)
{
   ...
   if ( !set_baud_rate(uart_ctxt, open_cfg->baud_rate) )
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_ERROR, "reg_init: failed to set baud rate");
      return FALSE;
   }
...
FUNCTION: register_rx_falling_isr

DESCRIPTION:
   Registers an interrupt that triggers on the falling edge of the RX line.  This is used for
   RX wakeup (when the remote UART sends a character the start bit will be the falling edge).
...
static DALResult register_rx_falling_isr(UART_CONTEXT *uart_ctxt)
{
   ...
   if (uart_ctxt->rx_irq.type == TAL_INTERRUPT_TYPE_INVALID)
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_ERROR, "register_rx_falling_isr: unspecified RX IRQ");
      result = TAL_ERROR;
   }
   else
   {
      if (uart_ctxt->rx_rising_isr_registered)
      {
         // If the UART is in the break state, we've already registered the rising edge
         // of the RX line to detect the end of the break condition, so we need to wait
         // for the UART to exit the break state before registering the RX falling ISR.
      ...
      else
      {
         ...
         result = tal_interrupt_register(uart_ctxt->interrupt_handle, &uart_ctxt->rx_irq,
                                         TAL_INTERRUPT_TRIGGER_FALLING, rx_falling_isr, uart_ctxt);

         if (result == TAL_SUCCESS)
         {
            uart_ctxt->rx_falling_isr_registered = TRUE;
         }
         else
         {
            DALSYS_LogEvent(id, DALSYS_LOGEVENT_ERROR, "register_rx_falling_isr: failed");
         }
      }
   }

   return (result == TAL_SUCCESS) ? (DAL_SUCCESS) : (DAL_ERROR);
}

/*==================================================================================================

FUNCTION: register_rx_rising_isr

DESCRIPTION:
   Registers an interrupt that triggers on the rising edge of the RX line.  This is used to
   detect the end of break.
...
static DALResult register_rx_rising_isr(UART_CONTEXT *uart_ctxt)
{
   ...
   result = tal_interrupt_register(uart_ctxt->interrupt_handle, &uart_ctxt->rx_irq,
                                   TAL_INTERRUPT_TRIGGER_RISING, rx_rising_isr, uart_ctxt);

   if (result == TAL_SUCCESS)
   {
      uart_ctxt->rx_rising_isr_registered = TRUE;
   }
   else
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_ERROR, "register_rx_rising_isr: failed");
   }
...
static boolean register_uart_isr(UART_CONTEXT *uart_ctxt)
{
   ...
   result = tal_interrupt_register(uart_ctxt->interrupt_handle, &uart_ctxt->uart_irq,
                                   TAL_INTERRUPT_TRIGGER_HIGH, uart_isr, uart_ctxt);
   ...
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_ERROR, "register_uart_isr: failed");
   }
...
static boolean resume_from_sleep(UART_CONTEXT *uart_ctxt)
{
   ...
   // restore incoming flow control to user setting
   set_rts_control(uart_ctxt, uart_ctxt->rts_control);
...
FUNCTION: rx_break_change_intr

DESCRIPTION:
   This function handles the start of an RX break condition.
...
static void rx_break_change_intr(UART_CONTEXT *uart_ctxt)
{
   ...
   DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "+rx_break_change_intr");

   // This interrupt is only used to detect break start.  We can't use it to detect
   // break end because the client may power down the UART in response to break start.
   // We must therefore use an RX rising edge interrupt to detect break end.
   //
   // Note that current versions of UARTDM have dedicated RXBREAK_START and RXBREAK_END
   // interrupts.  We are using RXBREAK_CHANGE because those interrupts are not supported
   // on legacy UARTs.  [UPDATE: We can switch to RXBREAK_START/RXBREAK_END because
   // we no longer support legacy UARTs.]

   DALSYS_SyncEnter(uart_ctxt->register_sync);
   HALuart_DisableInterrupts(uart_hal, HAL_UART_RXBREAK_CHANGE_IMR_FLAG);
   DALSYS_SyncLeave(uart_ctxt->register_sync);
...
DESCRIPTION:
   This function gets called by the DMOV driver after an RX DMA interrupt.  We should do
   very little here since we are executing in the DMOV IST.  In the past we have experienced
   deadlocks due to interactions between our critical sections and DMOV's critical sections.
   Therefore we will just signal the UART IST and process the interrupt there.
...
static void rx_dma_callback(void *context)
{
   ...
   // simulate a UART HW interrupt to indicate there was an RX DMA event
   uart_ctxt->rx_dma_flag = TRUE;
   tal_interrupt_trigger(uart_ctxt->interrupt_handle, &uart_ctxt->uart_irq);
...
static void rx_dma_intr(UART_CONTEXT *uart_ctxt)
{
   ...
   DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "+rx_dma_intr");

   interrupt_type = dma_rx_intr_type(uart_ctxt->dma_ctxt);
   uart_ctxt->rx_dma_flag = FALSE;
...
FUNCTION: rx_falling_isr

DESCRIPTION:
   The DAL Interrupt Controller calls this function in the context of an interrupt
   service thread (IST) when a falling edge is detected on the RX line.  This is
...
static void *rx_falling_isr(void *context)
{
   ...
   DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "+rx_falling_isr");
...
FUNCTION: rx_rising_isr

DESCRIPTION:
   The DAL Interrupt Controller calls this function in the context of an interrupt
   service thread (IST) when a rising edge is detected on the RX line.  This is
...
static void *rx_rising_isr(void *context)
{
   ...
   DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "+rx_rising_isr");
   ...
   DALSYS_SyncEnter(uart_ctxt->rx_sync);
   if (uart_ctxt->power_state == POWER_025)  // (inband-sleep / wake-on-rx)
   {
      // If we are here then we must have entered POWER_025 during a break state.
      // At that time we deferred registering the falling edge ISR and we must
      // register it now.

      register_rx_falling_isr(uart_ctxt);
   }
   DALSYS_SyncLeave(uart_ctxt->rx_sync);
...
static void rx_stale_intr(UART_CONTEXT *uart_ctxt)
{
   ...
   DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "+rx_stale_intr");
   ...
   DALSYS_SyncEnter(uart_ctxt->rx_sync);
      ...
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "rx_stale_intr: SNAP = 0x%08X", snapshot);
   ...
   {
      rx_head = &uart_ctxt->rx_buf[uart_ctxt->rx_head];
      rx_fifo_copy(uart_ctxt, snapshot - rx_head->write_offset);
      rx_transfer_complete(uart_ctxt, &rx_data, &rx_error);
   }
...
static boolean rx_transfer_start(UART_CONTEXT *uart_ctxt)
{
   ...
   if (uart_ctxt->rx_halt)
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "rx_transfer_start: RX path is halted");
      goto exit;
   }
   ...
   if (uart_ctxt->rx_out_of_buffers)
   {
      ...
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "rx_transfer_start: ran out of buffers");
      goto exit;
   }
   ...
   if (uart_ctxt->use_dma)
   {
      ...
      rx_head->descriptor.physical_address  = rx_data_info.PhysicalAddr;
...
static void rx_wm_intr(UART_CONTEXT *uart_ctxt)
{
   ...
   DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "+rx_wm_intr");
   ...
   rx_fifo_copy(uart_ctxt, bytes_to_copy);
   buffer_room = RX_BUF_SIZE - rx_head->write_offset;
...
DESCRIPTION:
   This function is called to activate break detection.  It performs three discrete tasks:

   (1) check the current break state
   (2) report any previous break state changes that were missed
   (3) enable the appropriate interrupt to detect the next break state change

==================================================================================================*/
static void start_break_detection(UART_CONTEXT *uart_ctxt)
{
   HALUartObject *uart_hal = &uart_ctxt->uart_hal;
   boolean rx_halt = TRUE;
   uint32 break_active = 0;  // current status of RX break (0 = deasserted, 1 = asserted)

   if ( !uart_ctxt->detect_break ) { return; }

   // Check the RX path to see if it is halted.  If it is then the clock might be off (during
   // sleep or close) or we might be in the process of deregistering interrupts (during close).
   // Therefore we can't resume break detection until the RX path has resumed.

   DALSYS_SyncEnter(uart_ctxt->rx_sync);

   if (uart_ctxt->rx_halt == FALSE)
   {
      rx_halt = FALSE;
      break_active = HALuart_GetStatus(uart_hal) & HAL_UART_STATUS_RX_BREAK_START;
   }
   ...
   if (break_active && !uart_ctxt->break_notified)
   {
      notify_events(uart_ctxt, EVENT_BREAK);
      uart_ctxt->break_notified = TRUE;
   }
   else if (!break_active && uart_ctxt->break_notified)
   {
      notify_events(uart_ctxt, EVENT_BREAK_END);
      uart_ctxt->break_notified = FALSE;
   }

   // Check rx_halt again to make sure we didn't halt the RX path between critical sections.
   ...
   if (uart_ctxt->rx_halt == FALSE)
   {
      if (break_active)
      {
         register_rx_rising_isr(uart_ctxt);
      }
      else  // break not active
      {
         HALuart_ResetRxBreakChangeInterrupt(&uart_ctxt->uart_hal);

         DALSYS_SyncEnter(uart_ctxt->register_sync);
         HALuart_EnableInterrupts(&uart_ctxt->uart_hal, HAL_UART_RXBREAK_CHANGE_IMR_FLAG);
         DALSYS_SyncLeave(uart_ctxt->register_sync);
      }
   }
...
static boolean transmit_chunk(UART_CONTEXT *uart_ctxt, boolean first_chunk)
{
   ...
   if (uart_ctxt->tx_halt)
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "transmit_chunk: TX path is halted");
   }
   else
   {
      if (first_chunk) { HALuart_SetDmaNumTxChars(uart_hal, bytes_left); }
...
DESCRIPTION:
   This function gets called by the DMOV driver after a TX DMA interrupt.  We should do
   very little here since we are executing in the DMOV IST.  In the past we have experienced
   deadlocks due to interactions between our critical sections and DMOV's critical sections.
   Therefore we will just signal the UART IST and process the interrupt there.
...
static void tx_dma_callback(void *context)
{
   ...
   // simulate a UART HW interrupt to indicate there was a TX DMA event
   uart_ctxt->tx_dma_flag = TRUE;
   tal_interrupt_trigger(uart_ctxt->interrupt_handle, &uart_ctxt->uart_irq);
...
static void tx_dma_intr(UART_CONTEXT *uart_ctxt)
{
   ...
   DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "+tx_dma_intr");

   uart_ctxt->tx_dma_flag = FALSE;
   ...
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_ERROR, "tx_dma_intr: error detected");
   }

   if (uart_ctxt->tx_halt)
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "tx_dma_intr: TX path is halted");
   }
   else
   {
...
static void tx_ready_intr(UART_CONTEXT *uart_ctxt)
{
   ...
   DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "+tx_ready_intr");
...
static void tx_wm_intr(UART_CONTEXT *uart_ctxt)
{
   ...
   DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "+tx_wm_intr");
   ...
   DALSYS_SyncEnter(uart_ctxt->tx_sync);
   transmit_chunk(uart_ctxt, FALSE);
   DALSYS_SyncLeave(uart_ctxt->tx_sync);
...
DESCRIPTION:
   The DAL Interrupt Controller calls this function in the context of an interrupt
   service thread (IST) when a UART HW interrupt occurs or when a DMA event is
   signaled by the DalUartDma module.  If a new interrupt occurs while processing
   the current interrupt(s) then we expect the DAL Interrupt Controller to call this
   function again after we exit.  This prevents race conditions but it also creates
   the possibility that this function gets called with nothing to do.
...
static void *uart_isr(void *context)
{
   ...
   HALUartObject *uart_hal = &uart_ctxt->uart_hal;
   DALDEVICEID id = uart_ctxt->dal_id;
   uint32 isr, misr, sr, rx_dma_flag, tx_dma_flag;
   ...
   rx_dma_flag = uart_ctxt->rx_dma_flag;
   tx_dma_flag = uart_ctxt->tx_dma_flag;

   DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO,
      "uart_isr: isr=0x%04X, misr=0x%04X, sr=0x%04X, rx_dma_flag = %d, tx_dma_flag = %d",
      isr, misr, sr, rx_dma_flag, tx_dma_flag);

   if ( (misr & HAL_UART_RXLEV_IMR_FLAG) && (misr & HAL_UART_RXSTALE_IMR_FLAG) )
   {
      // Simultaneous RXLEV/RXSTALE interrupts can only occur in non-DMA mode.
      ...
      misr &= ~HAL_UART_RXLEV_IMR_FLAG;
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO,
                      "uart_isr: RXLEV/RXSTALE both set - ignoring RXLEV");
   }

   // RXSTALE must be processed before rx_dma_flag.  That is because there is a corner
   // condition when a stale event occurs after receiving between 1 and 15 bytes less
   // than the RX transfer size.  When that happens the UART HW will zero-pad the FIFO
   // and signal the final CRCI and the DMA transfer will complete on its own before
   // we flush it.  It is imperative that we handle RXSTALE first so we read the SNAP
   // register and determine the correct number of bytes in the transfer.

   if (misr & HAL_UART_RXSTALE_IMR_FLAG)        { rx_stale_intr       (uart_ctxt); }
   if (misr & HAL_UART_RXLEV_IMR_FLAG)          { rx_wm_intr          (uart_ctxt); }
   if (misr & HAL_UART_TXLEV_IMR_FLAG)          { tx_wm_intr          (uart_ctxt); }
   if (misr & HAL_UART_TX_READY_IMR_FLAG)       { tx_ready_intr       (uart_ctxt); }
   if (misr & HAL_UART_RXBREAK_CHANGE_IMR_FLAG) { rx_break_change_intr(uart_ctxt); }
   if (rx_dma_flag)                             { rx_dma_intr         (uart_ctxt); }
   if (tx_dma_flag)                             { tx_dma_intr         (uart_ctxt); }
   ...
   if (uart_ctxt->rx_halt)
   {
      ...
      DALSYS_EventCtrl(uart_ctxt->rx_idle_event, DALSYS_EVENT_CTRL_TRIGGER);
   }
   ...
   if (uart_ctxt->tx_halt)
   {
      ...
      DALSYS_EventCtrl(uart_ctxt->tx_idle_event, DALSYS_EVENT_CTRL_TRIGGER);
   }

   tal_interrupt_done(uart_ctxt->interrupt_handle, &uart_ctxt->uart_irq);
   DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "-uart_isr");
...
DALResult Uart_DumpRegs(DalDeviceHandle *h)
{
   ...
   if (uart_ctxt->gsbi_mem_handle)
   {
      ...
      if(result == DAL_SUCCESS)
      {
         ...
         DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "UartDbg: GSBIn_CTRL_REG=0x%08X", *gsbi);
      }
   }

   if (!uart_ctxt->clock_on)
   {
      DALSYS_LogEvent(id, DALSYS_LOGEVENT_INFO, "UartDbg: UART clocks are off");
      return DAL_SUCCESS;
   }
   ...
   else
   {
      ...
      else
      {
         ...
         if (uart_ctxt->use_dma)
         {
            ...
            uart_ctxt->tx_descriptor.physical_address  = tx_data_info.PhysicalAddr;
         ...
         else
         {
            ...
            transmit_ok = transmit_chunk(uart_ctxt, TRUE);
         }
