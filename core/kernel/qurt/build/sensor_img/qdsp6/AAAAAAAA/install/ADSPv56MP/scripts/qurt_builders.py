...
def hex_cfg_builder(target, source, env):
    ...
    if os.name == 'posix':
     if (int(ver.group(1))>6):
      timerlib = env.subst('${Q6_TOOLS_ROOT}/Tools/lib/iss/qtimer.so')
      l2viclib = env.subst('${Q6_TOOLS_ROOT}/Tools/lib/iss/l2vic.so')
     else:
      timerlib = env.subst('${Q6_TOOLS_ROOT}/qc/lib/iss/qtimer.so')
      l2viclib = env.subst('${Q6_TOOLS_ROOT}/qc/lib/iss/l2vic.so')
    elif os.name == 'nt':
     if (int(ver.group(1))>6):
      timerlib = env.subst('${Q6_TOOLS_ROOT}/Tools/lib/iss/qtimer.dll')
      l2viclib = env.subst('${Q6_TOOLS_ROOT}/Tools/lib/iss/l2vic.dll')
     else:
...
def const_autogen_builder(target, source, env):
    arglist = env.subst("${SIM} --quiet --pmu_statsfile nul --statsfile  nul  "+str(source[0])).split(" ")
    outinc_file = open(str(target[0]), 'w')
    print arglist
    cpp = subprocess.Popen(arglist, stdout=outinc_file)
    val = cpp.wait()
