        ...
        if (s.sh_flags & 2) != 0 and s.sh_size != 0:        # Section is memory resident
          ...
          for z in self.boot_mappings:
            if s.sh_addr >= z[1] and s.sh_addr < z[2]:
              skip = True
          if not skip:
            self.add_elf_mapping(s.sh_addr, s.sh_size, paddr, perm_write, perm_exec, asid, sname.endswith('.island'), i)
      asid += 1
  def add_elf_mapping(self, vaddr, size, paddr, perm_write, perm_exec, asid, island, infile):
    ...
    if self.CONFIG_ADVANCED_MEMORY:
      ...
      for t in self.opt_mappings:
        if t.locked and t.reloc:
          t.ppage -= (self.phys_base >> 12)
          print '%05X000 -> BASE  +  %05X000, length %05X000, perms X:%u W:%u R:%u U:%u C:%X' % (t.vpage, t.ppage, t.size,
                                                                                                 t.X, t.W, t.R, t.U, t.C)
          v.append(t.get_TLB_entry())
      for t in self.opt_mappings:
        if t.locked and not t.reloc:
          print '%05X000 -> Physical %05X000, length %05X000, perms X:%u W:%u R:%u U:%u C:%X' % (t.vpage, t.ppage, t.size,
                                                                                                 t.X, t.W, t.R, t.U, t.C)
