...
/*===========================================================================
 ...
 $Header: //components/rel/core.adsp/2.6.1/services/diag/DCM/common/src/diagcomm_smd.c#1 $ $DateTime: 2014/10/16 12:45:40 $ $Author: pwbldsvc $
...
===========================================================================*/
#if defined (DIAG_SIO_SUPPORT) && defined (DIAG_MP_MASTER)
void
diagcomm_smd2_inbound_pkt_notify (dsm_item_type ** item_ptr)
{
 ...
 #endif /* DIAG_SIO_SUPPORT  && DIAG_MP_MASTER */
                                               ...
                                               dsm_mem_op_enum_type mem_op)
{
   ...
   if (FALSE == bAllowFlow)
   {
	  ...
	  diag_tx_notify();
      return_val = osal_set_sigs(DIAG_TASK_HANDLE(), DIAG_EVENT_DRAIN_SIG, &return_sigs);
      ASSERT(OSAL_SUCCESS == return_val);
