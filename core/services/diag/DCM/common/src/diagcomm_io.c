...
/*===========================================================================
 ...
 $Header: //components/rel/core.adsp/2.6.1/services/diag/DCM/common/src/diagcomm_io.c#1 $
...
PARAMETERS
  ...
  is_cmd_rsp_type - Indicates whether it is cmd rsp that we are trying
                    to transmit
...
===========================================================================*/
void diagcomm_io_transmit( diagcomm_io_conn_type * conn, diagcomm_io_tx_params_type * tx_param, boolean is_cmd_rsp_type )
{
  ...
  case DIAGCOMM_IO_TYPE_SMDL:
    {
      ...
      {
        /*This is done to send out rsps even if we are in buffering mode*/
        dsm_enqueue(conn->open_params.smdl_params.cmd_tx_queue, &(tx_param->dsm_ptr));
        /* set DIAG_TX_SIG to indicate that dsm item is queued up for Tx */
        return_val = osal_set_sigs(&diag_tcb, DIAG_SMDL_CMD_WRITE_SIG, &return_sigs);
...
PARAMETERS
  conn - Diag I/O connection handle
  is_cmd_rsp_type - Indicates if intend to dequeue cmd rsp or data from wmq
 
RETURN VALUE
  Pointer to DSM dequeued
  
===========================================================================*/
dsm_item_type * diagcomm_io_dequeue_tx_wmq( diagcomm_io_conn_type * conn, boolean is_cmd_rsp_type )
{
  uint32 q_count = 0;
  ...
  case DIAGCOMM_IO_TYPE_SMDL:
    {
      if( is_cmd_rsp_type == TRUE )
      {
        if( conn->open_params.smdl_params.cmd_tx_queue )
        {
          if( conn->open_params.smdl_params.cmd_tx_queue->q_ptr ) 
            q_count = q_cnt(conn->open_params.smdl_params.cmd_tx_queue->q_ptr);
        }
        if( q_count > 0 )
        {
          return dsm_dequeue(conn->open_params.smdl_params.cmd_tx_queue);
        }
        break;
      } 
      else
