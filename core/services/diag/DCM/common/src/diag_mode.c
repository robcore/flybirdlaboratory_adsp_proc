...
$Header: //components/rel/core.adsp/2.6.1/services/diag/DCM/common/src/diag_mode.c#1 $
...
void diag_tx_mode_init( void )
{
  ...
  stream_index = DIAG_ID_TO_INDEX( 1 );
	...
	diag_tx_mode[stream_index].buffered_many_bytes_mark = 
	       ...
	       (diag_tx_mode[stream_index].pool_info.item_size * (DSM_POOL_ITEM_CNT(diag_tx_mode[0].pool_info.pool_id)));
...
boolean diag_drain_pool_check( uint8 stream_id )
{
  ...
  {
    if( DIAG_BUFFERED_DSM_CNT_TO_BYTES(dsm_cnt) <= diag_tx_mode[stream_index].buffered_few_bytes_mark )
    {
        diag_tx_mode[stream_index].drain = FALSE;
  }
  }
  ...
  if( (diag_tx_mode[stream_index].mode == DIAG_TX_MODE_STREAMING) || diag_tx_mode[stream_index].drain )
  {
    ...
    if( diagcomm_sio_tx_wmq_flow_enabled(port_num) == TRUE )
	{
	  return TRUE;
	}
  }
