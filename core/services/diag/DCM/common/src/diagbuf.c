...
Copyright (c) 1999-2014 by QUALCOMM Technologies, Incorporated.
All Rights Reserved.
...
/*===========================================================================
 ...
 $Header: //components/rel/core.adsp/2.6.1/services/diag/DCM/common/src/diagbuf.c#1 $
    ...
    while (status == DIAGBUF_READY_S && desc->state < DIAG_SEND_STATE_COMPLETE) {
      ...
      if (outbound == NULL) {
        ...
        if (((((int)desc->last - (int)desc->pkt) + outbound_used + hdlc_pad) >= DIAGBUF_SIO_TX_ITEM_SIZ)
          ...
          if (item_ptr) {
            item_ptr->used = outbound_used;
            diagcomm_send (item_ptr, outbound_used, desc->priority, DIAGCOMM_PORT_1);
            tx_cnt += outbound_used;
          }
          outbound = NULL;
