...
$Header: //components/rel/core.adsp/2.6.1/services/diag/LSM/qurt/src/Diag_LSM_Msg.c#1 $
...
void qsr_int_msg_send(const msg_qsr_const_type *const_blk, uint32 isHash)
{
  if (diag_qdi_handle >= DIAG_FD_MIN_VALUE)
  {
    ...
    if (pMsg)
    {
	  ...
	  if(stm_enabled)
	  {
		diag_stm_write_pkt( (void *)(pMsg +DIAG_REST_OF_DATA_POS), nLength - DIAG_REST_OF_DATA_POS);
  ...
  if (diag_qdi_handle >= DIAG_FD_MIN_VALUE)
  {
    ...
    if (pMsg)
    {
	  ...
	  if(stm_enabled)
	  {
		...
		msg_save_trace((const void *)const_blk, f3_args, 1, (void *)(pMsg + DIAG_REST_OF_DATA_POS), qshrink_type);
        /* Free data */
        diag_stm_msg_free(pMsg);
	  }
	  else
                               ...
                               unsigned int num_args, unsigned int *pLength, uint32 isHash, boolean stm_tag)
{
  ...
  if (valid_ssid && (const_blk->desc.ss_mask & rt_mask))
  {
    ...
    if (pMsg)
    {
	  ...
	  if(stm_tag)
	  {
		pdiag_data->diag_data_type = DIAG_DATA_TYPE_STM_F3;
	  }
	  else
#endif
	  {
