...
$Header: //components/rel/core.adsp/2.6.1/services/time/src/timer.c#1 $
...
#define MAX_TIMER_EXPIRED_DEBUG 20
...
)
{
  ...
  else if ( fn_cb_type == TIMER_DALSYS_EVENT_SIGNAL_TYPE)  
  {      
                                               ...
                                               uint32 *pgroup_idx)
{
  ...
  {
     MSG_1(MSG_SSID_ATS, MSG_LEGACY_ERROR,"Already max(%d) groups are created", TIMER_MAX_GROUPS);
...
)
{
  ...
  timer_sets[timer_set_cnt].timer_ext = free_timer->timer_ext;
  timer_sets[timer_set_cnt].timer_int = free_timer;
