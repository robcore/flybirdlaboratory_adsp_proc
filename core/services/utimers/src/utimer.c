...
$Header: //components/rel/core.adsp/2.6.1/services/utimers/src/utimer.c#1 $
...
)
{
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
   ...
   if(UTE_SUCCESS != (status = utimer_get_timer_struct(timer, timer_actual_addr, process_idx, &utimer_ptr)))
   {
      //UTIMER_FREE();
      return status;
   }
...
)
{
   ...
   if(UTE_SUCCESS == utimer_get_timer_struct(timer, timer_actual_addr, process_idx, &utimer_ptr))
   {
      //UTIMER_FREE();
  ...
  utimer_ptr->start              = ticks_now;       /* timer is defined at this timetick */
...
=============================================================================*/
utimer_error_type utimer_drv_set_64
(
  utimer_ptr_type                  timer,
/* Timer's Actual Address in its own Process Domain, for Logging purposes only */
  utimer_ptr_type                  timer_actual_addr,
...
)
{
   ...
   utimer_notify_type       callback_type;
   utimer_osal_notify_obj_ptr sigs_obj;
   utimer_osal_notify_data  sigs_data;
   ...
   time_ticks   = utimer_timetick_conv_to_ticks(time, unit);
   reload_ticks = utimer_timetick_conv_to_ticks(reload, unit);
  ...
  /* defensive programming. Clearing the signal that will
   ...
   callback_type = utimer_ptr->info.callback_type;
   sigs_obj      = utimer_ptr->cbfn.sigs_obj;
   sigs_data     = utimer_ptr->cbdt.sigs;
   
   if ( callback_type == UTIMER_NATIVE_OS_SIGNAL_TYPE )
   {
     qurt_anysignal_clear( (qurt_anysignal_t *) sigs_obj, sigs_data );
   }
   //else if(callback_type == UTIMER_NATIVE_OS_SIGNAL2_TYPE) 
   //{
   //  qurt_signal2_clear( (qurt_signal2_t *) sigs_obj, sigs_data );
   //}
   else if(callback_type == UTIMER_REX_TCB_SIGNAL_TYPE)
   {
#ifdef FEATURE_TIMER_REX_IS_AVAILABLE
    rex_clear_sigs((rex_tcb_type *)sigs_obj, sigs_data);
#else
    ERR_FATAL("On Adsp, signal cannot be of rex type. timer handle: 0x%x", *timer, 0, 0);
#endif /* FEATURE_TIMER_REX_IS_AVAILABLE */
   }
   ...
   utimer_ptr->reload = reload_ticks;
   utimer_ptr->expiry = ticks_now + time_ticks;
...
)
{
   ...
   *rem_time = 0;
   status = utimer_get_timer_struct(timer, timer_actual_addr, process_idx, &utimer_ptr);
