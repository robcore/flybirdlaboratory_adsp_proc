...
$Header: //components/rel/core.adsp/2.6.1/services/utimers/src/utimer_qdi_ddr.c#1 $ 
...
extern utimer_qdi_opener utimer_clients[];
extern uint32 utimer_clients_cntr;

/* Guest OS doesnt require pipe */
extern qurt_pipe_data_t utimer_clients_pipe[UTIMER_MAX_CLIENTS][UTIMER_CLIENT_PIPE_SIZE];
...
Pipe Apis
===========================================================================*/
int utimer_qdi_create_pipe(utimer_qdi_opener *clntobj, uint32 idx)
{
   ...
   qurt_pipe_attr_set_buffer(&pipe_attr, utimer_clients_pipe[idx]);
...
void utimer_qdi_release(qurt_qdi_obj_t *obj)
{
   ...
   utimer_clients[process_idx].opener_allocated = FALSE;
...
int utimer_qdi_open(int client_handle, utimer_qdi_opener *me)
{
   ...
   /* Find a free slot */
   for(idx=0; idx<UTIMER_MAX_CLIENTS && utimer_clients[idx].opener_allocated == TRUE; idx++);
   
   if(idx == UTIMER_MAX_CLIENTS)
   {
      UTIMER_QDI_FREE();
      ERR_FATAL("Max Processes %d reached, processes: %d", idx, utimer_clients_cntr, 0);
      return -1;
   }
   utimer_clients_cntr++;
   
   clntobj = &utimer_clients[idx];
   ...
   {      
      status = utimer_qdi_create_pipe(clntobj, idx);
...
) 
{
  ...
  for(i=0; i<UTIMER_MAX_CLIENTS; i++)
     utimer_clients[i].opener_allocated = FALSE;
