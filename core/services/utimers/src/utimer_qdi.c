...
$Header: //components/rel/core.adsp/2.6.1/services/utimers/src/utimer_qdi.c#1 $ 
...
utimer_qdi_opener utimer_clients[UTIMER_MAX_CLIENTS];
uint32 utimer_clients_cntr = 0;

/* Guest OS doesnt require pipe */
qurt_pipe_data_t utimer_clients_pipe[UTIMER_MAX_CLIENTS][UTIMER_CLIENT_PIPE_SIZE];
...
int utimer_qdi_get_cb_info(int client_handle, utimer_qdi_opener *obj, utimer_cb_info *cb)
{
   ...
   if( qurt_pipe_receive_cancellable(&obj->pipe, &data1) == QURT_ECANCEL ||
       qurt_pipe_receive_cancellable(&obj->pipe, &data2) == QURT_ECANCEL ||
       qurt_pipe_receive_cancellable(&obj->pipe, &data3) == QURT_ECANCEL )
   {
...
static inline utimer_error_type utimer_qdi_set_deferrable(int client_handle, utimer_ptr_type ptimer, boolean condition, unsigned int process_idx)
{
   utimer_type       timer;
   int               result;
   ...
   status = utimer_drv_set_deferrable(&timer, ptimer, condition, process_idx);
...
static inline utimer_error_type utimer_qdi_set_64(int client_handle, utimer_ptr_type ptimer, utimer_timetick_type time, 
                                             utimer_timetick_type reload, utimer_unit_type unit, unsigned int process_idx)
{
   int result;
   utimer_error_type status;
   utimer_type timer;
   ...
   status = utimer_drv_set_64(&timer, ptimer, time, reload, unit, process_idx);
...
static inline utimer_error_type utimer_qdi_get_64(int client_handle, utimer_ptr_type ptimer, utimer_unit_type     unit,
                                             unsigned int process_idx, utimer_timetick_type *ptick)
{
   int result;
   utimer_type timer;
   ...
   /* Call timer_drv_get */
   tick = utimer_drv_get_64(&timer, ptimer, unit, process_idx);
...
static inline utimer_error_type utimer_qdi_clr_64(int client_handle, utimer_ptr_type ptimer, 
                                                utimer_unit_type     unit, unsigned int process_idx, 
                                                utimer_timetick_type *ptick)
{
   ...
   status = utimer_drv_clr_64(&timer, ptimer, unit, process_idx, &tick);
...
static inline utimer_error_type utimer_qdi_undef(int client_handle, utimer_ptr_type ptimer, unsigned int process_idx)
{
   int result;
   utimer_type timer;
   utimer_error_type status;
   ...
   status = utimer_drv_undef(&timer, ptimer, process_idx);
                          ...
                          qurt_qdi_arg_t a9)
{
   ...
   switch(method)
   {
      ...
      case UTIMER_QDI_SET_DEFERRABLE: 
         return utimer_qdi_set_deferrable(client_handle, a1.ptr, a2.num, process_idx);
#endif /* #ifdef FEATURE_UTIMER_DEFER_SUPPORT */
      ...
      case UTIMER_QDI_SET_64:  
         return utimer_qdi_set_64(client_handle, a1.ptr, (utimer_timetick_type)COMBINE64(a2.num, a3.num), (utimer_timetick_type)COMBINE64(a4.num, a5.num), a6.num, process_idx);
         
      case UTIMER_QDI_GET_64: 
         return utimer_qdi_get_64(client_handle, a1.ptr, a2.num, process_idx, a3.ptr); 
         
      case UTIMER_QDI_CLR_64: 
         return utimer_qdi_clr_64(client_handle, a1.ptr, a2.num, process_idx, a3.ptr); 
         
      case UTIMER_QDI_UNDEF: 
         return utimer_qdi_undef(client_handle, a1.ptr, process_idx);
