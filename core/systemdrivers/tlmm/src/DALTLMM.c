...
$Header: //components/rel/core.adsp/2.6.1/systemdrivers/tlmm/src/DALTLMM.c#1 $
...
void TLMM_InitPlatformIO(TLMMClientCtxt *pCtxt)
{
  ...
  if( hPlatformInfo != NULL )
  {
     ...
     */
    if( DalChipInfo_ChipFamily() == DALCHIPINFO_FAMILY_MDM9x35 )
    {
          ...
          (PlatformInfo.platform == DALPLATFORMINFO_TYPE_MTP_MDM) )
      {
        ...
        else
        {
           ...
           * Get Fusion subtypes.
           */
          if( PlatformInfo.subtype == 3 )
          {
            pszPlatformIO = "TLMMPlatformIO_Fusion4p5";
          }
          else
          {
            pszPlatformIO = "TLMMPlatformIO_TYPE2_Fusion";
          }
        }
      }
    }
    ...
    {
      if( PlatformInfo.fusion == TRUE )
      {
        pszPlatformIO = "TLMMPlatformIO_FUSION";
      }
      else if( PlatformInfo.platform == DALPLATFORMINFO_TYPE_CDP )
      {
    ...
    if( DAL_SUCCESS != eResult )
    {
      ...
      if( DAL_SUCCESS != eResult )
      {
        DALSYS_LogEvent(pCtxt->pDevCtxt->DevId, 
            ...
            "TLMM_InitPlatformIO: No platform IO found configurations found.");
      }
    }
  }
...
)
{
  ...
  (void)DALSYS_SyncEnter(pCtxt->pDevCtxt->hTlmmSync); 
  if( pCtxt->pDevCtxt->hPlatIO != NULL )
  {
    if(DAL_SUCCESS == 
