...
$Header: //components/rel/core.adsp/2.6.1/systemdrivers/GPIOInt/src/kernel/GPIOInt.c#3 $
...
)
{
  ...
  if(result == DAL_SUCCESS)
  {
    ...
    uGPIOInt_Init(device->gpioint_cntrl->summary_intr_id);
  }
  else
  {
...
)
{
  ...
  /*
   * Clear the uimage mapping for this gpio interrupt.
   */
...
)
{
  ...
  if(device->gpioint_cntrl->state[gpio].intr_type == HAL_GPIOINT_SUMMARY)
  {
    ...
    if(device->gpioint_cntrl->gpioint_uimage != 0)
    {
      uGPIOInt_SetDirectConnectGPIOMapping(gpio,device->gpioint_cntrl->summary_intr_id);
    }
  }
  else
  { 
