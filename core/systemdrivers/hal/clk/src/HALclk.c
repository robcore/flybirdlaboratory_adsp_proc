...
$Header: //components/rel/core.adsp/2.6.1/systemdrivers/hal/clk/src/HALclk.c#1 $
...
)
{
      ...
      HAL_clk.aSourceDesc[eSource].pControl->EnableVote != NULL)
  {
    HAL_clk.aSourceDesc[eSource].pControl->EnableVote(
       &HAL_clk.aSourceDesc[eSource], bAsync);
  }
...
/* ===========================================================================
**  HAL_clk_EnableSourceOverride
**
** ======================================================================== */

void HAL_clk_EnableSourceOverride
(
  HAL_clk_SourceType            eSource,
  HAL_clk_ClockDomainHandleType pClockDomainHandle
)
{
  ...
  if (eSource < HAL_CLK_NUM_OF_SOURCES &&
      HAL_clk.aSourceDesc[eSource].pControl != NULL &&
      HAL_clk.aSourceDesc[eSource].pControl->EnableVoteOverride != NULL &&
      pmClockDomainDesc != NULL)
  {
    HAL_clk.aSourceDesc[eSource].pControl->EnableVoteOverride(
       &HAL_clk.aSourceDesc[eSource], pmClockDomainDesc);
  }

} /* END HAL_clk_EnableSourceOverride */
...
/* ===========================================================================
**  HAL_clk_DisableSourceOverride
**
** ======================================================================== */

void HAL_clk_DisableSourceOverride
(
  HAL_clk_SourceType            eSource,
  HAL_clk_ClockDomainHandleType pClockDomainHandle
)
{
  ...
  if (eSource < HAL_CLK_NUM_OF_SOURCES &&
      HAL_clk.aSourceDesc[eSource].pControl != NULL &&
      HAL_clk.aSourceDesc[eSource].pControl->DisableVoteOverride != NULL &&
      pmClockDomainDesc != NULL)
  {
    HAL_clk.aSourceDesc[eSource].pControl->DisableVoteOverride(
       &HAL_clk.aSourceDesc[eSource], pmClockDomainDesc);
  }

} /* END HAL_clk_DisableSourceOverride */
...
/* ===========================================================================
**  HAL_clk_SlewConfigPLL
**
** ======================================================================== */

void HAL_clk_SlewConfigPLL
(
  HAL_clk_SourceType           eSource,
  const HAL_clk_PLLConfigType *pConfig
)
{
  ...
  if (eSource < HAL_CLK_NUM_OF_SOURCES &&
      HAL_clk.aSourceDesc[eSource].pControl != NULL &&
      HAL_clk.aSourceDesc[eSource].pControl->SlewConfigPLL != NULL)
  {
    HAL_clk.aSourceDesc[eSource].pControl->SlewConfigPLL(
      &HAL_clk.aSourceDesc[eSource], pConfig);
  }

} /* END HAL_clk_SlewConfigPLL */
...
uint32 HAL_clk_GetDeviceId( void )
{
  return(HAL_clk_GetDeviceNumber());
}
...
/* ===========================================================================
**  HAL_clk_FindChipFlag
** 
** 
** ======================================================================== */

static uint32 HAL_clk_FindChipFlag
(
   uint32 nDeviceNumber
)
{
  switch (nDeviceNumber)
  {
     case HAL_CLK_CHIP_SOC_MSM8974:
       return(HAL_CLK_CHIP_FLAG_MSM8974);

     case HAL_CLK_CHIP_SOC_MDM9x25:
       return(HAL_CLK_CHIP_FLAG_MDM9x25);

     case HAL_CLK_CHIP_SOC_MSM8x26:
       return(HAL_CLK_CHIP_FLAG_MSM8x26);

     case HAL_CLK_CHIP_SOC_MSM8x10:
       return(HAL_CLK_CHIP_FLAG_MSM8x10);

     case HAL_CLK_CHIP_SOC_APQ8084:
       return(HAL_CLK_CHIP_FLAG_APQ8084);

     case HAL_CLK_CHIP_SOC_MSM8962:
       return(HAL_CLK_CHIP_FLAG_MSM8962);

     case HAL_CLK_CHIP_SOC_MPQ8092:
       return(HAL_CLK_CHIP_FLAG_MPQ8092);

     case HAL_CLK_CHIP_SOC_MDM9x35:
       return(HAL_CLK_CHIP_FLAG_MDM9x35);

     case HAL_CLK_CHIP_SOC_MSM8994:
       return(HAL_CLK_CHIP_FLAG_MSM8994);

     default:
       return(HAL_CLK_CHIP_FLAG_ALL);
  }
...
/* ===========================================================================
**  HAL_clk_FilterClockDomains
**  
**  Cans an array of clock domains and filters out clocks that are not
**  supported on the current chipset.
** 
** ======================================================================== */

void HAL_clk_FilterClockDomains
(
   HAL_clk_ClockDomainDescType** pDomain
)
{
  uint32 nTotalClksIdx = 0;
  uint32 nSupportedClksIdx = 0;
  uint32 nCurrDomainIdx = 0;
  HAL_clk_ClockDomainDescType * pCurrDomain = pDomain[0];
  static uint32 nChipFlag = 0;

  /*
   * Make sure we have the chip information before filtering clock domains.
   */
  if(nChipFlag == 0)
  {
    nChipFlag = HAL_clk_FindChipFlag(HAL_clk_GetDeviceId());
  }

  /*
   * Check for any clocks that are not part of this domain and filter them out. 
   * This may shrink the domain array size and adjust clock indices.
   */
  while(pCurrDomain != NULL)
  {
    for(nTotalClksIdx = 0; nTotalClksIdx < pCurrDomain->nClockCount; ++nTotalClksIdx)
    {
      /*
       * If a clock is supported and the current list of supported clocks is 
       * a subset of the total defined clocks, adjust the array as needed. 
      */
      if((pCurrDomain->pmClocks[nTotalClksIdx].nChipsetFlag & nChipFlag) ||
         (pCurrDomain->pmClocks[nTotalClksIdx].nChipsetFlag == 0))
      {
         if(nTotalClksIdx != nSupportedClksIdx)
         {
           pCurrDomain->pmClocks[nSupportedClksIdx] = pCurrDomain->pmClocks[nTotalClksIdx];
         }
         nSupportedClksIdx++;
      }
    }
    pCurrDomain->nClockCount = nSupportedClksIdx;
    nSupportedClksIdx = 0;
    pCurrDomain = pDomain[++nCurrDomainIdx];
  }
...
void HAL_clk_AdjustSourceFrequency
(
  HAL_clk_SourceType    eSource,
  HAL_clk_PLLConfigType *pConfig
)
{
  if (eSource < HAL_CLK_NUM_OF_SOURCES && pConfig != NULL)
  {
    HAL_clk_AdjustSourceFrequencyInternal(&HAL_clk.aSourceDesc[eSource], pConfig);
  }
...
/* ===========================================================================
**  HAL_clk_GetSourceDelta
**
** ======================================================================== */

void HAL_clk_GetSourceDelta
(
  HAL_clk_SourceType    eSource,
  HAL_clk_PLLConfigType *pConfig
)
{
  if (eSource < HAL_CLK_NUM_OF_SOURCES && pConfig != NULL)
  {
    HAL_clk_GetSourceDeltaInternal(&HAL_clk.aSourceDesc[eSource], pConfig);
  }

} /* HAL_clk_GetSourceDelta */


void HAL_clk_SelectClockSource
(
   HAL_clk_ClockDomainHandleType pClockDomainHandle,
   HAL_clk_SourceType eSource
)
{
  ...
  if (eSource < HAL_CLK_NUM_OF_SOURCES && pmClockDomainDesc != NULL)
  {
    HAL_clk_SelectClockSourceInternal(pmClockDomainDesc, &HAL_clk.aSourceDesc[eSource]);
  }

} /* HAL_clk_SelectClockSource */
