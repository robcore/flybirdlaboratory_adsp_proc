...
$Header: //components/rel/core.adsp/2.6.1/systemdrivers/hal/clk/hw/lpass/src/HALclkTest.c#1 $
  ...
  *pnFreqHz = 0;
  ...
  switch (nTestClockType)
  {
    ...
    case HAL_CLK_LPASS_TEST:
      if ((HAL_clk_GetDeviceNumber() == HAL_CLK_CHIP_SOC_MSM8974) || 
          (HAL_clk_GetDeviceNumber() == HAL_CLK_CHIP_SOC_MSM8x26))
      {
        HWIO_OUT(LPASS_DBG_V1_CLK, nTestClockSel);
        HWIO_OUTF(LPASS_DBG_V1_CLK, DBG_CLK_ENABLE, 0x1);
        HWIO_OUTF(GCC_GCC_DEBUG_CLK_CTL, MUX_SEL, 
                  HAL_CLK_GCC_TEST_LPASS_GCC_DBG_CLK & HAL_CLK_TEST_SEL_MASK); 
      }
      else if(HAL_clk_GetDeviceNumber() == HAL_CLK_CHIP_SOC_MSM8x10)
      {
        HWIO_OUT(LPASS_DBG_V2_CLK, nTestClockSel);
        HWIO_OUTF(LPASS_DBG_V2_CLK, DBG_CLK_ENABLE, 0x1);
        HWIO_OUTF(GCC_GCC_DEBUG_CLK_CTL, MUX_SEL, 
                  HAL_CLK_GCC_TEST_LPASS_GCC_DBG_CLK & HAL_CLK_TEST_SEL_MASK); 
      }
      else
