...
$Header: //components/rel/core.adsp/2.6.1/systemdrivers/hal/clk/hw/lpass_v2/src/lcc/HALclkQ6.c#1 $
...
)
{
  ...
  /* LPAAUDIO Prius PLL */
  else if(pmConfig->eSource == HAL_CLK_SOURCE_LPAPLL2)
  {
    return 2;
  }
  /* AUD_REF_CLK Source */
  else if(pmConfig->eSource == HAL_CLK_SOURCE_AUD_REF_CLK)
  {
    return 3;
  }
...
)
{
  ...
  {
    nCmdCGRAddr = HWIO_ADDR(LPASS_QDSP6SS_CORE_CMD_RCGR);
    nCmdCGRVal  = HWIO_IN(LPASS_QDSP6SS_CORE_CMD_RCGR);
    nCfgCGRAddr = HWIO_ADDR(LPASS_QDSP6SS_CORE_CFG_RCGR);
    nCfgCGRVal  = HWIO_IN(LPASS_QDSP6SS_CORE_CFG_RCGR);

    /*
     * We'll be using the Q6SS PLL so we need to mask off the SPM PLL 
     * override bits corresponding to the LPA PLLs. 
     */
  ...
  else
  {
    ...
    {
      /*
       * Q6 will be sourced by one of the LPA PLLs so mask the Q6SS 
       * PLL. 
       */
      HWIO_OUTF(LPASS_QDSP6SS_SPM_EVENT_CTL, PLL_OVRRD, 0x1);
...
)
{
  ...
  //    break;
    ...
    case 2:
      ...
      /* AUD_REF_CLK Source */
    case 3:
      pmConfig->eSource = HAL_CLK_SOURCE_AUD_REF_CLK;
      break;
    case 7:
