...
$Header: //components/rel/core.adsp/2.6.1/systemdrivers/InterruptController/src/qurt/DALInterruptController.c#2 $ 
...
) 
{
  ...
  /*
   * Set the trigger in the driver at registration time
   */
...
)
{
  ...
  DALSYS_SyncEnter(pclientCtxt->hSyncIntrCtrlTbl);
  (void)SetTrigger(pDevice, nInterruptVector,nTrigger);
  DALSYS_SyncLeave(pclientCtxt->hSyncIntrCtrlTbl);
