...
/*
  ...
  $Header: //components/rel/core.adsp/2.6.1/systemdrivers/clock/src/ClockDriver.c#1 $
  ...
  if (!bSuppressibleRequest)
  {
    ...
    if (pClockDomain->nReferenceCount == 0)
    {
      ...
      if (pClockDomain->pSource != NULL)
      {
        ...
        /*
         * If this domain has the ability to enable/disable the SPMCTL source override, 
         * handle that here. 
         */
        HAL_clk_DisableSourceOverride(pClockDomain->pSource->eSource, pClockDomain->HALHandle);
      }
  ...
  if (!bSuppressibleRequest)
  {
    ...
    if (pClockDomain->nReferenceCount == 1)
    {
      if (pClockDomain->pSource != NULL)
      {
         ...
         * handle that here. 
         */
        HAL_clk_EnableSourceOverride(pClockDomain->pSource->eSource, pClockDomain->HALHandle);
      }
...
)
{
      ...
      pNewSource != NULL)
  {
    ...
    if (pDomain->nReferenceCount > 0)
    {
       ...
       * handle that here. 
       */
      HAL_clk_DisableSourceOverride(pDomain->pSource->eSource, pDomain->HALHandle);
    }
      ...
      pDomain->pSource != NULL)
  {
    ...
    if (pDomain->nReferenceCount > 0)
    {
       ...
       * handle that here. 
       */
      HAL_clk_EnableSourceOverride(pDomain->pSource->eSource, pDomain->HALHandle);
    }
