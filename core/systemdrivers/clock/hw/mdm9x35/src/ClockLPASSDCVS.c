...
/**
  ...
  DCVS NPA node definitions for MSM8974 LPASS clock driver.
*/
/*
  ====================================================================
  ...
  ==================================================================== 
  $Header: //components/rel/core.adsp/2.6.1/systemdrivers/clock/hw/mdm9x35/src/ClockLPASSDCVS.c#1 $
  $DateTime: 2014/10/16 12:45:40 $
  $Author: pwbldsvc $
 
  when       who     what, where, why
  --------   ---     -------------------------------------------------
  06/12/12   dcf     Added LDO voltage changing support.
...
)
{
  ...
  ClockCPUConfigType *pPreviousConfig, *pTempConfig;
  ...
  {
    if(pLPASSCtxt->pImageConfig->bGlobalLDOEnable == TRUE)
    {
      eResult = Clock_SetLDOVoltage(pDrvCtxt, pConfig, &VRegRequest);
    }
    else
    {
  ...
  if (pPreviousMux->pSourceFreqConfig != NULL && pMux->pSourceFreqConfig != NULL)
  {
    ...
    {
      pTempConfig = Clock_GetIntermediateConfig(pDrvCtxt, pPreviousMux);

      if (pTempConfig != NULL)
      {
        eResult = Clock_SetCPUConfigGFMux(pDrvCtxt, pTempConfig);
        if (eResult != DAL_SUCCESS)
        {
          DALCLOCK_FREE(pDrvCtxt);
          return;
