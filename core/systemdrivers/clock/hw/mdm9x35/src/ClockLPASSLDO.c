...
/*  
  ...
  ==================================================================== 
  $Header: //components/rel/core.adsp/2.6.1/systemdrivers/clock/hw/mdm9x35/src/ClockLPASSLDO.c#1 $
...
#define CLOCK_LDO_QFPROM_ENABLE_LPASS 0x10000000
...
)
{
  ...
  /*
   * Since 8974 and 8974 Pro exist in the same device configuration yet are 
   * of different families, ensure only 8974 Pro is allowed to run eLDO. 
   */
  ...
  {
    if(HWIO_IN(QFPROM_RAW_SPARE_REG27_ROW0_MSB) & CLOCK_LDO_QFPROM_ENABLE_LPASS)
    {
...
)
{
  ...
  if(pLPASSQ6Config->QDSP6Ctxt.pCPUConfig->Mux.eVRegLevel != 
...
)
{
  ...
  nNewVoltageUV = pConfig->nLDOQ6UV;
  ...
  if(TRUE == bLDOInited)
  {
    ...
    gnCurrentvoltage = nNewVoltageUV;
    /*
     * Program the new voltage data to the LDO VREF register and
     * switch to LDO mode. 
     */
    nLDOVddData = CLOCK_LDO_UV_MAP_TO_HW(nNewVoltageUV);
     ...
     */
    CLOCK_LDO_SETTLE_DELAY(nNewVoltageUV, pLPASSQ6Config->QDSP6Ctxt.pCPUConfig->nVDDCPU);
