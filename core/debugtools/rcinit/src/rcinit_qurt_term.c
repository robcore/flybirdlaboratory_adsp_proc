...
/*=============================================================================
Edit History
$Header: //components/rel/core.adsp/2.6.1/debugtools/rcinit/src/rcinit_qurt_term.c#1 $
...
void rcinit_register_termfn_group(RCINIT_GROUP group, void (*fn)(void))
{
   ...
   if (RCINIT_GROUP_7 >= group)
   {
      if (RCECB_NULL != rcecb_register_context_name(rcinit_term_rcecb[group], fn))
      {
#if defined(RCINIT_TRACER_SWEVT)
...
void rcinit_unregister_termfn_group(RCINIT_GROUP group, void (*fn)(void))
{
   ...
   if (RCINIT_GROUP_7 >= group)
   {
      if (RCECB_NULL != rcecb_unregister_context_name(rcinit_term_rcecb[group], fn))
      {
#if defined(RCINIT_TRACER_SWEVT)
...
void rcinit_internal_unprocess_groups(void)
{
   ...
   do
   {
      ...
      count = rcevt_getcontexts_name(rcinit_term_rcevt[rcinit_internal.group_curr]); // get registered count
      ...
      rcevt_register_sigex_name(RCINIT_RCEVT_TERMACK, RCEVT_SIGEX_TYPE_SIGQURT, &rcevt_sigex);

      qurt_anysignal_clear(&anysignal, SIG_ACK);                       // clear signals before waiting

      rcevt_signal_name(rcinit_term_rcevt[rcinit_internal.group_curr]);          // signal registered
      ...
      if (count > 0)                                                             // block only if count is positive
      {
         ...
         do
         {
            ...
            if (recv & SIG_BAIL)                                                 // BAIL OUT - MAXIMUM TIME
            {
               ...
               rcecb_signal_name(rcinit_term_rcecb[RCINIT_GROUP_0]);             // signal registered functions of their event, returns when done
      ...
      rcevt_unregister_sigex_name(RCINIT_RCEVT_TERMACK, RCEVT_SIGEX_TYPE_SIGQURT, &rcevt_sigex);
      ...
      MSG_SPRINTF_2(MSG_SSID_TMS, MSG_LEGACY_HIGH, "termfn processing registrants %d group %x", rcecb_getcontexts_name(rcinit_term_rcecb[rcinit_internal.group_curr]), rcinit_internal.group_curr);

      rcecb_signal_name(rcinit_term_rcecb[rcinit_internal.group_curr]);          // signal registered functions of their event
