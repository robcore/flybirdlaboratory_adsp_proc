...
/*===========================================================================
  ...
  $Header: //components/rel/core.adsp/2.6.1/debugtools/dog/src/dog_legacy.c#1 $ $DateTime: 2014/10/16 12:45:40 $ $Author: pwbldsvc $
...
#define DOG_WAKEUP_TIMER_SIG    0x0800 // internal use; clients select their own
...
typedef struct {
   ...
   uint32 pong;                                                                  // "ping/pong" type construct for dog_hb, pong is set via the report mechanism
   uint32 missed_pong;                                                           // "ping/pong" type construct for dog_hb, pong is set via the report mechanism
...
*/
static rex_timer_type dog_wakeup_timer;
static rex_timer_type dog_startup_timer;
...
*/
static boolean dog_auto_kicking = FALSE;
...
*/
static boolean dog_enabled = TRUE;                                               // TRACE32 Instrument to Disable Dog Task At Runtime

/*
**  This flag determines whether or not to set the dog startup timer.  Disable via debugger during init
**  Default value is true
**/
static boolean dog_enable_startup_timer = TRUE;                                  // TRACE32 Instrument to Disable Dog Task At Runtime
...
*/
static dog_state_table_type dog_state_table[DOG_NUM_TASKS];
...
===========================================================================*/
void dog_report(dog_report_type handle)
{
   ...
   state_table = state_table;                                                    // unused
...
===========================================================================*/
static void dog_set_autokicking(boolean enable)
{
...
===========================================================================*/
dog_report_type dog_register_rex(rex_tcb_type* tid, uint32 timeout, uint32 unused)
{
   dog_report_type i = 0;
   ...
   {
      i = DOG_NUM_TASKS;
      ERR("DOG register invalid tcb", 0, 0, 0);
   }
   ...
   else
   {
      ...
      {
         i = DOG_NUM_TASKS;
         ERR("DOG registration table full", 0, 0, 0);
      }
      ...
      else
      {
         ...
         // for dog_monitor_pause/dog_monitor_resume
...
===========================================================================*/
static void dog_internal_init(void)
{
   ...
   rcevt_register_name(RCINIT_RCEVT_INITGROUPS, TASK_START_SIG);
   ...
   rcevt_signal_name(DOG_HW_INIT_COMPLETE);
...
#if (!defined(NO_DOG))
   ERR_FATAL("DOG detects stalled initialization", 0, 0, 0);
#else
   ERR("DOG detects stalled initialization", 0, 0, 0);
#endif /* ! NO_DOG */
#endif
...
void dog_task(dword ignored)
{
   ...
   for (;;)
   {
...
#ifndef DOG_EXCLUDE_EXCEPTION
#if (!defined(NO_DOG))
                     ERR_FATAL("DOG detects task starvation of %d", i, 0, 0);
#else
                     ERR("DOG detects task starvation of %d", i, 0, 0);
#endif /* ! NO_DOG */
#endif
      ...
      if (dog_hb_cnt >= DOG_HB_CNT_MAX)
      {
         ...
         rcevt_signal_name(DOG_HB_RCEVT_PING);                                   // "ping/pong" construct, this is the ping, the pong is via the report
      }
...
DESCRIPTION
   ...
   KERNEL USE ONLY, OTHER USE IS A BUG

SIDE EFFECTS
   Not supported by kernel rex_emulation (l4, qurt) -- Deprecated Interface

===========================================================================*/
void dog_monitor_pause(dog_report_type report_id)
{
...
===========================================================================*/
void dog_monitor_resume(dog_report_type report_id)
{
   dog_report_type i = report_id ^ DOG_ID_MANGLER;
...
void dog_crash_report(void (*print_ptr)(char*))
{
   ...
   snprintf(log_buf, LOG_BUF_SIZE, "Dog Report Information (dog_state_table)\n");
   print_ptr(log_buf);

   snprintf(log_buf, LOG_BUF_SIZE, " [idx]   Task Name   Pri  Timeout   Count  Is_Blocked\n");
   print_ptr(log_buf);

   for (i = 0; i < DOG_NUM_TASKS; i++)
   {
      snprintf(log_buf, LOG_BUF_SIZE, " [%3d] %11s  %4d   %6d  %6d       %1d\n",
               i,
   ...
   snprintf(log_buf, LOG_BUF_SIZE, "End Dog Report\n\n");
   print_ptr(log_buf);
}
...
===========================================================================*/
uint32 dog_get_report_period(dog_report_type handle)
{
   dog_report_type i = handle ^ DOG_ID_MANGLER;
   uint32 rtn_val = 0;
...
===========================================================================*/
void dog_deregister(dog_report_type handle)
{
   dog_report_type i = handle ^ DOG_ID_MANGLER;
...
dog_report_type dog_hb_register_qurt(qurt_anysignal_t* anysignal_p, unsigned int anysignal_m)
{
   ...
   if (tid != 0)
   {
      ...
      else
      {
         ...
         rcevt_register_sigex_name(DOG_HB_RCEVT_PING, state_table->ping.type, &(state_table->ping.sigex));
      }
   ...
   {
      i = DOG_NUM_TASKS;
      ERR("DOG register TID 0 not a valid registrant", 0, 0, 0);
   }
...
void dog_hb_deregister(dog_report_type handle)
{
   ...
   if (state_table != NULL && state_table->tid != NULL && state_table->ping.type != RCEVT_SIGEX_TYPE_NONE)
   {
      ...
      rcevt_unregister_sigex_name(DOG_HB_RCEVT_PING, state_table->ping.type, &(state_table->ping.sigex));
