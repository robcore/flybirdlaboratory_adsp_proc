...
$Header: //components/rel/core.adsp/2.6.1/api/power/npa_resource.h#1 $
==========================================================================*/
...
typedef struct npa_client 
{
  ...
  void                    *log_handle;    /* ULog associated with client */
...
{
  struct npa_event_header  *header;
  npa_event                *creation_event;
} npa_event_list;

/* Return a pointer to the first event in a resource's event list */
#define NPA_EVENT_FIRST_EVENT( evt )  ( evt->resource->events.header->reg_event )
...
#define NPA_EVENT_REG_DATA( evt ) ( evt->reg_data )
