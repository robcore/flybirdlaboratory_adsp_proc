...
/**@ingroup func_qurt_pipe_attr_set_buffer_partition
  ...
  Specify the memory type where a pipe's buffer will be allocated.
  Pipes can be allocated in RAM or TCM/LPM.
  Pipes are allocated in TCM/LPM when they will be used in Island mode.

  NOTE - If a pipe is specified as being allocated in TCM/LPM, it must be created
  with the pipe init operation. The pipe create operation will result in an error.

  @datatypes
  #qurt_pipe_attr_t

  @param[in,out] attr Pipe attribute structure.
  @param[in] Pipe memory partition. Values:
             QURT_PIPE_ATTR_MEM_PARTITION_RAM - Pipe resides in RAM
             QURT_PIPE_ATTR_MEM_PARTITION_TCM - Pipe resides in TCM/LCM
...
int qurt_pipe_receive_cancellable(qurt_pipe_t *pipe, qurt_pipe_data_t *presult);
