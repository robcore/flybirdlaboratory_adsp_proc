#ifndef _ADSPMSGD_RPC_STUB_H
#define _ADSPMSGD_RPC_STUB_H
#include "adspmsgd_rpc.h"
#ifndef _QAIC_ENV_H
#define _QAIC_ENV_H
...
#ifndef _ADSPMSGD_RPC_SLIM_H
#define _ADSPMSGD_RPC_SLIM_H
#include "remote.h"
#include <stdint.h>
...
static const Method methods[1] = {{REMOTE_SCALARS_MAKEX(0,0,0x1,0x1,0x0,0x0),0x10,0x4,5,5,(&(parameterArrays[0])),0x4,0x4}};
static const Method* const methodArrays[1] = {&(methods[0])};
static const char strings[48] = "buff_addr\0ion_flags\0buf_size\0filter\0heapid\0init\0";
static const uint16_t methodStrings[6] = {43,36,10,29,20,0};
static const uint16_t methodStringsArrays[1] = {0};
__QAIC_SLIM_EXPORT const Interface __QAIC_SLIM(adspmsgd_rpc_slim) = {1,&(methodArrays[0]),0,0,&(methodStringsArrays [0]),methodStrings,strings};
#endif //_ADSPMSGD_RPC_SLIM_H
#ifdef __cplusplus
extern "C" {
#endif

#ifndef _const_adspmsgd_rpc_handle
#define _const_adspmsgd_rpc_handle ((remote_handle)-1)
#endif //_const_adspmsgd_rpc_handle

static void _adspmsgd_rpc_pls_dtor(void* data) {
   remote_handle* ph = (remote_handle*)data;
   if(_const_adspmsgd_rpc_handle != *ph) {
      (void)__QAIC_REMOTE(remote_handle_close)(*ph);
      *ph = _const_adspmsgd_rpc_handle;
   }
}

static int _adspmsgd_rpc_pls_ctor(void* ctx, void* data) {
   remote_handle* ph = (remote_handle*)data;
   *ph = _const_adspmsgd_rpc_handle;
...
__QAIC_STUB_EXPORT remote_handle _adspmsgd_rpc_handle(void) {
   remote_handle* ph;
   if(adsp_pls_add_lookup) {
      if(0 == adsp_pls_add_lookup((uint32_t)_adspmsgd_rpc_handle, 0, sizeof(*ph),  _adspmsgd_rpc_pls_ctor, "adspmsgd_rpc",  _adspmsgd_rpc_pls_dtor, (void**)&ph))  {
         return *ph;
      }
      return (remote_handle)-1;
   } else if(HAP_pls_add_lookup) {
      if(0 == HAP_pls_add_lookup((uint32_t)_adspmsgd_rpc_handle, 0, sizeof(*ph),  _adspmsgd_rpc_pls_ctor, "adspmsgd_rpc",  _adspmsgd_rpc_pls_dtor, (void**)&ph))  {
         return *ph;
      }
      return (remote_handle)-1;
   }
...
uint32_t _adspmsgd_rpc_atomic_CompareAndExchange(uint32_t * volatile puDest, uint32_t uExchange, uint32_t uCompare);
...
__QAIC_STUB_EXPORT remote_handle _adspmsgd_rpc_handle(void) {
   static remote_handle handle = _const_adspmsgd_rpc_handle;
   if((remote_handle)-1 != handle) {
      return handle;
   } else {
      remote_handle tmp;
      int nErr = _adspmsgd_rpc_pls_ctor("adspmsgd_rpc", (void*)&tmp);
      if(nErr) {
         return (remote_handle)-1;
      }
      if(((remote_handle)-1 != handle) || ((remote_handle)-1 != (remote_handle)_adspmsgd_rpc_atomic_CompareAndExchange((uint32_t*)&handle, (uint32_t)tmp, (uint32_t)-1))) {
         _adspmsgd_rpc_pls_dtor(&tmp);
      }
      return handle;
   }
}
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adspmsgd_rpc_skel_invoke)(uint32_t _sc, remote_arg* _pra) __QAIC_STUB_ATTRIBUTE {
   return __QAIC_REMOTE(remote_handle_invoke)(_adspmsgd_rpc_handle(), _sc, _pra);
}
...
__QAIC_STUB_EXPORT int __QAIC_STUB(adspmsgd_rpc_init)(int heapid, uint32 ion_flags, uint32 filter, uint32 buf_size, int* buff_addr) __QAIC_STUB_ATTRIBUTE {
   uint32_t _mid = 0;
   return _stub_method(_adspmsgd_rpc_handle(), _mid, (uint32_t*)&heapid, (uint32_t*)&ion_flags, (uint32_t*)&filter, (uint32_t*)&buf_size, (uint32_t*)buff_addr);
}
#ifdef __cplusplus
}
#endif
#endif //_ADSPMSGD_RPC_STUB_H
