...
SUMMARY_FILE_NAME  = "\\temp\\Summary.txt"
DD_FILE_NAME  = "DD.txt"
...
def Error_Info_Area(OutputPath):
    ...
    f_dsp.close()
def UpdateCrashSWThread(OutputPath,TCBPointer):
    sum = OutputPath+"\\"+DSPANALYSIS_FILE_NAME
    f_sum = open(sum,'a+')
...
def QurtSytemState(OutputPath,QurtSystemState,CacheFlushFlag):
    ...
    if (((QurtSystemState) < 1) or ((QurtSystemState) > 35)):
...
IF \"ADSP is not able to service NMI\" : ADSP is in Hang State.")
    ...
    elif (((QurtSystemState) != 0x1B) and ((QurtSystemState) != 0xB) and ((QurtSystemState) != 0xC) and ((QurtSystemState) != 0x1F)):
...
def UpdateCacheStatus(f_dsp,CacheFlushFlag):
...
def CacheFlushStatus(OutputPath,CacheFlushFlag):
    
    dsp1 = OutputPath+"\\"+DSPANALYSIS_FILE_NAME
    f_dsp1 = open(dsp1,'a+')
    UpdateCacheStatus(f_dsp1,CacheFlushFlag)
    f_dsp1.close()

    dsp = OutputPath+"\\temp\\"+SUMMARY_FILE_NAME
    f_dsp = open(dsp,'a')
    UpdateCacheStatus(f_dsp,CacheFlushFlag)
    f_dsp.close()

    return
def CrashmanCommand(OutputPath,Target_Name,ramdumpfile,ADSP_BUILDROOT,Customer_Elf_Path): 
    dsp1 = OutputPath+"\\"+DSPANALYSIS_FILE_NAME
    f_dsp1 = open(dsp1,'a+')
...
def main():
    ...
    if "CacheFlushStatus" in CallFun:
        ...
        CacheFlushStatus(OutputPath,CacheFlushFlag)
    ...
    if "UpdateCrashSWThread" in CallFun:
        ...
        UpdateCrashSWThread(OutputPath,TCBPointer)
    ...
    if "CrashmanCommand" in CallFun:
        ...
        try:
            Customer_Elf_Path=sys.argv[6]
        except:
            Customer_Elf_Path=None
        CrashmanCommand(OutputPath,Target_Name,ramdumpfile,ADSP_BUILDROOT,Customer_Elf_Path)
    return
