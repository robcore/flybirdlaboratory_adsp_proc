...
if($Check=="0")
{
print OUTFILE ("    adspcrashman_dumps.bat $target $ramdump C\:\\Crashman_Temp $crmbuild $crmbuild\\build\\ms $BinaryLength $adspstartadd $choice $DumpCheckFlag $slpi $guest_os\n");
}
else
{
print OUTFILE ("    adspcrashman_dumps.bat $target $ramdump C\:\\Crashman_Temp $crmbuild $customerelf $BinaryLength $adspstartadd $choice $DumpCheckFlag $slpi $guest_os\n");
}
print OUTFILE ("    pause\n");
print OUTFILE (")\n");
print OUTFILE (") else (\n");
