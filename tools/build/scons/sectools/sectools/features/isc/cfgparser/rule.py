...
class _ImageList(object):
    ...
    def validate(self, images, *args):
        ...
        for image in image_list:
            ...
            # If both soc_hw_version and msm_part are provided, the flag field must also be there to tell the signer which one to use
            if soc_hw_version is not None and msm_part is not None and in_use_soc_hw_version is None:
                retval = False
                error_str += "\nsign_id=%s: Cannot choose between SOC_HW_VERSION and MSM_PART without a supplied IN_USE_SOC_HW_VERSION field" % (sign_id)

            # in_use_soc_hw_version specified must correspond with an existing tag
            if in_use_soc_hw_version == 0 and msm_part is None:
                retval = False
                error_str += "\nsign_id=%s: IN_USE_SOC_HW_VERSION specifies MSM_PART is used but MSM_PART tag is missing" % (sign_id)
